...
clear_poetry_env() {
    local dir=$1
    echo "Clearing virtual environment in $dir"
    echo "------------------------------------------"  # Add spacing for readability
    poetry env remove --all --directory "$dir" >/dev/null 2>&1 || true
}

# Function to remove the poetry.lock file
remove_poetry_lock() {
    local dir=$1
    echo "Removing poetry.lock in $dir"
    echo "------------------------------------------"  # Add spacing for readability
    rm -f "$dir/poetry.lock" || true
}

# Function to install dependencies
install_dependencies() {
    local dir=$1
    echo "Installing dependencies in $dir with poetry install"
    echo "------------------------------------------"  # Add spacing for readability
    poetry install --no-cache --directory "$dir" || return 1
}
