...
class IntegrationTest(BaseIntegrationTest):
    ...
    def setUpClass(cls):
        ...
        cls.bus = cls.create_bus('accent-headers', 'headers')
...
class TestWizardGet(IntegrationTest):
    ...
    def test_get(self, sysconfd, auth):
        ...
        assert_that(
            ...
            has_entries(
                ...
                configurable_status=has_entries(
                    {'accent-auth': 'ok', 'accent-provd': 'ok'}
                ),
            ),
        )
        ...
        assert_that(
            ...
            has_entries(
                configurable=False,
                configurable_status=has_entries({'accent-provd': 'fail'}),
            ),
        )
...
class TestWizardPost(IntegrationTest):
    ...
    def validate_sysconfd(self, sysconfd, data):
        sysconfd.assert_request(
            '/accentctl', method='POST', json={'accent-service': 'enable'}
        )
        sysconfd.assert_request(
            '/accentctl', method='POST', json={'accent-service': 'start'}
        )
        sysconfd.assert_request(
            '/hosts',
            method='POST',
        ...
        assert_that(
            ...
            contains_inanyorder(
                ...
                has_entries(
                    ...
                    parent_ids=[],
                    raw_config={
                        'X_accent_phonebook_ip': ip_address,
                        'ntp_enabled': True,
                        'ntp_ip': ip_address,
                    },
                ),
...
class TestWizardSteps(IntegrationTest):
    ...
    def validate_sysconfd(self, sysconfd, data):
        sysconfd.assert_no_request(
            '/accentctl', method='POST', body=json.dumps({'accent-service': 'enable'})
        )
        sysconfd.assert_no_request(
            '/hosts',
            method='POST',
