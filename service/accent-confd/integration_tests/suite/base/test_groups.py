...
@fixtures.group(accent_tenant=MAIN_TENANT)
@fixtures.group(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.groups.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_item(main)), not_(has_item(sub)))

    response = confd.groups.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

    response = confd.groups.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_get_multi_tenant(main, sub):
    response = confd.groups(main['uuid']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Group'))

    response = confd.groups(sub['uuid']).get(accent_tenant=MAIN_TENANT)
    assert_that(response.item, has_entries(**sub))

    response = confd.groups(main['id']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Group'))

    response = confd.groups(sub['id']).get(accent_tenant=MAIN_TENANT)
    assert_that(response.item, has_entries(**sub))
...
def test_create_multi_tenant():
    response = confd.groups.post(label='MyGroup', accent_tenant=SUB_TENANT)
    response.assert_created('groups')
...
@fixtures.moh(label='hidden', accent_tenant=SUB_TENANT)
...
def test_edit_multi_tenant(main, sub):
    response = confd.groups(main['id']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Group'))

    response = confd.groups(sub['id']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()

    response = confd.groups(main['uuid']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Group'))

    response = confd.groups(sub['uuid']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()


@fixtures.group(accent_tenant=MAIN_TENANT)
@fixtures.group(accent_tenant=SUB_TENANT)
@fixtures.moh(accent_tenant=MAIN_TENANT)
@fixtures.moh(accent_tenant=SUB_TENANT)
...
def test_delete_multi_tenant(main, sub):
    response = confd.groups(main['id']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Group'))

    response = confd.groups(sub['id']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
...
def test_delete_multi_tenant_by_uuid(main, sub):
    response = confd.groups(main['uuid']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Group'))

    response = confd.groups(sub['uuid']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
