...
@fixtures.device(accent_tenant=MAIN_TENANT)
@fixtures.device(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.devices.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_item(main)), not_(has_item(sub)))

    response = confd.devices.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

    response = confd.devices.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_list_unallocated(main, sub):
    ...
    response = confd.devices.unallocated.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_item(main), not_(has_item(sub))))
...
def test_get_multi_tenant(main, sub):
    response = confd.devices(sub['id']).get(accent_tenant=MAIN_TENANT)
    assert_that(response.item, has_entries(**sub))

    response = confd.devices(main['id']).get(accent_tenant=SUB_TENANT)
...
def test_create_device_multi_tenant():
    response = confd.devices.post(accent_tenant=SUB_TENANT)
    response.assert_created('devices')
...
def test_create_2_devices_with_same_mac_different_tenants(device):
    response = confd.devices.post(mac=device['mac'], accent_tenant=SUB_TENANT)
    response.assert_match(400, e.resource_exists('Device'))
...
def test_create_2_devices_with_same_ip_different_tenants(device):
    response = confd.devices.post(ip=device['ip'], accent_tenant=SUB_TENANT)
    response.assert_created('devices')
...
def test_edit_device_multi_tenant(main, sub):
    response = confd.devices(main['id']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Device'))

    response = confd.devices(sub['id']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()


@fixtures.device(accent_tenant=MAIN_TENANT)
def test_change_tenant_unallocated(main):
    response = confd.devices.unallocated(main['id']).put(accent_tenant=SUB_TENANT)
    response.assert_updated()
...
def test_edit_device_with_same_mac_different_tenants(first_device, second_device):
    response = confd.devices(first_device['id']).put(
        mac=second_device['mac'], accent_tenant=MAIN_TENANT
    )
    response.assert_match(400, e.resource_exists('Device'))
...
def test_delete_device_multi_tenant(main, sub):
    response = confd.devices(main['id']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found('Device'))

    response = confd.devices(sub['id']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
...
@mocks.provd()
@fixtures.context(label='main_ctx', accent_tenant=MAIN_TENANT)
@fixtures.context(label='sub_ctx', accent_tenant=SUB_TENANT)
@fixtures.device(accent_tenant=MAIN_TENANT)
@fixtures.device(accent_tenant=SUB_TENANT)
def test_reset_to_autoprov_multi_tenant(
    ...
    def aux(main_line, sub_line):
        with a.line_device(main_line, main_device, check=False) and a.line_device(
            ...
            response = confd.lines(sub_line['id']).get(accent_tenant=MAIN_TENANT)
            assert_that(response.item, has_entries(device_id=none()))
...
def test_synchronize_device_multi_tenant(provd, main, sub):
    response = confd.devices(main['id']).synchronize.get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found('Device'))

    timestamp = datetime.utcnow()

    response = confd.devices(sub['id']).synchronize.get(accent_tenant=MAIN_TENANT)
    response.assert_ok()
