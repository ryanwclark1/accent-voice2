...
@contextmanager
def line_and_device(endpoint_type='sip', accent_tenant=None):
    device = h.device.generate_device(accent_tenant=accent_tenant)

    line_etc = line_fellowship(endpoint_type=endpoint_type, accent_tenant=accent_tenant)
...
def test_get_device_associated_to_line_from_sub_tenant_errors():
    with line_and_device('sip', accent_tenant=MAIN_TENANT) as (
        line,
        device,
    ), a.line_device(line, device):
        response = confd.lines(line['id']).devices.get(accent_tenant=SUB_TENANT)
        response.assert_status(404)

    with line_and_device('sccp', accent_tenant=MAIN_TENANT) as (
        line,
        device,
    ), a.line_device(line, device):
        response = confd.lines(line['id']).devices.get(accent_tenant=SUB_TENANT)
...
def test_get_line_associated_to_a_device_from_sub_tenant_errors():
    with line_and_device('sip', accent_tenant=MAIN_TENANT) as (
        ...
        response = confd.devices(device['id']).lines.get(accent_tenant=SUB_TENANT)
        response.assert_status(404)
    ...
    assert_that(
        ...
        has_entries(
            ...
            raw_config=has_entries(
                X_key='',
                config_version=1,
                X_accent_user_uuid=user['uuid'],
                X_accent_phonebook_profile=line['context'],
                exten_dnd='*25',
...
@fixtures.device(accent_tenant=DEFAULT_DEVICE_TENANT)
def test_associate_sip_line_change_tenant(device):
    ...
    with line_fellowship(
        'sip', generate_username_password=True, accent_tenant=SUB_TENANT
...
def test_associate_multi_tenant(main_device, sub_device):
    with line_fellowship(
        'sip', generate_username_password=True, accent_tenant=MAIN_TENANT
    ) as (
        user,
        main_line,
        extension,
    ...
    with line_fellowship('sip', accent_tenant=MAIN_TENANT) as (
        user,
        main_line,
        extension,
        sip,
...
def test_associate_sccp_line_change_tenant(device):
    ...
    with line_fellowship('sccp', accent_tenant=SUB_TENANT) as (
        user,
        line,
        extension,
        sccp,
        ...
        device_config = provd.devices.get(device['id'], accent_tenant=SUB_TENANT)
        assert_that(device_config['tenant_uuid'], is_(SUB_TENANT))
...
def test_dissociate_multi_tenant():
    with line_and_device('sip', accent_tenant=MAIN_TENANT) as (
        main_line,
        main_device,
    ), line_and_device('sip', accent_tenant=SUB_TENANT) as (sub_line, sub_device):
        response = (
            confd.lines(main_line['id'])
            .devices(sub_device['id'])
