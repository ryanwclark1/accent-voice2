...
def test_list_multi_tenant(main, sub):
    response = confd.users(main['uuid']).callerids.outgoing.get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='User'))

    response = confd.users(sub['uuid']).callerids.outgoing.get(accent_tenant=MAIN_TENANT)
