...
@fixtures.phone_number(accent_tenant=MAIN_TENANT)
@fixtures.phone_number(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.phone_numbers.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_item(main)), not_(has_item(sub)))

    response = confd.phone_numbers.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

    response = confd.phone_numbers.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_get_multi_tenant(main, sub):
    response = confd.phone_numbers(main['uuid']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='PhoneNumber'))

    response = confd.phone_numbers(sub['uuid']).get(accent_tenant=MAIN_TENANT)
...
def test_create_range_singleton():
    ...
    response = confd.phone_numbers.ranges.post(parameters, accent_tenant=MAIN_TENANT)
    response.assert_status(201)
...
def test_edit_multi_tenant(main, sub):
    response = confd.phone_numbers(main['uuid']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='PhoneNumber'))

    response = confd.phone_numbers(sub['uuid']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
...
def test_delete_multi_tenant(main, sub):
    response = confd.phone_numbers(main['uuid']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='PhoneNumber'))

    response = confd.phone_numbers(sub['uuid']).delete(accent_tenant=MAIN_TENANT)
