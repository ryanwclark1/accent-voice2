...
def test_list_multi_tenant(main, sub):
    response = confd.funckeys.templates.get(accent_tenant=MAIN_TENANT)
    assert_that(
        response.items,
    ...
    response = confd.funckeys.templates.get(accent_tenant=SUB_TENANT)
    assert_that(
        response.items,
        all_of(has_item(sub), not_(has_item(main))),
    )

    response = confd.funckeys.templates.get(accent_tenant=MAIN_TENANT, recurse=True)
    assert_that(
        response.items,
        has_items(main, sub),
    )
...
def test_get_multi_tenant(main, sub):
    response = confd.funckeys.templates(main['id']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='FuncKeyTemplate'))

    response = confd.funckeys.templates(sub['id']).get(accent_tenant=MAIN_TENANT)
...
def test_get_position_multi_tenant(main, sub):
    response = confd.funckeys.templates(main['id'])(1).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='FuncKeyTemplate'))

    response = confd.funckeys.templates(sub['id'])(1).get(accent_tenant=MAIN_TENANT)
...
def test_edit_multi_tenant(main, sub):
    response = confd.funckeys.templates(main['id']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='FuncKeyTemplate'))

    response = confd.funckeys.templates(sub['id']).put(accent_tenant=MAIN_TENANT)
...
def test_edit_position_multi_tenant(main, sub):
    response = confd.funckeys.templates(main['id'])(1).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='FuncKeyTemplate'))
...
def test_delete_multi_tenant(main, sub):
    response = confd.funckeys.templates(main['id']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='FuncKeyTemplate'))

    response = confd.funckeys.templates(sub['id']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
...
def test_delete_position_multi_tenant(main, sub):
    response = confd.funckeys.templates(main['id'])(1).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='FuncKeyTemplate'))

    response = confd.funckeys.templates(sub['id'])(1).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
