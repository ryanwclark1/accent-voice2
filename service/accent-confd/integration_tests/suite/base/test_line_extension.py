...
@fixtures.context(accent_tenant=MAIN_TENANT, label='main-internal')
@fixtures.context(accent_tenant=SUB_TENANT, label='sub-internal')
def test_associate_multi_tenant(main_ctx, sub_ctx):
    ...
    @fixtures.line_sip(context=main_ctx, accent_tenant=MAIN_TENANT)
    @fixtures.line_sip(context=sub_ctx, accent_tenant=SUB_TENANT)
...
def test_dissociate_multi_tenant(main_ctx, sub_ctx):
    ...
    def aux(main_exten, sub_exten, main_line, sub_line):
        ...
        response = url.delete(accent_tenant=SUB_TENANT)
        response.assert_match(404, e.not_found('Extension'))
...
def test_extension_create_multi_tenant(context):
    @fixtures.line_sip(context={'name': context['name']}, accent_tenant=SUB_TENANT)
    @fixtures.line_sip(context={'name': context['name']}, accent_tenant=SUB_TENANT)
    @fixtures.line_sip()
