...
def test_search_multi_tenant(main, sub):
    response = confd.contexts(main['id']).ranges('user').get(accent_tenant=MAIN_TENANT)
    response.assert_status(200)

    response = confd.contexts(main['id']).ranges('user').get(accent_tenant=SUB_TENANT)
    response.assert_status(404)

    response = confd.contexts(sub['id']).ranges('user').get(accent_tenant=MAIN_TENANT)
    response.assert_status(200)

    response = confd.contexts(sub['id']).ranges('user').get(accent_tenant=SUB_TENANT)
    response.assert_status(200)
