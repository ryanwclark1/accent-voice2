...
@fixtures.user(accent_tenant=MAIN_TENANT)
@fixtures.user(accent_tenant=SUB_TENANT)
def test_get_user_funckeys_multi_tenant(main, sub):
    response = confd.users(main['uuid']).funckeys.get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='User'))

    response = confd.users(sub['uuid']).funckeys.get(accent_tenant=MAIN_TENANT)
    response.assert_ok()
...
def test_edit_user_funckeys_multi_tenant(main, sub):
    response = confd.users(main['uuid']).funckeys.put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='User'))

    response = confd.users(sub['uuid']).funckeys.put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
...
def test_get_user_funckeys_position_multi_tenant(main, sub):
    ...
    response = confd.users(main['uuid']).funckeys(1).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='User'))

    response = confd.users(sub['uuid']).funckeys(1).get(accent_tenant=MAIN_TENANT)
    response.assert_ok()
...
def test_edit_user_funckeys_position_multi_tenant(main, sub):
    ...
    response = confd.users(main['uuid']).funckeys(1).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='User'))

    response = confd.users(sub['uuid']).funckeys(1).put(accent_tenant=MAIN_TENANT, **body)
    response.assert_updated()
...
def test_delete_user_funckeys_position_multi_tenant(main, sub):
    ...
    response = confd.users(main['uuid']).funckeys(1).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='User'))

    response = confd.users(sub['uuid']).funckeys(1).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()


@fixtures.funckey_template(accent_tenant=MAIN_TENANT)
@fixtures.funckey_template(accent_tenant=SUB_TENANT)
def test_get_template_users_multi_tenant(main, sub):
    response = confd.funckeys.templates(main['id']).users.get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='FuncKeyTemplate'))

    response = confd.funckeys.templates(sub['id']).users.get(accent_tenant=MAIN_TENANT)
    response.assert_ok()
...
def test_get_user_templates_multi_tenant(main, sub):
    response = confd.users(main['uuid']).funckeys.templates.get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='User'))

    response = confd.users(sub['uuid']).funckeys.templates.get(accent_tenant=MAIN_TENANT)
    response.assert_ok()
...
def test_dissociate_multi_tenant(main_user, sub_user, main_template, sub_template):
    response = (
        confd.users(main_user['uuid'])
        .funckeys.templates(sub_template['id'])
        .delete(accent_tenant=SUB_TENANT)
    )
    response.assert_match(404, e.not_found('User'))
...
@fixtures.context(label='sub-tenant', accent_tenant=SUB_TENANT)
@fixtures.user(accent_tenant=MAIN_TENANT)
@fixtures.funckey_template(accent_tenant=MAIN_TENANT)
@fixtures.user(accent_tenant=SUB_TENANT)
@fixtures.funckey_template(accent_tenant=SUB_TENANT)
@fixtures.user(accent_tenant=SUB_TENANT)
@fixtures.group(accent_tenant=SUB_TENANT)
@fixtures.queue(accent_tenant=SUB_TENANT)
@fixtures.conference(accent_tenant=SUB_TENANT)
@fixtures.agent(accent_tenant=SUB_TENANT)
@fixtures.paging(accent_tenant=SUB_TENANT)
@fixtures.call_filter(accent_tenant=SUB_TENANT)
@fixtures.parking_lot(accent_tenant=SUB_TENANT)
def test_func_key_destinations_multi_tenant(
    ...
    for position, destination in funckeys.items():
        ...
        if position not in exclude_for_template:
            ...
            response = confd.funckeys.templates.post(
                template_args, accent_tenant=SUB_TENANT
            )
