...
@fixtures.context(accent_tenant=config.SUB_TENANT)
...
@fixtures.context(type='incall', accent_tenant=config.SUB_TENANT)
...
def test_given_csv_has_accent_user_fields_then_accent_user_created():
    csv = [
        {
            "firstname": "Thômas",
            "lastname": "dakin",
    ...
    accent_user = auth.users.get(user_uuid)
    assert_that(
        accent_user,
        has_entries(
            uuid=user_uuid,
...
@unittest.skip('PUT has been disabled')
@fixtures.csv_entry()
def test_update_accent_user_fields_then_accent_user_updated(entry):
...
@fixtures.csv_entry()
@fixtures.csv_entry()
def test_update_accent_auth_is_resynchronise_after_error_and_update(entry1, entry2):
    ...
    accent_user = auth.users.get(user_uuid)
    assert_that(accent_user, has_entries(uuid=user_uuid, firstname="user-import"))
...
def test_given_csv_has_error_then_accent_user_deleted():
    ...
    accent_user = auth.users.list()
    assert_that(
        accent_user['items'], is_not(has_items(has_entries(firstname=unique_firstname)))
    )
...
@fixtures.call_permission(accent_tenant=config.SUB_TENANT)
