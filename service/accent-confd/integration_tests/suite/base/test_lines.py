...
def test_get_multi_tenant(main_ctx, sub_ctx):
    with fixtures.line(context=main_ctx['name']) as main:
        with fixtures.line(context=sub_ctx['name']) as sub:
            response = confd.lines(main['id']).get(accent_tenant=SUB_TENANT)
            response.assert_match(404, e.not_found(resource='Line'))

            response = confd.lines(sub['id']).get(accent_tenant=MAIN_TENANT)
...
def test_list_multi_tenant(main_ctx, sub_ctx):
    with fixtures.line(context=main_ctx['name']) as main:
        with fixtures.line(context=sub_ctx['name']) as sub:
            response = confd.lines.get(accent_tenant=MAIN_TENANT)
            assert_that(response.items, all_of(has_item(main), not_(has_item(sub))))

            response = confd.lines.get(accent_tenant=SUB_TENANT)
            assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

            response = confd.lines.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_edit_multi_tenant(main_ctx, sub_ctx):
    with fixtures.line(context=main_ctx['name']) as main:
        with fixtures.line(context=sub_ctx['name']) as sub:
            response = confd.lines(main['id']).put(accent_tenant=SUB_TENANT)
            response.assert_match(404, e.not_found(resource='Line'))

            response = confd.lines(sub['id']).put(accent_tenant=MAIN_TENANT)
            response.assert_updated()
