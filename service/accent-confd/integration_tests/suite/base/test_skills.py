...
@fixtures.skill(accent_tenant=MAIN_TENANT)
@fixtures.skill(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.agents.skills.get(accent_tenant=MAIN_TENANT)
    assert_that(
        response.items,
        all_of(
    ...
    response = confd.agents.skills.get(accent_tenant=SUB_TENANT)
    assert_that(
        response.items,
        all_of(
    ...
    response = confd.agents.skills.get(accent_tenant=MAIN_TENANT, recurse=True)
    assert_that(
        response.items,
...
def test_get_multi_tenant(main, sub):
    response = confd.agents.skills(main['id']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Skill'))

    response = confd.agents.skills(sub['id']).get(accent_tenant=MAIN_TENANT)
...
def test_create_multi_tenant():
    response = confd.agents.skills.post(name='MySkill', accent_tenant=SUB_TENANT)
...
def test_edit_multi_tenant(main, sub):
    response = confd.agents.skills(main['id']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Skill'))

    response = confd.agents.skills(sub['id']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
...
def test_delete_multi_tenant(main, sub):
    response = confd.agents.skills(main['id']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Skill'))

    response = confd.agents.skills(sub['id']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
