...
@fixtures.agent(accent_tenant=MAIN_TENANT)
@fixtures.agent(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.agents.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_item(main)), not_(has_item(sub)))

    response = confd.agents.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

    response = confd.agents.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_get_multi_tenant(main, sub):
    response = confd.agents(main['id']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Agent'))

    response = confd.agents(sub['id']).get(accent_tenant=MAIN_TENANT)
...
def test_create_multi_tenant():
    ...
    response_sub = confd.agents.post(number=number, accent_tenant=SUB_TENANT)
    response_sub.assert_created('agents')

    assert_that(response_sub.item, has_entries(tenant_uuid=SUB_TENANT))

    response_main = confd.agents.post(number=number, accent_tenant=MAIN_TENANT)
...
@fixtures.agent(number='1234', accent_tenant=MAIN_TENANT)
@fixtures.agent(number='1234', accent_tenant=SUB_TENANT)
def test_edit_multi_tenant(main, sub):
    response = confd.agents(main['id']).put(number='1234', accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Agent'))

    response = confd.agents(sub['id']).put(number='1234', accent_tenant=MAIN_TENANT)
    response.assert_updated()
...
def test_delete_multi_tenant(main, sub):
    response = confd.agents(main['id']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Agent'))

    response = confd.agents(sub['id']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
...
def test_create_multi_tenant_same_number():
    number = '5678'
    response = confd.agents.post(number=number, accent_tenant=SUB_TENANT)
    response.assert_created('agents')
    assert_that(response.item, has_entries(tenant_uuid=SUB_TENANT))

    response = confd.agents.post(number=number, accent_tenant=SUB_TENANT2)
    response.assert_created('agents')
