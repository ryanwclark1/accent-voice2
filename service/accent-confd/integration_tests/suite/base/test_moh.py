...
def test_list_multi_tenant(main, sub):
    response = confd.moh.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_item(main)), not_(has_item(sub)))

    response = confd.moh.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

    response = confd.moh.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_get_multi_tenant(main, sub):
    response = confd.moh(main['uuid']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='MOH'))

    response = confd.moh(sub['uuid']).get(accent_tenant=MAIN_TENANT)
...
def test_edit_multi_tenant(main, sub):
    response = confd.moh(main['uuid']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='MOH'))

    response = confd.moh(sub['uuid']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
...
def test_delete_multi_tenant(main, sub):
    response = confd.moh(main['uuid']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='MOH'))

    response = confd.moh(sub['uuid']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
...
def test_add_update_delete_filename_multi_tenant(moh):
    ...
    response = (
        client.url.moh(moh['uuid'])
        .files('foo.wav')
        .put(content=VALID_WAV_FILE.getvalue(), accent_tenant=SUB_TENANT)
    )
    response.assert_match(404, e.not_found(resource='MOH'))

    response = client.url.moh(moh['uuid']).files('foo.wav').get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='MOH'))

    response = (
        client.url.moh(moh['uuid']).files('foo.wav').delete(accent_tenant=SUB_TENANT)
    )
    response.assert_match(404, e.not_found(resource='MOH'))
    ...
    response = (
        client.url.moh(moh['uuid'])
        .files('foo.wav')
        .put(content=VALID_WAV_FILE.getvalue(), accent_tenant=MAIN_TENANT)
    )
    response.assert_status(204)

    response = client.url.moh(moh['uuid']).files('foo.wav').get(accent_tenant=MAIN_TENANT)
    assert_that(response.content, equal_to(VALID_WAV_FILE.getvalue()))

    response = (
        client.url.moh(moh['uuid']).files('foo.wav').delete(accent_tenant=MAIN_TENANT)
    )
    response.assert_deleted()
