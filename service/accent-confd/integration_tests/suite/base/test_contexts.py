...
@fixtures.context(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.contexts.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_item(main)), not_(has_item(sub)))

    response = confd.contexts.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

    response = confd.contexts.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_create_out_of_tree_tenant():
    response = confd.contexts.post(
        label='MyContext', accent_tenant='00000000-0000-0000-0000-000000000000'
    )
    response.assert_status(401)


def test_create_in_authorized_tenant():
    response = confd.contexts.post(label='Context', accent_tenant=SUB_TENANT)
...
def test_create_contexts_parallel():
    def create_context():
        return confd.contexts.post(label='MyContext', accent_tenant=DELETED_TENANT)

    # create tenants (including DELETED_TENANT) in accent-auth
    BaseIntegrationTest.mock_auth.set_tenants(*ALL_TENANTS)

    # check DELETED_TENANT does not exist in accent-confd (not yet created)
    response = confd.tenants(DELETED_TENANT).get(accent_tenant=MAIN_TENANT)
