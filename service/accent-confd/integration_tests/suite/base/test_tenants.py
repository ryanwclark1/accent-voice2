...
@fixtures.trunk(accent_tenant=SUB_TENANT)  # Note: This creates the tenant in the DB
def test_get_multi_tenant(_):
    response = confd.tenants(MAIN_TENANT).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Tenant'))

    response = confd.tenants(SUB_TENANT).get(accent_tenant=MAIN_TENANT)
    assert_that(response.item, has_entries(uuid=SUB_TENANT))


@fixtures.trunk(accent_tenant=SUB_TENANT)  # Note: This creates the tenant in the DB
def test_list_multi_tenant(_):
    response = confd.tenants.get(accent_tenant=MAIN_TENANT)
    assert_that(
        response.items,
        all_of(
    ...
    response = confd.tenants.get(accent_tenant=SUB_TENANT)
    assert_that(
        response.items,
        all_of(
    ...
    response = confd.tenants.get(accent_tenant=MAIN_TENANT, recurse=True)
    assert_that(
        response.items,
...
def _reset_tenants():
    ...
    # accent-manage-db creates a tenant (with a random uuid)
...
def _create_tenant_ready_for_deletion():
    # add the tenant DELETED_TENANT in accent-auth for the authorization,
    # to be able to create the tenant in accent-confd
    BaseIntegrationTest.mock_auth.set_tenants(*ALL_TENANTS)

    with ExitStack() as stack:
        user = stack.enter_context(fixtures.user(accent_tenant=DELETED_TENANT))
        group = stack.enter_context(fixtures.group(accent_tenant=DELETED_TENANT))
        incall = stack.enter_context(fixtures.incall(accent_tenant=DELETED_TENANT))
        outcall = stack.enter_context(fixtures.outcall(accent_tenant=DELETED_TENANT))
        conference = stack.enter_context(
            fixtures.conference(accent_tenant=DELETED_TENANT)
        )
        context = stack.enter_context(
            fixtures.context(label='mycontext', accent_tenant=DELETED_TENANT)
        )
        stack.enter_context(
            fixtures.funckey_template(
                keys={'1': {'destination': {'type': 'custom', 'exten': '123'}}},
                accent_tenant=DELETED_TENANT,
            )
        )
        switchboard = stack.enter_context(
            fixtures.switchboard(accent_tenant=DELETED_TENANT)
        )
        stack.enter_context(fixtures.device(accent_tenant=DELETED_TENANT))
        queue = stack.enter_context(fixtures.queue(accent_tenant=DELETED_TENANT))
        trunk_sip = stack.enter_context(fixtures.trunk(accent_tenant=DELETED_TENANT))
        trunk_iax = stack.enter_context(fixtures.trunk(accent_tenant=DELETED_TENANT))
        trunk_custom = stack.enter_context(fixtures.trunk(accent_tenant=DELETED_TENANT))
        user_sip = stack.enter_context(
            fixtures.sip(name='endpoint_sip', accent_tenant=DELETED_TENANT)
        )
        sip = stack.enter_context(
            fixtures.sip(name='endpoint_sip2', accent_tenant=DELETED_TENANT)
        )
        iax = stack.enter_context(
            fixtures.iax(name='endpoint_iax', accent_tenant=DELETED_TENANT)
        )
        custom = stack.enter_context(
            fixtures.custom(interface='endpoint_custom', accent_tenant=DELETED_TENANT)
        )
        agent = stack.enter_context(
            fixtures.agent(number='5678', accent_tenant=DELETED_TENANT)
        )
        skill = stack.enter_context(fixtures.skill(accent_tenant=DELETED_TENANT))
        call_pickup = stack.enter_context(
            fixtures.call_pickup(accent_tenant=DELETED_TENANT)
        )
        user2 = stack.enter_context(fixtures.user(accent_tenant=DELETED_TENANT))
        stack.enter_context(
        ...
        stack.enter_context(fixtures.call_filter(accent_tenant=DELETED_TENANT))
        schedule = stack.enter_context(fixtures.schedule(accent_tenant=DELETED_TENANT))
        trunk = stack.enter_context(fixtures.trunk(accent_tenant=DELETED_TENANT))
        stack.enter_context(
            fixtures.ivr(
        ...
        voicemail = stack.enter_context(
            fixtures.voicemail(context=context['name'], accent_tenant=DELETED_TENANT)
        )
        line = stack.enter_context(
            fixtures.line_sip(
                context={'name': context['name']}, accent_tenant=DELETED_TENANT
            )
        )
