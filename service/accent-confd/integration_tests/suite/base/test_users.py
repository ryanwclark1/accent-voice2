...
def test_list_multi_tenant(main, sub):
    response = confd.users.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_item(main)), not_(has_item(sub)))

    response = confd.users.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

    response = confd.users.get(accent_tenant=MAIN_TENANT, recurse=True)
...
@fixtures.user(firstname='Alice', accent_tenant=SUB_TENANT)
@fixtures.user(firstname='Bob', accent_tenant=SUB_TENANT)
@fixtures.user(firstname='Charles', accent_tenant=SUB_TENANT)
...
def test_create_multi_tenant_moh(main_moh, sub_moh):
    ...
    response = confd.users.post(**parameters, accent_tenant=SUB_TENANT)
...
@fixtures.user(firstname='main', accent_tenant=MAIN_TENANT)
@fixtures.user(firstname='sub', accent_tenant=SUB_TENANT)
@fixtures.moh(accent_tenant=MAIN_TENANT)
@fixtures.moh(accent_tenant=SUB_TENANT)
...
@fixtures.context(accent_tenant=SUB_TENANT, name='default2')
...
@fixtures.incall(accent_tenant=MAIN_TENANT)
