...
@fixtures.context(label='main', accent_tenant=MAIN_TENANT)
@fixtures.context(label='sub', accent_tenant=SUB_TENANT)
def test_get_multi_tenant(main_ctx, sub_ctx):
    ...
    def aux(in_main, in_sub):
        response = confd.extensions(in_main['id']).get(accent_tenant=SUB_TENANT)
        response.assert_match(404, e.not_found('Extension'))

        response = confd.extensions(in_sub['id']).get(accent_tenant=SUB_TENANT)
...
@fixtures.context(accent_tenant=MAIN_TENANT)
def test_create_multi_tenant(in_main):
    response = confd.extensions.post(
        exten='1001', context=in_main['name'], accent_tenant=SUB_TENANT
    )
...
def test_edit_multi_tenant(main_ctx, sub_ctx):
    ...
    def aux(main, sub):
        response = confd.extensions(main['id']).put(accent_tenant=SUB_TENANT)
        response.assert_match(404, e.not_found(resource='Extension'))

        response = confd.extensions(sub['id']).put(accent_tenant=MAIN_TENANT)
        response.assert_updated()

        response = confd.extensions(sub['id']).put(
            context=main_ctx['name'], accent_tenant=SUB_TENANT
        )
        response.assert_status(400)
...
def test_update_and_multi_tenant(main_ctx, sub_ctx):
    ...
    def aux(in_main, in_sub):
        response = confd.extensions(in_main['id']).put(
            accent_tenant=SUB_TENANT, enabled=False
        )
        response.assert_match(404, e.not_found('Extension'))
...
def test_that_changing_tenant_is_not_possible(main_ctx, sub_ctx):
    ...
    def aux(in_main, in_sub):
        ...
        response = confd.extensions(in_main['id']).put(accent_tenant=MAIN_TENANT, **body)
...
def test_search_multi_tenant(main_ctx, sub_ctx):
    ...
    @fixtures.extension(exten='1001', context=sub_ctx['name'])
    def aux(*_):
        response = confd.extensions.get(accent_tenant=SUB_TENANT)
        assert_that(
            response.items,
            all_of(
        ...
        response = confd.extensions.get(accent_tenant=MAIN_TENANT)
        assert_that(
            response.items,
            has_items(
        ...
        response = confd.extensions.get(recurse=True, accent_tenant=MAIN_TENANT)
        assert_that(
            response.items,
            has_items(
                has_entries(context=main_ctx['name']),
...
@fixtures.context(name='main', accent_tenant=MAIN_TENANT)
@fixtures.context(name='sub', accent_tenant=SUB_TENANT)
def test_delete_multi_tenant(main_ctx, sub_ctx):
    @fixtures.extension(exten='1001', context=main_ctx['name'])
    @fixtures.extension(exten='1001', context=sub_ctx['name'])
    def aux(in_main, in_sub):
        response = confd.extensions(in_main['id']).delete(accent_tenant=SUB_TENANT)
        response.assert_match(404, e.not_found('Extension'))

        response = confd.extensions(in_sub['id']).delete(accent_tenant=SUB_TENANT)
        response.assert_deleted()
