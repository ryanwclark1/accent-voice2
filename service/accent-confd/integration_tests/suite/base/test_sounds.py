...
from . import BaseIntegrationTest, accent_sound, ari, asterisk_sound, confd
...
def setup_module():
    accent_sound.create_directory(MAIN_TENANT, name='')
    accent_sound.create_directory(SUB_TENANT, name='')
...
@fixtures.sound(accent_tenant=MAIN_TENANT)
...
@fixtures.sound(accent_tenant=MAIN_TENANT, name='category_1')
@fixtures.sound(accent_tenant=SUB_TENANT, name='category_2')
def test_list(sound1, sound2):
    sound_system = has_entries(name='system')
    response = confd.sounds.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, has_items(sound1, sound_system))

    response = confd.sounds.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, has_items(sound2, sound_system))

    response = confd.sounds.get(accent_tenant=MAIN_TENANT, recurse=True)
    assert_that(response.items, has_items(sound1, sound2, sound_system))

    response = confd.sounds.get(accent_tenant=SUB_TENANT, order='name', limit=1)
...
@fixtures.sound(accent_tenant=MAIN_TENANT)
@fixtures.sound(accent_tenant=SUB_TENANT)
def test_get_multi_tenant(main, sub):
    response = confd.sounds(main['name']).get(accent_tenant=MAIN_TENANT)
    assert_that(response.item, has_entries(name=main['name']))

    response = confd.sounds(main['name']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Sound'))

    response = confd.sounds(sub['name']).get(accent_tenant=MAIN_TENANT)
    response.assert_match(404, e.not_found(resource='Sound'))
...
def test_get_with_files(sound):
    ...
    client.url.sounds(sound['name']).files('ivr').put(
        accent_tenant=MAIN_TENANT, query_string={'format': 'slin', 'language': 'fr_FR'}
    ).assert_updated()
    client.url.sounds(sound['name']).files('ivr').put(
        accent_tenant=MAIN_TENANT, query_string={'format': 'ogg', 'language': 'en_US'}
    ).assert_updated()

    response = confd.sounds(sound['name']).get(accent_tenant=MAIN_TENANT)
    assert_that(
        response.item,
        has_entries(
            name=sound['name'],
                ...
                has_entries(
                    ...
                    formats=contains_inanyorder(
                        has_entries(
                            ...
                            language='fr_FR',
                            path=(
                                '/var/lib/accent/sounds/tenants/{tenant}/{category}/fr_FR/ivr'.format(
                        ...
                        has_entries(
                            ...
                            language='en_US',
                            path=(
                                '/var/lib/accent/sounds/tenants/{tenant}/{category}/en_US/ivr'.format(
                                    tenant=MAIN_TENANT, category=sound['name']
                                )
                            ),
                            text=None,
...
@fixtures.sound(accent_tenant=MAIN_TENANT, name='test_category_1')
@fixtures.sound(accent_tenant=SUB_TENANT, name='test_category_2')
def test_search_sound_multi_tenant(sound1, sound2):
    response = confd.sounds.get(accent_tenant=MAIN_TENANT, search='test_category')
    assert_that(
        response.items,
        contains(
    ...
    response = confd.sounds.get(
        accent_tenant=MAIN_TENANT, search='test_category', recurse=True
    )
    assert_that(
        response.items,
        contains_inanyorder(
...
def test_create_minimal_parameters():
    response = confd.sounds.post(accent_tenant=MAIN_TENANT, name='sound1')
    ...
    confd.sounds(response.item['name']).delete(accent_tenant=MAIN_TENANT).assert_deleted()
...
def test_create_unauthorized_tenant():
    response = confd.sounds.post(accent_tenant='wrong_tenant', name='sound1')
...
def test_delete_multi_tenant(main, sub):
    ...
    response = confd.sounds(main['name']).delete(accent_tenant=MAIN_TENANT)
...
def test_delete_default_sound():
    for name in DEFAULT_CATEGORY:
        accent_sound.create_directory(MAIN_TENANT, name)
        response = confd.sounds(name).delete(accent_tenant=MAIN_TENANT)
        response.assert_status(400)
...
def test_get_file(sound):
    ...
    response = (
        ...
        .files('ivr')
        .get(accent_tenant=MAIN_TENANT, format='slin', language='fr_FR')
    )
...
def test_get_file_system_errors():
    ...
    response = confd.sounds('system').files(sound['id']).get(accent_tenant=MAIN_TENANT)
    response.assert_status(404)
    ari.reset()
...
def test_add_update_delete_file_multi_tenant(main, sub):
    ...
    response = (
        ...
        .files('foo')
        .put(accent_tenant=SUB_TENANT, content='content is not checked')
    )
    response.assert_match(404, e.not_found(resource='Sound'))
    response = (
        client.url.sounds(sub['name'])
        .files('foo')
        .put(accent_tenant=MAIN_TENANT, content='content is not checked')
    )
    response.assert_match(404, e.not_found(resource='Sound'))
    response = (
        client.url.sounds(main['name'])
    ...
    # get the files
    response = client.url.sounds(main['name']).files('foo').get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Sound'))

    response = client.url.sounds(sub['name']).files('foo').get(accent_tenant=MAIN_TENANT)
    response.assert_match(404, e.not_found(resource='Sound'))
    ...
    response = (
        client.url.sounds(main['name'])
        .files('foo')
        .put(accent_tenant=SUB_TENANT, content='some new content')
    )
    response.assert_match(404, e.not_found(resource='Sound'))
    response = (
        client.url.sounds(sub['name'])
        .files('foo')
        .put(accent_tenant=MAIN_TENANT, content='some new content')
    )
    response.assert_match(404, e.not_found(resource='Sound'))

    # delete the files
    response = (
        client.url.sounds(main['name']).files('foo').delete(accent_tenant=SUB_TENANT)
    )
    response.assert_match(404, e.not_found(resource='Sound'))

    response = (
        client.url.sounds(sub['name']).files('foo').delete(accent_tenant=MAIN_TENANT)
    )
    response.assert_match(404, e.not_found(resource='Sound'))
...
def test_delete_files_with_partial_errors(sound):
    accent_sound.create_directory(
        MAIN_TENANT, '{}/fr_FR'.format(sound['name']), mode='555'
    )
    accent_sound.create_file(
        MAIN_TENANT, '{}/fr_FR/ivr.mp3'.format(sound['name']), mode='444'
    )
    client = _new_sound_file_client()
    client.url.sounds(sound['name']).files('ivr').put(
        accent_tenant=MAIN_TENANT
    ).assert_updated()

    response = confd.sounds(sound['name']).files('ivr').delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
