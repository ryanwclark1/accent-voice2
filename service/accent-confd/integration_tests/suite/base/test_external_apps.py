...
@fixtures.external_app(accent_tenant=MAIN_TENANT)
@fixtures.external_app(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.external.apps.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_item(main)), not_(has_item(sub)))

    response = confd.external.apps.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

    response = confd.external.apps.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_get_multi_tenant(main, sub):
    response = confd.external.apps(main['name']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='ExternalApp'))

    response = confd.external.apps(sub['name']).get(accent_tenant=MAIN_TENANT)
    response.assert_match(404, e.not_found(resource='ExternalApp'))
...
def test_create_multi_tenant_with_same_name(main):
    response = confd.external.apps(main['name']).post(accent_tenant=SUB_TENANT)
    response.assert_created('external_apps', location='external/apps')
...
def test_edit_multi_tenant(main, sub):
    response = confd.external.apps(main['name']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='ExternalApp'))

    response = confd.external.apps(sub['name']).put(accent_tenant=MAIN_TENANT)
    response.assert_match(404, e.not_found(resource='ExternalApp'))
...
def test_delete_multi_tenant(main, sub):
    response = confd.external.apps(main['name']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='ExternalApp'))

    response = confd.external.apps(sub['name']).delete(accent_tenant=MAIN_TENANT)
    response.assert_match(404, e.not_found(resource='ExternalApp'))
