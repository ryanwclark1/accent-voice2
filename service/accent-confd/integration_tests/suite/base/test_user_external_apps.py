...
@fixtures.user_external_app(accent_tenant=MAIN_TENANT)
@fixtures.user_external_app(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.users(main['user_uuid']).external.apps.get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='User'))

    response = confd.users(sub['user_uuid']).external.apps.get(accent_tenant=MAIN_TENANT)
...
def test_create_multi_tenant(main, sub):
    name = 'myapp'
    response = (
        confd.users(main['uuid']).external.apps(name).post(accent_tenant=SUB_TENANT)
    )
    response.assert_match(404, e.not_found(resource='User'))

    response = (
        confd.users(sub['uuid']).external.apps(name).post(accent_tenant=MAIN_TENANT)
    )
...
def test_delete_multi_tenant(main, sub):
    ...
    response = (
        confd.users(sub['user_uuid'])
        .external.apps(sub['name'])
        .delete(accent_tenant=MAIN_TENANT)
    )
    response.assert_deleted()
