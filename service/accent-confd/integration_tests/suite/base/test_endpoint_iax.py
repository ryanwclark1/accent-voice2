...
@fixtures.iax(accent_tenant=MAIN_TENANT)
@fixtures.iax(accent_tenant=SUB_TENANT)
def test_get_multi_tenant(main, sub):
    response = confd.endpoints.iax(main['id']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='IAXEndpoint'))

    response = confd.endpoints.iax(sub['id']).get(accent_tenant=MAIN_TENANT)
...
def test_list_multi_tenant(main, sub):
    response = confd.endpoints.iax.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_items(main)), not_(has_items(sub)))

    response = confd.endpoints.iax.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_items(sub), not_(has_items(main))))

    response = confd.endpoints.iax.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_edit_multi_tenant(main, sub):
    response = confd.endpoints.iax(main['id']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='IAXEndpoint'))

    response = confd.endpoints.iax(sub['id']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
...
def test_delete_multi_tenant(main, sub):
    response = confd.endpoints.iax(main['id']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='IAXEndpoint'))

    response = confd.endpoints.iax(sub['id']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
