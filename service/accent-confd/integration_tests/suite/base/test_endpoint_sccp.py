...
@fixtures.sccp(accent_tenant=MAIN_TENANT)
@fixtures.sccp(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.endpoints.sccp.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_items(main)), not_(has_items(sub)))

    response = confd.endpoints.sccp.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_items(sub), not_(has_items(main))))

    response = confd.endpoints.sccp.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_get_multi_tenant(main, sub):
    response = confd.endpoints.sccp(main['id']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='SCCPEndpoint'))

    response = confd.endpoints.sccp(sub['id']).get(accent_tenant=MAIN_TENANT)
...
def test_edit_multi_tenant(main, sub):
    response = confd.endpoints.sccp(main['id']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='SCCPEndpoint'))

    response = confd.endpoints.sccp(sub['id']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
...
def test_delete_multi_tenant(main, sub):
    response = confd.endpoints.sccp(main['id']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='SCCPEndpoint'))

    response = confd.endpoints.sccp(sub['id']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
