...
@fixtures.ingress_http(accent_tenant=MAIN_TENANT, uri='http://main')
@fixtures.ingress_http(accent_tenant=SUB_TENANT, uri='http://sub')
@fixtures.meeting(accent_tenant=MAIN_TENANT)
@fixtures.meeting(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main_ingress, sub_ingress, main, sub):
    response = confd.meetings.get(accent_tenant=MAIN_TENANT)
    assert_that(
        response.items,
        all_of(
            has_item(
    ...
    response = confd.meetings.get(accent_tenant=SUB_TENANT)
    assert_that(
        response.items,
        all_of(
    ...
    response = confd.meetings.get(accent_tenant=MAIN_TENANT, recurse=True)
    assert_that(
        response.items,
        has_items(
            has_entries(
...
def test_get_multi_tenant(_, __, main, sub):
    response = confd.meetings(main['uuid']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Meeting'))

    response = confd.meetings(sub['uuid']).get(accent_tenant=MAIN_TENANT)
...
@fixtures.ingress_http(uri='https://accent.example.com:10443')
@fixtures.user()
@fixtures.user()
...
def test_guest_endpoint_sip_creation(_):
    ...
    endpoint_sip_name = 'accent-meeting-{uuid}-guest'.format(**meeting)
    ...
    assert_that(endpoint_context, equal_to('accent-meeting-guest'))
...
def test_edit_multi_tenant(_, __, main, sub):
    response = confd.meetings(main['uuid']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Meeting'))

    response = confd.meetings(sub['uuid']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
...
def test_delete_multi_tenant(_, __, main, sub):
    response = confd.meetings(main['uuid']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Meeting'))

    response = confd.meetings(sub['uuid']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
