...
def test_given_user_in_another_tenant(user_main, user_sub):
    ...
    response = confd_csv.users.export.get(accent_tenant=MAIN_TENANT)
    assert_that(response.csv(), contains(has_entries(uuid=user_main['uuid'])))

    response = confd_csv.users.export.get(accent_tenant=SUB_TENANT)
    assert_that(response.csv(), contains(has_entries(uuid=user_sub['uuid'])))

    response = confd_csv.users.export.get(accent_tenant=UNKNOWN_TENANT)
