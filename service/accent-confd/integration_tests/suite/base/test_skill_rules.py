...
@fixtures.skill_rule(accent_tenant=MAIN_TENANT)
@fixtures.skill_rule(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.queues.skillrules.get(accent_tenant=MAIN_TENANT)
    assert_that(
        response.items,
        all_of(
    ...
    response = confd.queues.skillrules.get(accent_tenant=SUB_TENANT)
    assert_that(
        response.items,
        all_of(
    ...
    response = confd.queues.skillrules.get(accent_tenant=MAIN_TENANT, recurse=True)
    assert_that(
        response.items,
...
def test_get_multi_tenant(main, sub):
    response = confd.queues.skillrules(main['id']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='SkillRule'))

    response = confd.queues.skillrules(sub['id']).get(accent_tenant=MAIN_TENANT)
...
def test_create_multi_tenant():
    response = confd.queues.skillrules.post(name='MySkillRule', accent_tenant=SUB_TENANT)
    response.assert_created('skillrules')
...
def test_edit_multi_tenant(main, sub):
    response = confd.queues.skillrules(main['id']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='SkillRule'))

    response = confd.queues.skillrules(sub['id']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
...
def test_delete_multi_tenant(main, sub):
    response = confd.queues.skillrules(main['id']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='SkillRule'))

    response = confd.queues.skillrules(sub['id']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
