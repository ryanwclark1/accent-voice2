...
@fixtures.schedule(accent_tenant=MAIN_TENANT)
@fixtures.schedule(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.schedules.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_item(main)), not_(has_item(sub)))

    response = confd.schedules.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

    response = confd.schedules.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_get_multi_tenant(main, sub):
    response = confd.schedules(main['id']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Schedule'))

    response = confd.schedules(sub['id']).get(accent_tenant=MAIN_TENANT)
...
def test_edit_multi_tenant(main, sub):
    response = confd.schedules(main['id']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Schedule'))

    response = confd.schedules(sub['id']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
...
def test_delete_multi_tenant(main, sub):
    response = confd.schedules(main['id']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Schedule'))

    response = confd.schedules(sub['id']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
