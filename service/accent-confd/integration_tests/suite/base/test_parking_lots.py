...
@fixtures.parking_lot(accent_tenant=MAIN_TENANT)
@fixtures.parking_lot(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.parkinglots.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_item(main)), not_(has_item(sub)))

    response = confd.parkinglots.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

    response = confd.parkinglots.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_get_multi_tenant(main, sub):
    response = confd.parkinglots(main['id']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='ParkingLot'))

    response = confd.parkinglots(sub['id']).get(accent_tenant=MAIN_TENANT)
...
def test_create_multitenant_moh(main_moh, sub_moh):
    ...
    response = confd.parkinglots.post(**parameters, accent_tenant=SUB_TENANT)
...
def test_edit_multi_tenant(main, sub):
    response = confd.parkinglots(main['id']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='ParkingLot'))

    response = confd.parkinglots(sub['id']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
...
@fixtures.parking_lot(slots_start='801', slots_end='850', accent_tenant=SUB_TENANT)
...
def test_delete_multi_tenant(main, sub):
    response = confd.parkinglots(main['id']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='ParkingLot'))

    response = confd.parkinglots(sub['id']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
