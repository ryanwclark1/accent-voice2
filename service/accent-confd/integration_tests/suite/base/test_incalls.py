...
@fixtures.incall(accent_tenant=SUB_TENANT)
def test_search_multi_tenant(main, sub):
    response = confd.incalls.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_item(main)), not_(has_item(sub)))

    response = confd.incalls.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

    response = confd.incalls.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_edit_multi_tenant(main, sub):
    response = confd.incalls(main['id']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Incall'))

    response = confd.incalls(sub['id']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
...
def test_delete_multi_tenant(main, sub):
    response = confd.incalls(main['id']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Incall'))

    response = confd.incalls(sub['id']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
