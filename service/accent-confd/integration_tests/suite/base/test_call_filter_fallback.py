...
def test_get_multi_tenant(main, sub):
    response = confd.callfilters(main['id']).fallbacks.get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='CallFilter'))

    response = confd.callfilters(sub['id']).fallbacks.get(accent_tenant=MAIN_TENANT)
    response.assert_ok()
...
def test_edit_multi_tenant(main, sub):
    response = confd.callfilters(main['id']).fallbacks.put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='CallFilter'))

    response = confd.callfilters(sub['id']).fallbacks.put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
