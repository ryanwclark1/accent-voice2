...
@fixtures.application(accent_tenant=MAIN_TENANT)
@fixtures.application(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.applications.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_item(main)), not_(has_item(sub)))

    response = confd.applications.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

    response = confd.applications.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_get_multi_tenant(main, sub):
    response = confd.applications(main['uuid']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Application'))

    response = confd.applications(sub['uuid']).get(accent_tenant=MAIN_TENANT)
...
def test_create_with_moh(main_moh, sub_moh):
    ...
    response = confd.applications.post(**parameters_main, accent_tenant=SUB_TENANT)
    response.assert_created('applications')
    confd.applications(response.item['uuid']).delete().assert_deleted()
    ...
    response = confd.applications.post(**parameters_sub, accent_tenant=SUB_TENANT)
    response.assert_match(400, e.not_found(resource='MOH'))
...
def test_edit_multi_tenant(main, sub):
    response = confd.applications(main['uuid']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Application'))

    response = confd.applications(sub['uuid']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
...
def test_delete_multi_tenant(main, sub):
    response = confd.applications(main['uuid']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Application'))

    response = confd.applications(sub['uuid']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
