...
@fixtures.context(accent_tenant=DELETED_TENANT)
...
def test_remove_user_with_line(context, user):
    with fixtures.line_sip(context=context, accent_tenant=DELETED_TENANT) as line:
...
def test_not_reset_default_templates_when_exist():
    with BaseIntegrationTest.create_auth_tenant(CREATED_TENANT):
        ...
        response = confd.endpoints.sip.templates(uuid_1).put(
            asterisk_id='42',
            accent_tenant=CREATED_TENANT,
        )

        BaseIntegrationTest.sync_db()

        response = confd.endpoints.sip.templates(uuid_1).get(accent_tenant=CREATED_TENANT)
