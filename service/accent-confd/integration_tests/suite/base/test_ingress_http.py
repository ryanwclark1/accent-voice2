...
@fixtures.ingress_http(uri='search', accent_tenant=MAIN_TENANT)
@fixtures.ingress_http(uri='hidden', accent_tenant=SUB_TENANT)
...
@fixtures.ingress_http(accent_tenant=MAIN_TENANT)
@fixtures.ingress_http(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.ingresses.http.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_items(main)), not_(has_items(sub)))

    response = confd.ingresses.http.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_items(sub), not_(has_items(main))))

    response = confd.ingresses.http.get(accent_tenant=MAIN_TENANT, recurse=True)
    assert_that(response.items, has_items(main, sub))


@fixtures.ingress_http(accent_tenant=MAIN_TENANT, uri='main')
@fixtures.ingress_http(accent_tenant=SUB_TENANT, uri='sub')
def test_list_fallback_tenant(main, sub):
    response = confd.ingresses.http.get(accent_tenant=SUB_TENANT, view='default')
    assert_that(
        response.items,
        contains_exactly(has_entries(uri='sub')),
    )
    response = confd.ingresses.http.get(accent_tenant=SUB_TENANT, view='fallback')
    assert_that(
        response.items,
        contains_exactly(has_entries(uri='sub')),
    )
...
@fixtures.ingress_http(uri='sort1', accent_tenant=MAIN_TENANT)
@fixtures.ingress_http(uri='sort2', accent_tenant=SUB_TENANT)
...
def test_get_multi_tenant(main, sub):
    response = confd.ingresses.http(main['uuid']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='IngressHTTP'))

    response = confd.ingresses.http(sub['uuid']).get(accent_tenant=MAIN_TENANT)
...
def test_delete_multi_tenant(main, sub):
    response = confd.ingresses.http(main['uuid']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='IngressHTTP'))

    response = confd.ingresses.http(sub['uuid']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
