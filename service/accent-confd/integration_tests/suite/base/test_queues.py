...
@fixtures.queue(accent_tenant=MAIN_TENANT)
@fixtures.queue(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.queues.get(accent_tenant=MAIN_TENANT)
    assert_that(
        response.items,
        all_of(
    ...
    response = confd.queues.get(accent_tenant=SUB_TENANT)
    assert_that(
        response.items,
        all_of(
    ...
    response = confd.queues.get(accent_tenant=MAIN_TENANT, recurse=True)
    assert_that(
        response.items,
...
def test_get_multi_tenant(main, sub):
    response = confd.queues(main['id']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Queue'))

    response = confd.queues(sub['id']).get(accent_tenant=MAIN_TENANT)
...
def test_create_multi_tenant():
    response = confd.queues.post(name='MyQueue', accent_tenant=SUB_TENANT)
    response.assert_created('queues')
...
def test_create_multi_tenant_moh(main_moh, sub_moh):
    ...
    response = confd.queues.post(**parameters, accent_tenant=SUB_TENANT)
...
def test_edit_multi_tenant(main, sub):
    response = confd.queues(main['id']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Queue'))

    response = confd.queues(sub['id']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
...
def test_delete_multi_tenant(main, sub):
    response = confd.queues(main['id']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Queue'))

    response = confd.queues(sub['id']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
