...
def test_create_multi_tenant(in_main):
    response = confd.voicemails.post(context=in_main['name'], accent_tenant=SUB_TENANT)
...
@fixtures.voicemail(
    ...
    email="searchemail@accentvoice.io",
    pager="searchpager@accentvoice.io",
)
@fixtures.voicemail(
    name="HiddenVoicemail",
    email="hiddenvoicemail@accentvoice.io",
    pager="hiddenpager@accentvoice.io",
)
...
def test_list_multi_tenant(main_ctx, sub_ctx):
    with fixtures.voicemail(context=main_ctx['name']) as main:
        with fixtures.voicemail(context=sub_ctx['name']) as sub:
            response = confd.voicemails.get(accent_tenant=MAIN_TENANT)
            assert_that(response.items, all_of(has_item(main), not_(has_item(sub))))

            response = confd.voicemails.get(accent_tenant=SUB_TENANT)
            assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

            response = confd.voicemails.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_get_multi_tenant(main_ctx, sub_ctx):
    with fixtures.voicemail(context=main_ctx['name']) as main:
        with fixtures.voicemail(context=sub_ctx['name']) as sub:
            response = confd.voicemails(main['id']).get(accent_tenant=SUB_TENANT)
            response.assert_match(404, e.not_found(resource='Voicemail'))

            response = confd.voicemails(sub['id']).get(accent_tenant=MAIN_TENANT)
...
def test_edit_multi_tenant(main_ctx, sub_ctx):
    with fixtures.voicemail(context=main_ctx['name']) as main:
        with fixtures.voicemail(context=sub_ctx['name']) as sub:
            response = confd.voicemails(main['id']).put(accent_tenant=SUB_TENANT)
            response.assert_match(404, e.not_found(resource='Voicemail'))

            response = confd.voicemails(sub['id']).put(accent_tenant=MAIN_TENANT)
            response.assert_updated()
...
def test_delete_multi_tenant(main_ctx, sub_ctx):
    with fixtures.voicemail(context=main_ctx['name']) as main:
        with fixtures.voicemail(context=sub_ctx['name']) as sub:
            response = confd.voicemails(main['id']).delete(accent_tenant=SUB_TENANT)
            response.assert_match(404, e.not_found(resource='Voicemail'))

            response = confd.voicemails(sub['id']).delete(accent_tenant=MAIN_TENANT)
            response.assert_deleted()
