...
@fixtures.sip_template(accent_tenant=MAIN_TENANT)
@fixtures.sip_template(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.endpoints.sip.templates.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_items(main)), not_(has_items(sub)))

    response = confd.endpoints.sip.templates.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_items(sub), not_(has_items(main))))

    response = confd.endpoints.sip.templates.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_get_multi_tenant(main, sub):
    response = confd.endpoints.sip.templates(main['uuid']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='SIPEndpointTemplate'))

    response = confd.endpoints.sip.templates(sub['uuid']).get(accent_tenant=MAIN_TENANT)
...
def test_create_multi_tenant():
    response = confd.endpoints.sip.templates.post(accent_tenant=SUB_TENANT)
    response.assert_created()
...
def test_edit_multi_tenant(main, sub):
    response = confd.endpoints.sip.templates(main['uuid']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='SIPEndpointTemplate'))

    response = confd.endpoints.sip.templates(sub['uuid']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()
