...
@fixtures.trunk(accent_tenant=MAIN_TENANT)
@fixtures.trunk(accent_tenant=SUB_TENANT)
def test_list_multi_tenant(main, sub):
    response = confd.trunks.get(accent_tenant=MAIN_TENANT)
    assert_that(response.items, all_of(has_item(main)), not_(has_item(sub)))

    response = confd.trunks.get(accent_tenant=SUB_TENANT)
    assert_that(response.items, all_of(has_item(sub), not_(has_item(main))))

    response = confd.trunks.get(accent_tenant=MAIN_TENANT, recurse=True)
...
def test_get_multi_tenant(main, sub):
    response = confd.trunks(main['id']).get(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Trunk'))

    response = confd.trunks(sub['id']).get(accent_tenant=MAIN_TENANT)
...
def test_create_multi_tenant(context):
    response = confd.trunks.post(context=context['name'], accent_tenant=SUB_TENANT)
...
def test_edit_multi_tenant(context, main, sub):
    response = confd.trunks(main['id']).put(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Trunk'))

    response = confd.trunks(sub['id']).put(accent_tenant=MAIN_TENANT)
    response.assert_updated()

    response = confd.trunks(sub['id']).put(
        context=context['name'], accent_tenant=MAIN_TENANT
    )
    response.assert_match(400, e.different_tenant())
...
def test_delete_multi_tenant(main, sub):
    response = confd.trunks(main['id']).delete(accent_tenant=SUB_TENANT)
    response.assert_match(404, e.not_found(resource='Trunk'))

    response = confd.trunks(sub['id']).delete(accent_tenant=MAIN_TENANT)
    response.assert_deleted()
