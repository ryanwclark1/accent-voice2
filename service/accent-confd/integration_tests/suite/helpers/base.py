...
from accent_test_helpers.filesystem import FileSystemClient as GenericFileSystemClient
...
class IntegrationTest(AssetLaunchingTestCase):
    ...
    @classmethod
    def sync_db(cls):
        cls.docker_exec(['accent-confd-sync-db', '--debug'])

    @classmethod
    def purge_meetings(cls):
        cls.docker_exec(['accent-confd-purge-meetings', '--debug'])

    @classmethod
    def purge_meeting_authorizations(cls):
        return cls.docker_exec(
            ['accent-confd-purge-meetings', '--debug', '--authorizations-only'],
        )
    ...
    def delete_auth_tenant(cls, tenant_uuid):  # tenant_uuid improve readability
        ...
        yield
        # NOTE: re-add DELETED_TENANT to be able to make
    ...
    def confd_with_config(cls, config):
        ...
        config_file = f'/etc/accent-confd/conf.d/10-{name}.yml'
