...
class TestSelfCheck(unittest.TestCase):
    ...
    @patch(
        'accent_confd.service_discovery.requests.get', return_value=Mock(status_code=404)
    )
    ...
    @patch(
        'accent_confd.service_discovery.requests.get', return_value=Mock(status_code=200)
    )
    ...
    @patch(
        'accent_confd.service_discovery.requests.get', return_value=Mock(status_code=401)
    )
    ...
    @patch('accent_confd.service_discovery.requests.get', side_effect=Exception)
