...
class TestSysconfdClient(TestCase):
    def setUp(self):
        ...
        session_init_patch = patch('accent_confd._sysconfd.requests.Session')
    ...
    def test_accent_service_start(self):
        self.session.request.return_value = Mock(status_code=200)
        expected_json = {'accent-service': 'start'}

        self.client.accent_service_start()
        self.client.flush()

        method, url, json = self.extract_request()
        expected_url = "http://localhost:8668/accentctl"
        self.assertEqual(method, "POST")
        self.assertEqual(url, expected_url)
        self.assertEqual(json, expected_json)

    def test_accent_service_enable(self):
        self.session.request.return_value = Mock(status_code=200)
        expected_json = {'accent-service': 'enable'}

        self.client.accent_service_enable()
        self.client.flush()
