...
from accent_confd.config import DEFAULT_CONFIG
...
def main():
    port = get_accent_confd_port()

    for _ in iterations(TIMEOUT, INTERVAL):
        if tcp_port_is_open(HOST, port):
            exit(0)
    else:
        print(f'Could not connect to accent-confd on {HOST}:{port}', file=sys.stderr)
        exit(1)
