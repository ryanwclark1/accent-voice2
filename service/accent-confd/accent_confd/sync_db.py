...
from accent_dao import init_db_from_config, tenant_dao
...
from accent_dao.resources.tenant import dao as tenant_resources_dao

from accent_confd._sysconfd import SysconfdPublisher
from accent_confd.config import DEFAULT_CONFIG, _load_key_file
from accent_confd.plugins.event_handlers.service import DefaultSIPTemplateService

logger = logging.getLogger('accent-confd-sync-db')
...
def main():
    ...
    with session_scope() as session:
        ...
        logger.debug('accent-confd tenants: %s', confd_tenants)
        logger.debug('accent-auth tenants: %s', auth_tenants)
