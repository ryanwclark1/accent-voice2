...
class SysconfdPublisher:
    ...
    def restart_provd(self):
        self.service_action('accent-provd', 'restart')

    def restart_phoned(self):
        self.service_action('accent-phoned', 'restart')

    def dhcpd_update(self):
        url = "{}/dhcpd_update".format(self.base_url)
        self.add_request('GET', url)

    def accent_service_start(self):
        data = {'accent-service': 'start'}
        url = "{}/accentctl".format(self.base_url)
        self.add_request('POST', url, json=data)

    def accent_service_enable(self):
        data = {'accent-service': 'enable'}
        url = "{}/accentctl".format(self.base_url)
        self.add_request('POST', url, json=data)
