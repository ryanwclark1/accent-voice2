...
# AccentUser need to be an associator to have the user uuid
class AccentUserAssociator(Associator):
    def associate(self, entry):
        user = entry.get_resource('user')
        accent_user = entry.get_resource('accent_user')
        accent_user['uuid'] = user.uuid
        return self.service.create(accent_user)
