...
from accent_confd import sysconfd
...
DEFAULT_ROOT_POLICY = 'accent_default_master_user_policy'
...
class WizardService:
    ...
    def get(self):
        ...
        configurable_status = {
            'accent-provd': provd_status,
            'accent-auth': auth_status,
        }

        return {
            'configured': wizard_db.get_accent_configured().configured,
            'configurable': all(
    ...
    def create(self, wizard):
        ...
        wizard_db.set_accent_configured()
        self.notifier.created()
        wizard['accent_uuid'] = self.infos_dao.get().uuid
        return wizard
    ...
    def _send_sysconfd_cmd(self, hostname, domain, nameserver, steps):
        if steps['manage_services']:
            self.sysconfd.accent_service_enable()
            self.sysconfd.flush()
            self.sysconfd.accent_service_start()
            self.sysconfd.flush()
    ...
    def _initialize_admin(self, username, password):
        ...
        # user will be in the same tenant as accent-auth-cli
