...
def incoming_queue_set_features(agi, cursor, args):
    ...
    referer = agi.get_variable('ACCENT_FWD_REFERER')
    ...
    agi.set_variable('ACCENT_REAL_NUMBER', queue.number)
    agi.set_variable('ACCENT_REAL_CONTEXT', queue.context)
    agi.set_variable('ACCENT_QUEUENAME', queue.name)
    agi.set_variable('ACCENT_QUEUEOPTIONS', options)
    agi.set_variable('ACCENT_QUEUENEEDANSWER', needanswer)
    agi.set_variable('ACCENT_QUEUEURL', queue.url)
    agi.set_variable('ACCENT_QUEUEANNOUNCEOVERRIDE', queue.announceoverride)
    ...
    agi.set_variable('ACCENT_QUEUEPREPROCESS_SUBROUTINE', preprocess_subroutine)
    agi.set_variable('ACCENT_QUEUETIMEOUT', timeout)
    ...
    agi.set_variable('ACCENT_QUEUESTATUS', 'ok')

    # schedule
    # 'incall' schedule has priority over queue's schedule
    path = agi.get_variable('ACCENT_PATH')
    if path is None or len(path) == 0:
        agi.set_variable('ACCENT_PATH', 'queue')
        agi.set_variable('ACCENT_PATH_ID', queue.id)

    # pickup
    pickups = queue.pickupgroups()
    agi.set_variable('ACCENT_PICKUPGROUP', ','.join(pickups))
...
def set_call_record_side(agi, queue):
    agi.set_variable('ACCENT_CALL_RECORD_SIDE', 'caller')
    agi.set_variable('__ACCENT_LOCAL_CHAN_MATCH_UUID', str(uuid4()))
