...
CONFIG_FILE = "/etc/accent/asterisk/accent_fax.conf"
...
#   dstnum -- the content of the ACCENT_DSTNUM dialplan variable
#   args -- args specific to the backend
def _new_mail_backend(
    subject: str, content_file: str, email_from: str, email_realname: str = 'Accent Fax'
) -> Backend:
...
def handle_fax(agi: FastAGI, cursor: DictCursor, args: list[str]) -> None:
    try:
        faxfile = args[0]
        dstnum = agi.get_variable("ACCENT_DSTNUM")
...
def _build_backends_list(
    ...
    for backend_id in backend_ids:
        if backend_id in available_backends:
            ...
            logger.warning(
                'Destination %s is referencing unknown backend "%s" in accent_fax.conf',
                destination,
                backend_id,
            )
    return backends
