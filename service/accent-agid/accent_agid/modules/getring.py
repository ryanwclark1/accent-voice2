...
CONFIG_FILE = "/etc/accent/asterisk/accent_ring.conf"
...
def getring(agi: agid.FastAGI, cursor: DictCursor, args: list[str]) -> None:
    dstnum = agi.get_variable('ACCENT_REAL_NUMBER')
    context = agi.get_variable('ACCENT_REAL_CONTEXT')
    origin = agi.get_variable('ACCENT_CALLORIGIN')
    referer = agi.get_variable('ACCENT_FWD_REFERER').split(':', 1)[0]
    forwarded = agi.get_variable('ACCENT_CALLFORWARDED')
    ...
    agi.set_variable('ACCENT_RINGTYPE', "")
    ...
    try:
        ...
        agi.set_variable('ACCENT_RINGTYPE', ringtype)
        agi.set_variable('ACCENT_PHONETYPE', phonetype)
