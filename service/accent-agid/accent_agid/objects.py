...
from accent_agid.schedule import (
...
class Queue:
    ...
    def set_dial_actions(self):
        ...
        if action.action in ['voicemail', 'sound']:
            self.agi.set_variable("ACCENT_QUEUELOG_EVENT", "REROUTEGUIDE")
...
class DialAction:
    ...
    def set_agi_variables(
        ...
        agi.set_variable(f"ACCENT_FWD_{xtype}_ACTION", action)

        # Sometimes, it's useful to know whether these variables were
        # set manually, or by this object.
        if isda:
            agi.set_variable(f"ACCENT_FWD_{xtype}_ISDA", "1")

        action_arg_1 = actionarg1.replace('|', ';') if actionarg1 else ""
        action_arg_2 = actionarg2 or ""

        agi.set_variable(f"ACCENT_FWD_{xtype}_ACTIONARG1", action_arg_1)
        agi.set_variable(f"ACCENT_FWD_{xtype}_ACTIONARG2", action_arg_2)
...
class CallerID:
    ...
    def rewrite(self, force_rewrite):
        ...
        the ACCENT_CID_REWRITTEN channel variable in some cases.

        @force_rewrite:
            True <=> CID modification is always allowed in this case.
                ACCENT_CID_REWRITTEN is neither taken into account nor
                written.
            False <=> CID modification is only allowed if the channel
                variable ACCENT_CID_REWRITTEN is not set prior to the
                call to this method.  If the CID modification really
                took place, ACCENT_CID_REWRITTEN is created.
        """
        if not self.mode:
            return

        cidrewritten = self.agi.get_variable('ACCENT_CID_REWRITTEN')
        ...
        if force_rewrite or not cidrewritten:
            ...
            if not force_rewrite:
                self.agi.set_variable('ACCENT_CID_REWRITTEN', 1)
