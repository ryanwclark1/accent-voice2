...
class TestUserFeatures(_BaseTestCase):
    def setUp(self):
        ...
        self._variables = {
            'ACCENT_USERID': '42',
            'ACCENT_DSTID': '33',
            'ACCENT_CALLORIGIN': 'my_origin',
            'ACCENT_SRCNUM': '1000',
            'ACCENT_DSTNUM': '1003',
            'ACCENT_DST_EXTEN_ID': '983274',
            'ACCENT_BASE_CONTEXT': 'default',
            'ACCENT_CALL_RECORD_ACTIVE': '0',
            'ACCENT_USER_MOH_UUID': '00000000-feed-dada-1ced-c0ffee000000',
        }
    ...
    @patch('accent_agid.objects.User', Mock())
    def test_set_members(self):
        ...
        with patch.multiple(
            ...
            self.assertEqual(userfeatures._userid, self._variables['ACCENT_USERID'])
            self.assertEqual(userfeatures._dstid, self._variables['ACCENT_DSTID'])
            self.assertEqual(userfeatures._zone, self._variables['ACCENT_CALLORIGIN'])
            self.assertEqual(userfeatures._srcnum, self._variables['ACCENT_SRCNUM'])
            self.assertEqual(userfeatures._dstnum, self._variables['ACCENT_DSTNUM'])
            self.assertEqual(
                userfeatures._moh_uuid, self._variables['ACCENT_USER_MOH_UUID']
            )
    ...
    def test_set_options_with_moh(self):
        ...
        self._agi.set_variable.assert_called_once_with(
            'ACCENT_CALLOPTIONS', 'm(my-music-class)'
        )
    ...
    def test_set_caller(self):
        ...
        userfeatures._userid = self._variables['ACCENT_USERID']
        ...
        with patch.object(objects.User, '__init__') as user_init:
            ...
            user_init.assert_called_with(
                self._agi, self._cursor, int(self._variables['ACCENT_USERID'])
            )
    ...
    def test_set_call_record_enabled_already_recorded(self):
        self._variables['ACCENT_CALL_RECORD_ACTIVE'] = '1'
    ...
    def test_set_call_record_enabled_from_group(self):
        self._variables['ACCENT_FROMGROUP'] = '1'
    ...
    @patch('accent_agid.handlers.userfeatures.extension_dao')
    @patch('accent_agid.handlers.userfeatures.line_extension_dao')
    @patch('accent_agid.handlers.userfeatures.line_dao')
    @patch('accent_agid.handlers.userfeatures.user_line_dao')
    def test_set_line(self, user_line_dao, line_dao, line_extension_dao, extension_dao):
        ...
        userfeatures._dstid = self._variables['ACCENT_DSTID']
    ...
    def test_set_user(self):
        ...
        with patch.multiple(
            userfeatures,
            _set_accent_user_name=Mock(),
            _set_accent_redirecting_info=Mock(),
            _set_accent_uuid=Mock(),
        ):
            userfeatures._set_user()

            self.assertTrue(userfeatures._user is None)
            self.assertEqual(userfeatures._set_accent_user_name.call_count, 0)  # type: ignore
            ...
            with patch.object(objects.User, '__init__') as user_init:
                ...
                userfeatures._set_accent_user_name.assert_called_once()  # type: ignore
                userfeatures._set_accent_redirecting_info.assert_called_once()  # type: ignore
                userfeatures._set_accent_uuid.assert_called_once()  # type: ignore
    ...
    def test_set_accent_user_name(self):
        userfeatures = UserFeatures(self._agi, self._cursor, self._args)

        userfeatures._set_accent_user_name()
    ...
    def test_set_accent_redirecting_info_full_callerid(self):
        userfeatures = UserFeatures(self._agi, self._cursor, self._args)
        ...
        userfeatures._set_accent_redirecting_info()

        expected_calls = [
            call('ACCENT_DST_REDIRECTING_NAME', 'Foobar'),
            call('ACCENT_DST_REDIRECTING_NUM', '123'),
        ]
        self.assertEqual(self._agi.set_variable.call_args_list, expected_calls)

    def test_set_accent_redirecting_info_no_callerid(self):
        userfeatures = UserFeatures(self._agi, self._cursor, self._args)
        ...
        expected_calls = [
            call('ACCENT_DST_REDIRECTING_NAME', 'First Last'),
            call('ACCENT_DST_REDIRECTING_NUM', '42'),
        ]
        self.assertEqual(self._agi.set_variable.call_args_list, expected_calls)

    def test_set_accent_redirecting_info_line(self):
        userfeatures = UserFeatures(self._agi, self._cursor, self._args)
        ...
        expected_calls = [
            call('ACCENT_DST_REDIRECTING_NAME', 'Foobar'),
            call('ACCENT_DST_REDIRECTING_NUM', '32'),
        ]
        self.assertEqual(self._agi.set_variable.call_args_list, expected_calls)

    def test_set_callfilter_ringseconds_zero(self):
        userfeatures = UserFeatures(self._agi, self._cursor, self._args)
        name = 'ACCENT_CALLFILTER_TIMEOUT'
        value = 0
    ...
    def test_set_video_enabled(self):
        ...
        assert_that(self._agi.set_variable.called_once_with('ACCENT_VIDEO_ENABLED', '1'))
...
class TestSetForwardNoAnswer(_BaseTestCase):
    def test_forward_no_answer_to_a_user_dialaction(self):
        ...
        assert_that(
            ...
            contains_exactly(
                call('ACCENT_FWD_USER_NOANSWER_ACTION', 'user'),
                call('ACCENT_FWD_USER_NOANSWER_ISDA', '1'),
                call('ACCENT_FWD_USER_NOANSWER_ACTIONARG1', '5'),
                call('ACCENT_FWD_USER_NOANSWER_ACTIONARG2', ''),
            ),
        )
    ...
    def test_forward_no_answer_to_a_user_from_exten(self):
        ...
        assert_that(
            self._agi.set_variable.call_args_list,
            contains_exactly(
                call('ACCENT_FWD_USER_NOANSWER_ACTION', 'extension'),
                call('ACCENT_FWD_USER_NOANSWER_ACTIONARG1', '555'),
                call('ACCENT_FWD_USER_NOANSWER_ACTIONARG2', sentinel.context),
            ),
        )
    ...
    def test_setrna_exten_disabled_noanswer_enabled(self):
        ...
        with patch.multiple(
            ...
            assert_that(self._agi.set_variable.called_once_with('ACCENT_ENABLERNA', True))
...
class TestSetForwardBusy(_BaseTestCase):
    def test_forward_busy_to_a_user_dialaction(self):
        ...
        assert_that(
            self._agi.set_variable.call_args_list,
            contains_exactly(
                call('ACCENT_FWD_USER_BUSY_ACTION', 'user'),
                call('ACCENT_FWD_USER_BUSY_ISDA', '1'),
                call('ACCENT_FWD_USER_BUSY_ACTIONARG1', '5'),
                call('ACCENT_FWD_USER_BUSY_ACTIONARG2', ''),
            ),
        )
    ...
    def test_forward_busy_to_a_user_from_exten(self):
        ...
        assert_that(
            self._agi.set_variable.call_args_list,
            contains_exactly(
                call('ACCENT_FWD_USER_BUSY_ACTION', 'extension'),
                call('ACCENT_FWD_USER_BUSY_ACTIONARG1', '666'),
                call('ACCENT_FWD_USER_BUSY_ACTIONARG2', sentinel.context),
            ),
        )
    ...
    def test_set_rbusy_exten_disabled_noanswer_enabled(self):
        ...
        with patch.multiple(
            ...
            assert_that(
                self._agi.set_variable.called_once_with('ACCENT_ENABLEBUSY', True)
            )
