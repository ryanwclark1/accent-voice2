...
from accent_agid.objects import sanitize_aliased_column, sanitize_column
...
class GroupFeatures(Handler):
    ...
    def _set_vars(self) -> None:
        self._agi.set_variable('ACCENT_REAL_NUMBER', self._exten)
        self._agi.set_variable('ACCENT_REAL_CONTEXT', self._context)
        self._agi.set_variable('ACCENT_GROUPNAME', self._name)
        self._agi.set_variable('ACCENT_GROUP_LABEL', self._label)
        self._agi.set_variable('ACCENT_GROUP_STRATEGY', self._group_strategy)
        self._agi.set_variable('ACCENT_GROUP_MAX_CALLS', self._max_calls)
        if self._musicclass:
    ...
    def _set_options(self) -> None:
        ...
        self._agi.set_variable('ACCENT_GROUPOPTIONS', options)
        self._agi.set_variable('ACCENT_GROUPNEEDANSWER', needanswer)

    def _set_preprocess_subroutine(self) -> None:
        if self._preprocess_subroutine:
            self._agi.set_variable(
                'ACCENT_GROUPPREPROCESS_SUBROUTINE', self._preprocess_subroutine
            )

    def _set_timeout(self) -> None:
        if self._timeout:
            self._agi.set_variable('ACCENT_GROUPTIMEOUT', self._timeout)
        else:
            self._agi.set_variable('ACCENT_GROUPTIMEOUT', "")

        if self._user_timeout:
            self._agi.set_variable('ACCENT_GROUP_USER_TIMEOUT', self._user_timeout)
        else:
            self._agi.set_variable('ACCENT_GROUP_USER_TIMEOUT', "")

        if self._group_retry_delay:
            self._agi.set_variable('ACCENT_GROUP_RETRY_DELAY', self._group_retry_delay)
        else:
            self._agi.set_variable('ACCENT_GROUP_RETRY_DELAY', "0")
    ...
    def _set_schedule(self) -> None:
        path = self._agi.get_variable('ACCENT_PATH')
        if path is None or len(path) == 0:
            self._agi.set_variable('ACCENT_PATH', 'group')
            self._agi.set_variable('ACCENT_PATH_ID', self._id)

    def _set_call_record_side(self) -> None:
        self._agi.set_variable('ACCENT_CALL_RECORD_SIDE', 'caller')
        self._agi.set_variable('__ACCENT_LOCAL_CHAN_MATCH_UUID', str(uuid4()))
