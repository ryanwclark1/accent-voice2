...
_SELECTED_CALLED_ID_HEADER = 'X-Accent-Selected-Caller-ID'
...
class OutgoingFeatures(Handler):
    ...
    def _set_tenant_country(self) -> None:
        ...
        self._agi.set_variable('ACCENT_TENANT_COUNTRY', country)
    ...
    def _set_anonymous(self):
        ...
        self._agi.set_variable('ACCENT_OUTGOING_ANONYMOUS_CALL', '1')
        if self.outcall.callerid:
            _, pai_tel = objects.CallerID.parse(self.outcall.callerid)
            if pai_tel:
                self._agi.set_variable('_ACCENT_OUTCALL_PAI_NUMBER', pai_tel)
