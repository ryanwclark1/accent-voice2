...
class TestBuildSIPInterface(unittest.TestCase):
    ...
    @patch('accent_agid.helpers._is_webrtc', Mock(return_value=False))
    @patch('accent_agid.helpers._has_mobile_connection', Mock(return_value=False))
    ...
    @patch('accent_agid.helpers._is_webrtc', Mock(return_value=True))
    @patch('accent_agid.helpers._is_mobile_reachable', Mock(return_value=False))
    @patch('accent_agid.helpers._has_mobile_connection', Mock(return_value=True))
    def test_mobile_connection_webrtc_not_registered(self):
        aor_name = 'abcd'

        interface = build_sip_interface(self.agi, s.user_uuid, aor_name)

        self.assertEqual(interface, 'Local/abcd@accent_wait_for_registration')

    @patch('accent_agid.helpers._is_webrtc', Mock(return_value=True))
    @patch('accent_agid.helpers._is_mobile_reachable', Mock(return_value=True))
...
class TestHasMobileConnection(unittest.TestCase):
    ...
    def test_with_mobile_connections(self):
        ...
        assert_that(result, equal_to(True))
        self.agi.set_variable.called_once_with('ACCENT_MOBILE_CONNECTION', True)
