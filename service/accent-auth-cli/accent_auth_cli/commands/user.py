...
class UserAdd(UserIdentifierMixin, PolicyIdentifierMixin, GroupIdentifierMixin, Command):
    ...
    def get_parser(self, *args, **kwargs):
        ...
        relation.add_argument('--policy', help='The name or UUID of the policy to add to this user')
        relation.add_argument('--group', help='The name or UUID of the group to add to this user')
...
class UserUpdate(UserIdentifierMixin, Command):
    ...
    def get_parser(self, *args, **kwargs):
        parser = super().get_parser(*args, **kwargs)
        parser.add_argument('identifier', help="The username or UUID of the user to update")
        parser.add_argument('--enable', action='store_true', help="Enable the login")
        parser.add_argument('--disable', action='store_true', help="Disable the login")
        return parser
...
class UserRemove(UserIdentifierMixin, PolicyIdentifierMixin, GroupIdentifierMixin, Command):
    ...
    def get_parser(self, *args, **kwargs):
        parser = super().get_parser(*args, **kwargs)
        relation = parser.add_mutually_exclusive_group(required=True)
        relation.add_argument('--policy', help='The name or UUID of the policy to remove from this user')
        relation.add_argument('--group', help='The name or UUID of the group to remove from this user')
        relation.add_argument('--session', help='The UUID of the session to remove from this user')
        parser.add_argument('identifier', help='username or UUID')
        return parser
...
class UserSetPassword(UserIdentifierMixin, Command):
    ...
    from env variable ACCENT_AUTH_CLI_USER_PASSWORD."""

    password_env_variable = 'ACCENT_AUTH_CLI_USER_PASSWORD'
