...
class AccentAuthCLI(App):
    DEFAULT_VERBOSE_LEVEL = 0

    def __init__(self):
        super().__init__(
            description='A CLI for the accent-auth service',
            command_manager=CommandManager('accent_auth_cli.commands'),
            version='0.0.1',
        )
    ...
    def build_option_parser(self, *args, **kwargs):
        ...
        config_path_from_env = os.getenv('ACCENT_AUTH_CLI_CONFIG', None)
        config_path_default = os.path.expanduser(
            os.path.join('~', '.config', 'accent-auth-cli')
        )
        parser.add_argument(
            '--config',
        ...
        parser.add_argument('--hostname', help='The accent-auth hostname')
        parser.add_argument('--port', help='The accent-auth port')
        ...
        auth_or_token.add_argument('--token', help='The accent-auth token to use')
    ...
    def initialize_app(self, argv):
        self.LOG.debug('Accent Auth CLI')
