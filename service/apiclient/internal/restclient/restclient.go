...
const (
	...
	defaultBaseURL    = "https://api.accentvoice.io/"
	defaultUserAgent  = "accent_restclient" + "/" + Version
	uploadBaseURL     = "https://uploads.accentvoice.io/"

	headerAPIVersion    = "X-Accent-Api-Version"
	headerRateLimit     = "X-RateLimit-Limit"
	headerRateRemaining = "X-RateLimit-Remaining"
	headerRateReset     = "X-RateLimit-Reset"
	headerOTP           = "X-Accent-OTP"
	headerRetryAfter    = "Retry-After"
	headerTokenExpiration = "Accent-Authentication-Token-Expiration"
	headerTenant 			= "Accent-Tenant"

	mediaTypeV3                = "application/json"
	defaultMediaType           = "application/octet-stream"
	mediaTypeV3SHA             = "application/vnd.accent.v3.sha"
	mediaTypeV3Diff            = "application/vnd.accent.v3.diff"
	mediaTypeV3Patch           = "application/vnd.accent.v3.patch"
	mediaTypeOrgPermissionRepo = "application/vnd.accent.v3.repository+json"
	mediaTypeIssueImportAPI    = "application/vnd.accent.golden-comet-preview+json"

)

var errNonNilContext = errors.New("context must be non-nil")

// A Client manages communication with the Accent API.
type Client struct {
	clientMu sync.Mutex   // clientMu protects the client during calls that modify the CheckRedirect func.
	client   *http.Client // HTTP client used to communicate with the API.

	// Base URL for API requests. Defaults to the public Accent API, but can be
	// set to a domain endpoint to use with Accent Enterprise. BaseURL should
	...
	// User agent used when communicating with the Accent API.
	UserAgent string
	...
	// Services used for talking to different parts of the Accent API.
...
// Client returns the http.Client used by this Accent client.
// This should only be used for requests to the Accent API because
...
// NewClient returns a new Accent API client. If a nil httpClient is
...
//
// Another important thing is that by default, the Accent Enterprise URL format
...
// NewTokenClient returns a new Accent API client authenticated with the provided token.
...
// NewEnterpriseClient returns a new Accent API client with provided
// base URL and upload URL (often is your Accent Enterprise hostname).
//
...
// WithVersion overrides the Accent v3 API version for this individual request.
...
// Response is a Accent API response. This wraps the standard http.Response
// returned from Accent and provides convenient access to things like
// pagination links.
type Response struct {
	*http.Response
	...
	Filtered int // Number of items filtered by the API (if any).
	Total 	int // Total number of items available (if known).
...
func parseSecondaryRate(r *http.Response) *time.Duration {
	// According to Accent support, the "Retry-After" header value will be
	...
	// According to Accent support, endpoints might return x-ratelimit-reset instead,
...
// The token expiration, located in the response body, within the json data nested in the "data" key and has a key of utc_expires_at and is in UTC."

// func parseTokenExpiration(r *http.Response) Timestamp {
// 	var tokenBodyResponse TokenBodyResponse
// 	if v := r.Body; v != nil {
// 		body, err := io.ReadAll(v)
// 		if err != nil {
// 			return Timestamp{} // 0001-01-01 00:00:00
// 		}
// 		err = json.Unmarshal(body, &tokenBodyResponse)
// 		if err != nil {
// 			return Timestamp{} // 0001-01-01 00:00:00
// 		}
// 		expiresAt := tokenBodyResponse.BodyData.UTCExpiresAt
// 		// Converts the string to work with Timestamp, the time
// 		// is represented in a string 2024-03-29T21:09:22.196470 in UTC.
// 		// The time is in UTC, so we need to parse it as such.
// 		if t, err := time.Parse("2006-01-02T15:04:05.999999", *expiresAt); err == nil {
// 			return Timestamp{t}
// 		}
// 	}
// 	return Timestamp{} // 0001-01-01 00:00:00
...
// 	var tokenBodyResponse TokenBodyResponse
// 	if v := r.Body; v != nil {
// 		err := json.Unmarshal(v, &tokenBodyResponse)
// 		if err != nil {
...
// RateLimitError occurs when Accent returns 403 Forbidden response with a rate limit
...
// AcceptedError occurs when Accent returns 202 Accepted response with an
// empty body, which means a job was scheduled on the Accent side to process
...
func (*AcceptedError) Error() string {
	return "job scheduled on Server side; try again later"
}
...
// AbuseRateLimitError occurs when Accent returns 403 Forbidden response with the
// "documentation_url" field value equal to "https://XXXXXXXX".
...
These are the possible validation error codes:
	...
	custom:
	    some resources return this, additional
	    information is set in the Message field of the Error

Accent error responses structure are often undocumented and inconsistent.
...
func CheckResponse(r *http.Response) error {
	...
	// Re-populate error response body because Accent error responses are often
...
// parseBoolResponse determines the boolean result from a Accent API response.
// Several Accent API methods return boolean responses indicated by the HTTP
...
that need to use a higher rate limit associated with your OAuth application.
	...
	t := &accent.UnauthenticatedRateLimitedTransport{
		ClientID:     "your app's client ID",
		ClientSecret: "your app's client secret",
	}
	client := accent.NewClient(t.Client())
...
type UnauthenticatedRateLimitedTransport struct {
	// ClientID is the Accent OAuth client ID of the current application, which
	// can be found by selecting its entry in the list
	ClientID string

	// ClientSecret is the Accent OAuth client secret of the current
	// application.
	ClientSecret string
...
// their Accent account.
type BasicAuthTransport struct {
	Username string // Accent username
	Password string // Accent password
