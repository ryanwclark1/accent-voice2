"""add accent reverse lookup fields
...
def upgrade():
    dird_sources = get_accent_sources()
...
def get_accent_sources():
    accent_sources = sql.select(
        [
    ...
    ).where(dird_source_table.c.backend == 'accent')
    return op.get_bind().execute(accent_sources)
