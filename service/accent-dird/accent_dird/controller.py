...
class Controller:
    ...
    def __init__(self, config: Config):
        ...
        if not self.config['auth'].get('master_tenant_uuid'):
            self.token_renewer.subscribe_to_next_token_details_change(auth.init_master_tenant)
    ...
    def run(self):
        ...
        with self.token_renewer, self.bus, ServiceCatalogRegistration(*self._service_registration_params):
            try:
                self.rest_api.run()
            finally:
    ...
    def stop(self, reason):
        logger.warning('Stopping accent-dird: %s', reason)
