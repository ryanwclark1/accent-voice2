...
class Contact(Base):
    ...
    user_uuid = Column(String(UUID_LENGTH), ForeignKey('dird_user.user_uuid', ondelete='CASCADE'))

    phonebook_uuid = Column(UUID(as_uuid=False), ForeignKey('dird_phonebook.uuid', ondelete='CASCADE'))
...
class ContactFields(Base):
    ...
    __table_args__ = (schema.Index('dird_contact_fields__idx__contact_uuid', 'contact_uuid'),)
...
class Phonebook(Base):
    ...
    id = Column(Integer, Sequence("dird_phonebook_id_seq", start=1), nullable=False, unique=True)
    uuid = Column(UUID(as_uuid=False), server_default=text('uuid_generate_v4()'), primary_key=True)
...
class ProfileService(Base):
    ...
    profile_tenant_uuid = Column(String(UUID_LENGTH))
    service_uuid = Column(String(UUID_LENGTH), ForeignKey('dird_service.uuid', ondelete='CASCADE'))
    config = Column(JSON)
...
class Source(Base):
    ...
    phonebook = relationship(lambda: Phonebook, foreign_keys=[phonebook_uuid], lazy='joined')
