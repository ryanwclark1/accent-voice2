    ...
    def _associate_all_services(self, session, tenant_uuid, profile_uuid, services):
        for service_name, config in services.items():
            ...
            for source in config['sources']:
                ...
                self._create_profile_service_source(session, tenant_uuid, profile_service_uuid, source_uuid)

    def _dissociate_all_services(self, session, profile_uuid):
        session.query(ProfileService).filter(ProfileService.profile_uuid == profile_uuid).delete(
