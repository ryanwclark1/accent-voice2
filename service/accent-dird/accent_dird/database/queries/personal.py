...
from accent_dird.exception import DuplicatedContactException, NoSuchContact
...
class PersonalContactSearchEngine(BaseDAO):
    ...
    def _find_personal_contacts_with_filter(self, filter_, limit=None):
        ...
        with self.new_session() as s:
            base_query = s.query(distinct(ContactFields.contact_uuid)).join(Contact).join(User).filter(filter_)
            query = base_query.limit(limit) if limit else base_query
...
class PersonalContactCRUD(BaseDAO):
    def list_personal_contacts(self, user_uuid=None, search_params=None):
        ...
        with self.new_session() as s:
            ...
            return self._apply_search_params(list_contacts_by_uuid(s, contact_uuids), order, limit, offset, reverse)

    def create_personal_contact(self, user_uuid, contact_info):
        with self.new_session() as s:
            for contact in self._create_personal_contacts(s, user_uuid, [contact_info], fail_on_duplicate=True):
                return contact
    ...
    def _create_personal_contacts(self, session, user_uuid, contact_infos, fail_on_duplicate=False):
        ...
        existing_hashes_and_id = self._find_existing_contact_by_hash(session, user_uuid, list(hash_and_contact.keys()))
    ...
    def get_personal_contact(self, user_uuid, contact_uuid):
        with self.new_session() as s:
            filter_ = and_(User.user_uuid == user_uuid, ContactFields.contact_uuid == contact_uuid)
            contact_uuids = s.query(distinct(ContactFields.contact_uuid)).join(Contact).join(User).filter(filter_)
