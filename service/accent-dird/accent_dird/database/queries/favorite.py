...
from accent_dird.exception import DuplicatedFavoriteException, NoSuchFavorite
...
class FavoriteCRUD(BaseDAO):
    def create(self, user_uuid, backend, source_name, contact_id):
        with self.new_session() as s:
            ...
            favorite = Favorite(source_uuid=source.uuid, contact_id=contact_id, user_uuid=user.user_uuid)
            s.add(favorite)
    ...
    def get(self, user_uuid):
        with self.new_session() as s:
            favorites = s.query(Favorite.contact_id, Source.name).join(Source).filter(Favorite.user_uuid == user_uuid)
            return [(f.name, f.contact_id) for f in favorites.all()]

    def _get_source(self, session, backend, source_name):
        source = session.query(Source).filter(and_(Source.name == source_name, Source.backend == backend)).first()
