...
from accent_dird.http import AuthResource, LegacyAuthResource
...
class Lookup(LegacyAuthResource, DisplayAwareResource):
    ...
    def get(self, profile):
        ...
        raw_results = self.lookup_service.lookup(profile_config, tenant.uuid, term, user_uuid, token=token)
        favorites = self.favorite_service.favorite_ids(profile_config, user_uuid).by_name
...
class LookupByUUID(AuthResource, DisplayAwareResource):
    ...
    def get(self, profile, user_uuid):
        ...
        raw_results = self.lookup_service.lookup(profile_config, tenant_uuid, term, user_uuid, token=token)
        favorites = self.favorite_service.favorite_ids(profile_config, user_uuid).by_name
        formatter = _ResultFormatter(display)
        response = formatter.format_results(raw_results, favorites)
...
class Reverse(LegacyAuthResource):
    ...
    def get(self, profile, user_uuid):
        ...
        raw_result = self.reverse_service.reverse(profile_config, exten, profile, user_uuid=user_uuid, token=token)
...
class FavoritesRead(LegacyAuthResource, DisplayAwareResource):
    ...
    def get(self, profile):
        ...
        try:
            raw_results = self.favorites_service.favorites(profile_config, token_infos['metadata']['uuid'], token)
...
class FavoritesWrite(LegacyAuthResource):
    ...
    def put(self, directory, contact):
        ...
        tenant = Tenant.autodetect()
        try:
            self.favorites_service.new_favorite(tenant.uuid, directory, contact, token_infos['metadata']['uuid'])
    ...
    def delete(self, directory, contact):
        ...
        tenant = Tenant.autodetect()
        try:
            self.favorites_service.remove_favorite(tenant.uuid, directory, contact, token_infos['metadata']['uuid'])
            return '', 204
        except (
...
class Personal(LegacyAuthResource, DisplayAwareResource):
    ...
    def get(self, profile):
        ...
        raw_results = self.personal_service.list_contacts(tenant.uuid, token_infos['metadata']['uuid'])

        try:
            favorites = self.favorite_service.favorite_ids(profile_config, token_infos['metadata']['uuid']).by_name
...
class _ResultFormatter:
    def __init__(self, display):
        ...
        if self._has_favorites:
            self._favorite_field = next(d.field for d in display if d.type == 'favorite')
