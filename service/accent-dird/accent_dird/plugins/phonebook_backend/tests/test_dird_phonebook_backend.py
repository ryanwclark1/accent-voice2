...
class TestDirdPhonebook(unittest.TestCase):
    ...
    def test_load_with_no_phonebook_uuid(self):
        with patch(f'{PhonebookPlugin.__module__}.Session'), self.assertRaises(InvalidConfigError):
            self.source.load({'config': {'name': 'test-load', 'tenant_uuid': '123'}})

    def test_that_the_id_is_used_if_supplied(self):
        id_ = self.source._get_phonebook_key(s.tenant_uuid, {'phonebook_uuid': "some-uuid"})
