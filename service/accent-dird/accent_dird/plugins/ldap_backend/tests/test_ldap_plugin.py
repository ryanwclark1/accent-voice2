...
from accent_dird.plugins.base_plugins import BaseSourcePlugin
...
class TestLDAPConfig(unittest.TestCase):
    ...
    def test_attributes_with_unique_column_only_returns_none(self):
        ldap_config = self.new_ldap_config({BaseSourcePlugin.UNIQUE_COLUMN: 'entryUUID'})
    ...
    def test_build_search_filter_multiple_columns(self):
        ldap_config = _LDAPConfig({BaseSourcePlugin.SEARCHED_COLUMNS: ['givenName', 'sn']})

        term = 'foo'
        expected = f'(|(givenName=*{term}*)(sn=*{term}*))'
...
class TestLDAPResultFormatter(unittest.TestCase):
    ...
    def test_format(self):
        ...
        raw_results = [('dn', {'entryUUID': [b'0123'], 'givenName': [b'Gr\xc3\xa9goire']})]
        expected_results = [self.SourceResult({'entryUUID': '0123', 'givenName': 'Grégoire'})]
    ...
    def test_format_with_binary_uid(self):
        ...
        expected_results = [self.SourceResult({'entryUUID': encoded_uid, 'givenName': 'John'})]
    ...
    def test_format_with_referals(self):
        ...
        raw_results = [
            ...
            (
                None,
                ['ldap://b.example.com/cn=test,dc=lan-quebec,dc=accent,dc=com??sub'],
            ),
        ]
        expected_results = [self.SourceResult({'entryUUID': '0123', 'givenName': 'John'})]
