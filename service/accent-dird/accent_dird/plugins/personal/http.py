...
class PersonalAll(LegacyAuthResource):
    ...
    def get(self):
        user_uuid = _get_calling_user_uuid()
        try:
            contacts = self.personal_service.list_contacts_raw(user_uuid, search_params=request.args)
    ...
    def format_csv(contacts):
        ...
        fieldnames = sorted({attribute for contact in contacts for attribute in contact})
...
class PersonalOne(LegacyAuthResource):
    ...
    def put(self, contact_id):
        ...
        try:
            contact = self.personal_service.edit_contact(contact_id, new_contact, user_uuid)
            return contact, 200
        except self.personal_service.NoSuchContact as e:
            error = {'reason': [str(e)], 'timestamp': [time()], 'status_code': 404}
            return error, 404
