...
import yaml
from accent.consul_helpers import ServiceFinder
from accent_bus.resources.services.event import ServiceRegisteredEvent
...
class _Service:
    def __init__(self, config, bus, source_manager, controller):
        ...
        fetcher_thread = threading.Thread(target=self._add_remote_services, args=(finder, service_disco_config))
    ...
    def _on_service_added(self, service_name, host, port, uuid):
        ...
        config = self._source_config_generator.generate_from_new_service(service_name, uuid, host, port)
        if not config:
            return
        ...
        source_service.create('accent', **config)
    ...
    def _source_exists(self, source_service, config):
        ...
        return bool(source_service.list_(**search_params))
...
class SourceConfigGenerator:
    ...
    def __init__(self, service_discovery_config):
        ...
        self._template_files = {
            service: config['template'] for service, config in service_discovery_config['services'].items()
        }
        self.enabled = True
