...
from accent_dird.plugins.source_result import (
...
class TestSourceResult(unittest.TestCase):
    def setUp(self):
        self.accent_id = sentinel.accent_id
        self.fields = {
            'client_no': 1,
            'firstname': 'fn',
            'lastname': 'ln',
    ...
    def test_source(self):
        r = _SourceResult(self.fields, self.accent_id)
    ...
    def test_agent_relation(self):
        r = _SourceResult(self.fields, self.accent_id, agent_id=sentinel.agent_id)

        assert_that(
            r.relations,
            equal_to(
                {
                    'accent_id': sentinel.accent_id,
    ...
    def test_endpoint_relation(self):
        r = _SourceResult(self.fields, sentinel.accent_id, endpoint_id=sentinel.endpoint_id)
...
class TestMakeResultClass(unittest.TestCase):
    ...
    def test_source_unique_column(self):
        SourceResult = make_result_class(sentinel.backend, sentinel.source_name, 'the-unique-column')
    ...
    def test_format_columns(self):
        SourceResult = make_result_class(sentinel.backend, sentinel.source_name, format_columns={'to': '{from}'})
    ...
    def test_deletable(self):
        SourceResult = make_result_class(sentinel.backend, sentinel.source_name, is_deletable=True)
