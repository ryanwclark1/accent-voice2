...
from accent_dird import make_result_class

from ..plugin import AccentUserPlugin

TENANT_UUID = '02153e33-4b59-4a9f-8cd1-7e917b306e1d'
AUTH_CONFIG = {
    'host': 'accent.example.com',
    'backend': 'accent_user',
    'username': 'foo',
    'password': 'bar',
}
CONFD_CONFIG = {'host': 'accent.example.com', 'port': 9486, 'version': '1.1'}
DEFAULT_ARGS = {
    'config': {
...
class _BaseTest(unittest.TestCase):
    def setUp(self):
        self._source = AccentUserPlugin()
        self._confd_client = Mock()
        self._source._client = self._confd_client


class TestAccentUserBackendSearch(_BaseTest):
    def setUp(self):
        super().setUp()
    ...
    def test_search_on_excluded_column(self):
        ...
        self._confd_client.users.list.assert_called_once_with(recurse=True, view='directory', search='paul')
    ...
    def test_that_search_uses_extra_search_params(self):
        ...
        with patch('accent_dird.plugins.accent_user_backend.plugin.registry') as registry:
            self._source.load(DEFAULT_ARGS)

            self._source.search(term='paul')

            client = registry.get.return_value
            client.users.list.assert_called_once_with(recurse=True, view='directory', search='paul', context='inside')
    ...
    def test_search_with_no_accent(self):
        ...
        self._confd_client.users.list.assert_called_once_with(recurse=True, view='directory', search='accent')
    ...
    def test_search_with_wrong_accent(self):
        ...
        self._confd_client.users.list.assert_called_once_with(recurse=True, view='directory', search='accént')
    ...
    def test_first_match(self):
        ...
        self._confd_client.users.list.assert_called_once_with(recurse=True, view='directory', search='1234')
    ...
    def test_first_match_return_none_when_no_result(self):
        ...
        self._confd_client.users.list.assert_called_once_with(recurse=True, view='directory', search='12')
    ...
    def test_match_all(self):
        ...
        self._confd_client.users.list.assert_called_once_with(recurse=True, view='directory', exten='1234,5678')
    ...
    def test_match_all_when_no_result(self):
        ...
        self._confd_client.users.list.assert_called_once_with(recurse=True, view='directory', exten='12')
    ...
    def test_list_with_unknown_id(self):
        ...
        self._confd_client.users.list.assert_called_once_with(recurse=True, view='directory')
