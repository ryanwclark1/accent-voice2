...
class _Resource(LegacyAuthResource):
    ...
    def _find_tenant(self, scoping_tenant: Tenant, name: str):
        tenants = self._auth_client.tenants.list(tenant_uuid=scoping_tenant.uuid, name=name)['items']
...
class DeprecatedPhonebookAll(_Resource):
    ...
    def get(self, tenant):
        ...
        count = self.phonebook_service.count_phonebook([matching_tenant['uuid']], **parser.count_params())
        phonebooks = self.phonebook_service.list_phonebook([matching_tenant['uuid']], **parser.list_params())
    ...
    def post(self, tenant):
        ...
        matching_tenant = self._find_tenant(scoping_tenant, tenant)
        return (
            self.phonebook_service.create_phonebook(matching_tenant['uuid'], request.json),
            201,
        )
...
class DeprecatedPhonebookContactImport(_Resource):
    ...
    def post(self, tenant, phonebook_id):
        ...
        to_add = list(csv.DictReader(data))
...
class DeprecatedPhonebookContactOne(_Resource):
    ...
    def get(self, tenant, phonebook_id, contact_uuid):
        ...
        matching_tenant = self._find_tenant(scoping_tenant, tenant)
        return (
            self.phonebook_service.get_contact([matching_tenant['uuid']], PhonebookKey(id=phonebook_id), contact_uuid),
            200,
        )
    ...
    def delete(self, tenant, phonebook_id, contact_uuid):
        scoping_tenant = Tenant.autodetect()
        matching_tenant = self._find_tenant(scoping_tenant, tenant)
        self.phonebook_service.delete_contact([matching_tenant['uuid']], PhonebookKey(id=phonebook_id), contact_uuid)
        return '', 204
...
class DeprecatedPhonebookOne(_Resource):
    ...
    def delete(self, tenant, phonebook_id):
        scoping_tenant = Tenant.autodetect()
        matching_tenant = self._find_tenant(scoping_tenant, tenant)
        self.phonebook_service.delete_phonebook([matching_tenant['uuid']], PhonebookKey(id=phonebook_id))
        return '', 204
    ...
    def get(self, tenant, phonebook_id):
        ...
        matching_tenant = self._find_tenant(scoping_tenant, tenant)
        return (
            self.phonebook_service.get_phonebook([matching_tenant['uuid']], PhonebookKey(id=phonebook_id)),
            200,
        )
