...
from accent_dird import database
from accent_dird.database.queries.phonebook import PhonebookKey
from accent_dird.exception import InvalidContactException, InvalidPhonebookException
...
class TestPhonebookPhonebookAPI(_BasePhonebookServiceTest):
    ...
    def test_edit_phonebook(self):
        ...
        result = self.service.edit_phonebook([s.tenant_uuid], PhonebookKey(uuid=s.phonebook_uuid), body)

        self.phonebook_crud.edit.assert_called_once_with([s.tenant_uuid], PhonebookKey(uuid=s.phonebook_uuid), body)
    ...
    def test_delete_phonebook(self):
        self.service.delete_phonebook([s.tenant_uuid], PhonebookKey(uuid=s.phonebook_uuid))

        self.phonebook_crud.delete.assert_called_once_with([s.tenant_uuid], PhonebookKey(uuid=s.phonebook_uuid))

    def test_get_phonebook(self):
        result = self.service.get_phonebook([s.tenant_uuid], PhonebookKey(uuid=s.phonebook_uuid))

        assert_that(result, equal_to(self.phonebook_crud.get.return_value))
        self.phonebook_crud.get.assert_called_once_with([s.tenant_uuid], PhonebookKey(uuid=s.phonebook_uuid))


class TestPhonebookServiceContactAPI(_BasePhonebookServiceTest):
    def test_count_contact(self):
        result = self.service.count_contact([s.tenant_uuid], PhonebookKey(uuid=s.phonebook_uuid))

        assert_that(result, equal_to(self.contact_crud.count.return_value))
        self.contact_crud.count.assert_called_once_with([s.tenant_uuid], PhonebookKey(uuid=s.phonebook_uuid))

    def test_count_contact_with_a_search_param(self):
        result = self.service.count_contact([s.tenant_uuid], PhonebookKey(uuid=s.phonebook_uuid), search=s.search)
    ...
    def test_create_contact(self):
        ...
        result = self.service.create_contact(s.tenant_uuid, PhonebookKey(uuid=s.phonebook_uuid), body)

        assert_that(result, equal_to(self.contact_crud.create.return_value))
        self.contact_crud.create.assert_called_once_with(s.tenant_uuid, PhonebookKey(uuid=s.phonebook_uuid), body)
    ...
    def test_edit_contact(self):
        body = {'firstname': 'Foobar'}
        result = self.service.edit_contact([s.tenant_uuid], PhonebookKey(uuid=s.phonebook_uuid), s.contact_uuid, body)
    ...
    def test_delete_contact(self):
        result = self.service.delete_contact([s.tenant_uuid], PhonebookKey(uuid=s.phonebook_uuid), s.contact_uuid)
    ...
    def test_get_contact(self):
        result = self.service.get_contact([s.tenant_uuid], PhonebookKey(uuid=s.phonebook_uuid), s.contact_uuid)
...
class TestPhonebookServiceContactList(_BasePhonebookServiceTest):
    ...
    def test_that_list_returns_the_db_result_when_no_pagination_or_sorting(self):
        ...
        result = self.service.list_contacts([s.tenant_uuid], PhonebookKey(uuid=s.phonebook_uuid), search=s.search)
...
class TestPhonebookServiceContactImport(_BasePhonebookServiceTest):
    def test_import_with_invalid_contacts(self):
        ...
        created, errors = self.service.import_contacts(s.tenant_uuid, PhonebookKey(uuid=s.phonebook_uuid), contacts)
