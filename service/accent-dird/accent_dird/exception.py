...
class accentConfdError(APIException):
    def __init__(self, confd_client, error):
        super().__init__(
            status_code=503,
            message='accent-confd request error',
            error_id='accent-confd-error',
            details={
                'accent_confd_config': {
...
class MasterTenantNotInitiatedException(APIException):
    def __init__(self):
        error_message = 'accent-dird master tenant is not initiated'
