...
class TestPost(BaseLDAPCRUDTestCase):
    ...
    def test_multi_tenant(self):
        ...
        assert_that(
            calling(sub_tenant_client.ldap_source.create).with_args(self.valid_body, tenant_uuid=MAIN_TENANT),
...
class TestPut(BaseLDAPCRUDTestCase):
    def setUp(self):
        ...
        self.new_body = {
            'name': 'new',
            'ldap_uri': 'ldap://accent.io',
            'ldap_base_dn': 'ou=other,dc=accent,dc=io',
...
class TestGet(BaseLDAPCRUDTestCase):
    ...
    def test_get(self, accent):
        response = self.client.ldap_source.get(accent['uuid'])
