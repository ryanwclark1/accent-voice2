...
from accent_test_helpers.auth import AuthClient as AuthMock
    ...
    "value": [
        {
            ...
            "emailAddresses": [
                {"address": "wbros@accentquebec.onmicrosoft.com"},
                {},
                {},
            ],
        }
...
class BaseOffice365TestCase(DirdAssetRunningTestCase):
    ...
    LOOKUP_ARGS = {'user_uuid': 'a-accent-uuid', 'token': 'a-token'}
    FAVORITE_ARGS = {'user_uuid': 'a-accent-uuid', 'token': 'a-token'}
    ...
    def setUp(self):
        ...
        self.source = self.client.backends.create_source(backend=self.BACKEND, body=self.config())
        self.display = self.client.displays.create({'name': 'display', 'columns': [{'field': 'firstname'}]})
...
class BaseOffice365PluginTestCase(BaseOffice365TestCase):
    ...
    def setUp(self):
        ...
        self.backend = BackendWrapper('office365', {'config': self.config(), 'api': Mock()})
    ...
    def test_plugin_lookup(self, office365_api):
        ...
        assert_that(
            ...
            contains(
                has_entries(
                    number='5555555555',
                    email='wbros@accentquebec.onmicrosoft.com',
                    **self.WARIO,
                )
            ),
        )
...
class TestDirdOffice365Plugin(BaseOffice365TestCase):
    ...
    def test_given_microsoft_source_when_get_all_contacts_then_contacts_fetched(self, office365_api):
        ...
        result = self.client.backends.list_contacts_from_source(backend=self.BACKEND, source_uuid=self.source['uuid'])
    ...
    @fixtures.office365_result(OFFICE365_CONTACTS)
    def test_given_non_existing_microsoft_source_when_get_all_contacts_then_not_found(self, office365_api):
        self.auth_client_mock.set_external_auth(self.MICROSOFT_EXTERNAL_AUTH)
        ...
        assert_that(
            ...
            raises(requests.HTTPError).matching(has_property('response', has_properties('status_code', 404))),
        )

    @fixtures.office365_result(OFFICE365_CONTACTS)
    def test_given_source_and_non_existing_tenant_when_get_all_contacts_then_not_found(self, office365_api):
        self.auth_client_mock.set_external_auth(self.MICROSOFT_EXTERNAL_AUTH)
...
class TestDirdOffice365PluginErrorEndpoint(BaseOffice365TestCase):
    ...
    def test_given_microsoft_when_fetch_all_contacts_with_error_endpoint(self, office365_api):
        assert_that(
            ...
            raises(requests.HTTPError).matching(has_property('response', has_properties('status_code', 503))),
        )
