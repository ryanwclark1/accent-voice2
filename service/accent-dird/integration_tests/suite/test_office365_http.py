...
class TestDelete(BaseOffice365CRUDTestCase):
    ...
    def test_delete_multi_tenant(self, sub, main):
        ...
        assert_that(
            calling(sub_client.backends.delete_source).with_args(BACKEND, main['uuid'], tenant_uuid=MAIN_TENANT),
            raises(Exception).matching(HTTP_401),
        )

        assert_that(
            calling(main_client.backends.delete_source).with_args(BACKEND, main['uuid'], tenant_uuid=SUB_TENANT),
            raises(Exception).matching(HTTP_404),
        )
...
class TestGet(BaseOffice365CRUDTestCase):
    ...
    def test_get_multi_tenant(self, sub, main):
        ...
        assert_that(
            calling(main_client.backends.get_source).with_args(BACKEND, main['uuid'], tenant_uuid=SUB_TENANT),
            raises(Exception).matching(HTTP_404),
        )
        ...
        assert_that(
            calling(sub_client.backends.get_source).with_args(BACKEND, main['uuid'], tenant_uuid=MAIN_TENANT),
            raises(Exception).matching(HTTP_401),
        )
...
class TestList(BaseOffice365CRUDTestCase):
    ...
    def test_multi_tenant(self, c, b, a):
        ...
        assert_that(
            calling(sub_client.backends.list_sources).with_args(BACKEND, tenant_uuid=MAIN_TENANT),
            raises(Exception).matching(HTTP_401),
        )
...
class TestPost(BaseOffice365CRUDTestCase):
    def test_post(self):
        ...
        with self.source(self.client, self.valid_body):
            assert_that(
                calling(self.client.backends.create_source).with_args(BACKEND, self.valid_body),
    ...
    def test_multi_tenant(self):
        ...
        with self.source(main_tenant_client, self.valid_body):
            assert_that(
                calling(sub_tenant_client.backends.create_source).with_args(BACKEND, self.valid_body),
                not_(raises(Exception)),
            )
...
class TestPut(BaseOffice365CRUDTestCase):
    ...
    def test_put(self, foobar, other):
        assert_that(
            calling(self.client.backends.edit_source).with_args(BACKEND, foobar['uuid'], other),
            raises(Exception).matching(HTTP_409),
        )

        assert_that(
            calling(self.client.backends.edit_source).with_args(BACKEND, UNKNOWN_UUID, self.new_body),
            raises(Exception).matching(HTTP_404),
        )
        ...
        assert_that(
            calling(self.client.backends.edit_source).with_args(BACKEND, foobar['uuid'], self.new_body),
            not_(raises(Exception)),
        )
    ...
    def test_put_multi_tenant(self, sub, main):
        ...
        assert_that(
            calling(sub_client.backends.edit_source).with_args(BACKEND, main['uuid'], self.new_body),
            raises(Exception).matching(HTTP_404),
        )
        ...
        assert_that(
            calling(main_client.backends.edit_source).with_args(BACKEND, sub['uuid'], self.new_body),
            not_(raises(Exception)),
        )
