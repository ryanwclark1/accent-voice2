...
from accent_test_helpers.auth import AuthClient as MockAuthClient
from accent_test_helpers.auth import MockUserToken
...
class TestAddPersonal(PersonalOnlyTestCase):
    def test_that_a_token_with_no_user_uuid_returns_401(self):
        ...
        result = self.post_personal_result({'firstname': 'Alice'}, token=invalid_token_uuid)
...
class TestRemovePersonal(PersonalOnlyTestCase):
    ...
    def test_that_removed_personal_are_not_listed(self):
        ...
        assert_that(
            result['items'],
            contains_inanyorder(has_entry('firstname', 'Alice'), has_entry('firstname', 'Charlie')),
        )
...
class TestPersonalVisibility(PersonalOnlyTestCase):
    ...
    def setUpClass(cls):
        ...
        tenant_uuid = MAIN_TENANT
        valid_token_1 = MockUserToken.some_token(token=VALID_TOKEN_1, metadata={'tenant_uuid': tenant_uuid})
        valid_token_2 = MockUserToken.some_token(token=VALID_TOKEN_2, metadata={'tenant_uuid': tenant_uuid})
    ...
    def test_that_personal_are_only_visible_for_the_same_token(self):
        ...
        assert_that(
            result_1['items'],
            contains_inanyorder(has_entry('firstname', 'Alice'), has_entry('firstname', 'Bob')),
        )
...
class TestLookupPersonal(PersonalOnlyTestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            auth_client.users.new(uuid=VALID_UUID_1, tenant_uuid=MAIN_TENANT, username='foobar2')
        except Exception:
            pass
    ...
    def test_that_lookup_includes_personal_contacts(self):
        ...
        assert_that(
            result['results'],
            contains_inanyorder(has_entry('column_values', contains('Alice', None, None, False))),
        )
    ...
    def test_that_lookup_accepts_non_ascii_in_term(self):
        ...
        assert_that(
            result['results'],
            contains_inanyorder(has_entry('column_values', contains('Céline', None, None, False))),
        )
    ...
    def test_that_lookup_matches_query_non_ascii_with_result_ascii(self):
        ...
        assert_that(
            result['results'],
            contains_inanyorder(has_entry('column_values', contains('Etienne', None, None, False))),
        )
    ...
    def test_that_lookup_does_not_return_duplicates_when_matching_multiple_fields(self):
        ...
        assert_that(
            result['results'],
            contains_inanyorder(has_entry('column_values', contains('john', 'john', None, False))),
        )
    ...
    def test_that_lookup_returns_None_when_a_column_is_empty(self):
        ...
        assert_that(
            result['results'],
            contains_inanyorder(has_entry('column_values', contains('empty-column', None, None, False))),
        )
    ...
    def test_lookup_user_only_uuid_visible(self):
        ...
        assert_that(
            result['results'],
            contains_inanyorder(has_entry('column_values', contains('William', 'Gates', '222222', False))),
        )
        result = self.lookup_user('Will', 'default', VALID_UUID)
        assert_that(
            result['results'],
            not_(contains_inanyorder(has_entry('column_values', contains('William', 'Gates', '222222', False)))),
        )
...
class TestEditPersonal(PersonalOnlyTestCase):
    def test_that_a_token_with_no_user_uuid_returns_401(self):
        ...
        result = self.put_personal_result('unknown-id', {'firstname': 'new'}, token=invalid_token_uuid)
        assert_that(result, has_properties(status_code=401))
    ...
    def test_that_edit_personal_contact_replaces_attributes(self):
        ...
        assert_that(put_result, has_entries({'firstname': 'Nicolas', 'company': 'acme'}))
...
class TestGetPersonal(PersonalOnlyTestCase):
    ...
    def test_that_get_returns_all_attributes(self):
        ...
        assert_that(result, has_entries({'firstname': 'Noémie', 'lastname': 'Narvidon'}))
...
class TestSearchPersonal(PersonalOnlyTestCase):
    ...
    def test_that_search_ordered_returns_right_order(self):
        ...
        response = self.list_personal_result(VALID_TOKEN_MAIN_TENANT, order=wrong_column)
        ...
        response = self.list_personal_result(VALID_TOKEN_MAIN_TENANT, direction=wrong_direction)
        assert_that(response.status_code, equal_to(400))
        assert_that(response.json(), has_entry('reason', 'direction must be asc or desc'))
