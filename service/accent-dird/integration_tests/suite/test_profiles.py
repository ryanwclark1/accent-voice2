...
class TestList(BaseProfileTestCase):
    ...
    def test_search(self, source, display):
        ...
        with (
            self.profile(self.client, body_abc) as abc,
    ...
    def test_multi_tenant(self, sub_source, main_source, sub_display, main_display):
        ...
        with (
            self.profile(main_tenant_client, body_main_profile) as main,
...
class TestPut(BaseProfileTestCase):
            ...
            assert_that(
                ...
                has_entries(
                    ...
                    services=has_entries(
                        ...
                        favorites=has_entries(sources=contains(has_entries(uuid=s2['uuid']))),
                    ),
                ),
            )
    ...
    def test_duplicate(self, s1, d1):
        ...
        with self.profile(self.client, dict(name='a', **body)), self.profile(self.client, dict(name='b', **body)) as b:
            assert_that(
        ...
        with self.profile(sub_tenant_client, body) as profile:
            ...
            assert_that(
                ...
                has_entries(
                    ...
                    name='profile',
                    services=has_entries(lookup=has_entries(sources=contains(has_entries(uuid=sub_source['uuid'])))),
                ),
            )
        ...
        with self.profile(self.client, body) as profile:
            for display_uuid in [sub_display['uuid'], UNKNOWN_UUID]:
                ...
                assert_that(
                    ...
                    raises(Exception).matching(has_properties(response=has_properties(status_code=400))),
                )
...
class TestGetSourcesFromProfile(BaseProfileTestCase):
    ...
    def test_when_get_then_sources_returned(self):
        ...
        assert_that(
            response['items'],
            contains_inanyorder(
                has_entries(name='a_accent_main', backend='accent'),
    ...
    def test_given_asc_direction_when_get_then_sources_returned(self):
        ...
        assert_that(
            response['items'],
            contains(
                has_entries(name='a_accent_main'),
    ...
    def test_given_random_direction_when_get_then_bad_request(self):
        ...
        assert_that(
            ...
            raises(HTTPError).matching(has_properties(response=has_properties(status_code=400))),
        )
    ...
    def test_given_wrong_tenant_when_get_then_not_found(self):
        assert_that(
            ...
            raises(HTTPError).matching(has_properties(response=has_properties(status_code=404))),
        )
    ...
    def test_given_sub_tenant_when_get_main_tenant_then_unauthorized(self):
        ...
        assert_that(
            ...
            raises(HTTPError).matching(has_properties(response=has_properties(status_code=401))),
        )
    ...
    def test_given_limit_when_get_then_sources_returned(self):
        ...
        assert_that(response['items'], contains(has_entries(name='a_accent_main', backend='accent')))
