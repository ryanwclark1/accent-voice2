...
from accent_dird.database.queries.phonebook import (
    PhonebookContactCRUD,
    PhonebookCRUD,
    PhonebookDict,
    PhonebookKey,
)
from accent_dird.exception import NoSuchPhonebook
...
@contextmanager
def phonebook(dao: PhonebookCRUD, tenant_uuid: str, body: dict) -> Iterator[PhonebookDict]:
    try:
...
class BasePhonebookCRUDTestCase(BasePhonebookTestCase):
    ...
    def _get_phonebook(self, tenant_uuid=MAIN_TENANT, name=None):
        if phonebooks := self.phonebook_dao.list(tenant_uuid, search=(name or self.valid_body['name'])):
            phonebook, *_ = phonebooks
            assert phonebook['uuid'] in {phonebook['uuid'] for phonebook in self.phonebooks}
            return phonebook
        else:
...
class TestDelete(BasePhonebookCRUDTestCase):
    ...
    @fixtures.phonebook_source(name='foomain', tenant_uuid=MAIN_TENANT, token=VALID_TOKEN_MAIN_TENANT)
    @fixtures.phonebook_source(name='foosub', tenant_uuid=SUB_TENANT, token=VALID_TOKEN_SUB_TENANT)
...
class TestList(BasePhonebookCRUDTestCase):
    ...
    @fixtures.phonebook_source(name='abc', tenant_uuid=MAIN_TENANT, token=VALID_TOKEN_MAIN_TENANT)
    @fixtures.phonebook_source(name='bcd', tenant_uuid=MAIN_TENANT, token=VALID_TOKEN_MAIN_TENANT)
    @fixtures.phonebook_source(name='cde', tenant_uuid=SUB_TENANT, token=VALID_TOKEN_SUB_TENANT)
...
class TestPost(BasePhonebookCRUDTestCase):
    def test_post(self):
        try:
            ...
            assert_that(e.response.json(), has_entries(message=ANY, error_id='invalid-data'))
        else:
            self.fail('Should have raised')

        try:
            self.client.phonebook_source.create({'name': 'phonebook-source-without-phonebook'})
        except requests.exceptions.HTTPError as e:
            assert_that(e.response.status_code, equal_to(400))
            assert_that(e.response.json(), has_entries(message=ANY, error_id='invalid-data'))
        else:
    ...
    def test_multi_tenant(self):
        ...
        with phonebook(self.phonebook_dao, SUB_TENANT, {'name': 'sub'}) as phonebook_sub:
            ...
            assert_that(
                calling(sub_tenant_client.phonebook_source.create).with_args(self.valid_body, tenant_uuid=MAIN_TENANT),
...
class TestPut(BasePhonebookCRUDTestCase):
    ...
    def test_put_multi_tenant(self, sub, main):
        ...
        assert_that(
            calling(sub_tenant_client.phonebook_source.edit).with_args(sub['uuid'], dict(sub, name=main['name'])),
...
class TestGet(BasePhonebookCRUDTestCase):
    ...
    def test_get(self, accent):
        response = self.client.phonebook_source.get(accent['uuid'])
...
class TestGetContacts(BasePhonebookCRUDTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.stack.callback(cls.phonebook_crud.delete, None, PhonebookKey(uuid=cls.phonebook['uuid']))
        cls.num_contacts = 6
...
class TestPluginLookup(BasePhonebookCRUDTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.stack.callback(phonebook_crud.delete, None, PhonebookKey(uuid=phonebook['uuid']))
    ...
    def test_plugin_lookup(self):
        ...
        max_time = 2
        assert timing['elapsed'] < max_time, f'Lookup took too long {timing["elapsed"]} max {max_time}'
    ...
    def test_plugin_favorites(self):
        ...
        assert_that(
            result,
            has_entries(results=contains_exactly(has_entries(column_values=fave['column_values']))),
        )
...
class TestGetContactsLoad(BasePhonebookCRUDTestCase):
    ...
    def test_get_paginated(self):
        ...
        with phonebook_source(
            ...
            for i in range(self.num_contacts // limit):
                ...
                max_time = 2
                assert timing['elapsed'] < max_time, f'Get paginated took too long: {timing["elapsed"]} max {max_time}'
    ...
    def test_get_filtered(self):
        ...
        with phonebook_source(
            ...
            max_time = 2
            assert timing['elapsed'] < max_time, f'Get filtered took too long: {timing["elapsed"]} max {max_time}'
