...
from .helpers.config import new_accent_users_config, new_csv_with_multiple_displays_config
...
class TestGraphQL(BaseDirdIntegrationTest):
    ...
    def _configure_auth(cls):
        ...
        main_tenant_token = MockUserToken.some_token(metadata={'uuid': 'my-user-uuid', 'tenant_uuid': MAIN_TENANT})
        cls.auth.set_token(main_tenant_token)
        cls.main_tenant_token = main_tenant_token.token_id
        cls.dird = DirdClient('127.0.0.1', cls.service_port(9489, 'dird'), prefix=None, https=False)
                    ...
                    has_entries(
                        {
                            ...
                            'accentReverse': 'Alice AAA',
                            'accentSourceName': any_of('my_csv', 'my_csv_2'),
                            'accentSourceEntryId': '1',
                            'accentBackend': 'csv',
                        }
                    ),
                ),
    ...
    def test_reverse_missing_contact_fields(self):
        ...
        assert_that(
            ...
            contains(
                has_entry(
                    'node',
                    has_entries({'firstname': 'Dave', 'lastname': 'DDD', 'email': None}),
                ),
            ),
        )
    ...
    def test_unreachable_auth_should_return_error(self):
        with self.auth_stopped():
            ...
            assert_that(
                response['errors'],
                contains(has_entries({'path': ['hello'], 'message': contains_string('unreachable')})),
            )


class TestGraphQLAccentBackend(BaseDirdIntegrationTest):
    ...
    def test_accent_backend_fields(self):
        query = {
            'query': '''
            {
                me {
