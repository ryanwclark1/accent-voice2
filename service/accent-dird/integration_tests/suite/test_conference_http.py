...
class TestDelete(BaseConferenceCRUDTestCase):
    ...
    def test_delete_multi_tenant(self, sub, main):
        ...
        assert_that(
            calling(sub_client.conference_source.delete).with_args(main['uuid'], tenant_uuid=MAIN_TENANT),
            raises(Exception).matching(HTTP_401),
        )

        assert_that(
            calling(main_client.conference_source.delete).with_args(main['uuid'], tenant_uuid=SUB_TENANT),
            raises(Exception).matching(HTTP_404),
        )
...
class TestGet(BaseConferenceCRUDTestCase):
    ...
    def test_get_multi_tenant(self, sub, main):
        ...
        assert_that(
            calling(main_client.conference_source.get).with_args(main['uuid'], tenant_uuid=SUB_TENANT),
            raises(Exception).matching(HTTP_404),
        )
        ...
        assert_that(
            calling(sub_client.conference_source.get).with_args(main['uuid'], tenant_uuid=MAIN_TENANT),
            raises(Exception).matching(HTTP_401),
        )
...
class TestPost(BaseConferenceCRUDTestCase):
    ...
    def test_multi_tenant(self):
        ...
        assert_that(
            calling(sub_tenant_client.conference_source.create).with_args(self.valid_body, tenant_uuid=MAIN_TENANT),
...
class TestPut(BaseConferenceCRUDTestCase):
    ...
    def test_put_multi_tenant(self, sub, main):
        ...
        assert_that(
            calling(sub_client.conference_source.edit).with_args(main['uuid'], self.new_body, tenant_uuid=MAIN_TENANT),
            raises(Exception).matching(HTTP_401),
        )

        assert_that(
            calling(main_client.conference_source.edit).with_args(main['uuid'], self.new_body, tenant_uuid=SUB_TENANT),
            raises(Exception).matching(HTTP_404),
        )
