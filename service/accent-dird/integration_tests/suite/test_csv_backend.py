...
class TestCSVBackend(CSVWithMultipleDisplayTestCase):
    ...
    def test_that_csv_file_changes_have_been_committed(self):
        ...
        file_to_update = os.path.join(self.assets_root, 'tmp', 'data', 'asset.all_routes.test.csv')
...
class TestCSVNoUnique(_BaseCSVFileTestCase):
    ...
    source_config = 'etc/accent-dird/sources.d/my_test_csv.yml'
...
class TestCSVSeparator(BaseDirdIntegrationTest):
    ...
    def test_lookup_with_pipe(self):
        ...
        assert_that(
            result['results'],
            contains_inanyorder(has_entries(column_values=contains('Alice', 'AAA', '5555555555'))),
        )
