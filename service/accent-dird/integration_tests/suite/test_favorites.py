...
class _BaseMultiTokenFavoriteTest(BaseDirdIntegrationTest):
    ...
    def setUpClass(cls):
        ...
        user_token_1 = MockUserToken.some_token(metadata={'tenant_uuid': tenants['items'][0]['uuid']})
        user_token_2 = MockUserToken.some_token(metadata={'tenant_uuid': tenants['items'][1]['uuid']})
        user_token_3 = MockUserToken.some_token(metadata={'tenant_uuid': tenants['items'][0]['uuid']})
...
class TestFavorites(_BaseMultiTokenFavoriteTest):
    def test_that_removed_favorites_are_not_listed(self):
        with (
            self.favorite('my_csv', '1', token=self.token_1),
    ...
    def test_that_favorites_are_saved_across_dird_restart(self):
        with self.favorite('my_csv', '1', token=self.token_1):
            ...
            assert_that(
                result['results'],
                contains_inanyorder(has_entry('column_values', contains('Alice', 'AAA', '5555555555', True))),
            )
    ...
    def test_that_favorites_lookup_results_show_favorites(self):
        ...
        assert_that(
            result['results'],
            contains_inanyorder(has_entry('column_values', contains('Alice', 'AAA', '5555555555', False))),
        )
...
class TestRemovingFavoriteAlreadyInexistant(CSVWithMultipleDisplayTestCase):
    def test_that_removing_an_inexisting_favorite_returns_404(self):
        result = self.delete_favorite_result('unknown_source', 'unknown_contact', token=VALID_TOKEN_MAIN_TENANT)
...
class TestFavoritesInPersonalResults(PersonalOnlyTestCase):
    ...
    def test_that_favorites_list_results_accept_personal(self):
        with self.personal({'firstname': 'Alice'}) as alice, self.favorite('personal', alice['id']):
    ...
    def test_that_adding_favorite_produces_bus_event(self):
        ...
        def favorite_bus_event_received(accumulator):
            assert_that(
                ...
                has_item(
                    has_entries(
                        message=has_entry('name', any_of('favorite_added', 'favorite_deleted')),
