...
from accent_dird.database.queries.base import ContactInfo as _ContactInfo
...
class TestPhonebookBackend(unittest.TestCase):
    def setUp(self):
        ...
        assert all({'number', 'firstname', 'lastname'} & fields.keys() for fields in contacts)
        (
            self.dumbledore,
            self.hermione,
            self.hagrid,
    ...
    def tearDown(self):
        self.phonebook_crud.delete([self.tenant_uuid], database.PhonebookKey(uuid=self.phonebook['uuid']))
