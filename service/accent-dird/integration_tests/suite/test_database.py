...
from accent_dird import database, exception
from accent_dird.database.queries import base
...
def with_user_uuid(f):
    ...
    def wrapped(self, *args, **kwargs):
        ...
        with closing(Session()) as session:
            ...
            session.query(database.User).filter(database.User.user_uuid == user_uuid).delete()
            session.commit()
        return result
...
class _BasePhonebookCRUDTest(_BaseTest):
    ...
    def _new_phonebook(self, tenant_uuid, name, description=None, delete=True):
        ...
        yield phonebook
        if delete:
            self._crud.delete([tenant_uuid], database.PhonebookKey(uuid=phonebook['uuid']))
...
class TestPhonebookCRUDCount(_BasePhonebookCRUDTest):
    def test_count(self):
        tenant_uuid = new_uuid()
        with (
            self._new_phonebook(tenant_uuid, 'a'),
    ...
    def test_that_only_matching_phonebooks_are_counted(self):
        tenant_uuid = new_uuid()
        with (
            self._new_phonebook(tenant_uuid, 'ab'),
            self._new_phonebook(tenant_uuid, 'bc'),
...
class TestPhonebookCRUDCreate(_BasePhonebookCRUDTest):
    ...
    def test_that_create_without_name_fails(self):
        ...
        assert_that(calling(self._crud.create).with_args(tenant_uuid, None), raises(Exception))
    ...
    def test_that_create_with_invalid_fields_raises(self):
        ...
        assert_that(calling(self._crud.create).with_args(tenant_uuid, body), raises(TypeError))
    ...
    def test_that_duplicate_tenants_are_not_created(self):
        ...
        with closing(Session()) as session:
            tenant_count = (
                session.query(func.count(database.Tenant.uuid)).filter(database.Tenant.uuid == tenant_uuid).scalar()
            )
...
class TestPhonebookCRUDDelete(_BasePhonebookCRUDTest):
    ...
    def test_that_deleting_an_unknown_phonebook_raises(self):
        ...
        assert_that(
            calling(self._crud.delete).with_args([wrong_tenant_uuid], database.PhonebookKey(uuid=new_uuid())),
    ...
    def test_that_deleting_another_tenant_phonebook_is_not_possible(self):
        ...
        with self._new_phonebook(tenant_uuid_1, 'main') as phonebook:
            assert_that(
                calling(self._crud.delete).with_args([tenant_uuid_2], database.PhonebookKey(uuid=phonebook['uuid'])),
                raises(exception.NoSuchPhonebook),
            )
    ...
    def test_that_tenants_are_not_created_on_delete(self):
        ...
        tenant_uuid_2 = new_uuid()
        with closing(Session()) as session:
            total_tenant_before = session.query(func.count(database.Tenant.uuid)).scalar()

        with self._new_phonebook(tenant_uuid_1, 'a') as phonebook:
            try:
                self._crud.delete([tenant_uuid_2], database.PhonebookKey(uuid=phonebook['uuid']))
            except exception.NoSuchPhonebook:
                pass  # as expected

        with closing(Session()) as session:
            total_tenant_after = session.query(func.count(database.Tenant.uuid)).scalar()
...
class TestPhonebookCRUDEdit(_BasePhonebookCRUDTest):
    def test_that_edit_changes_the_phonebook(self):
        ...
        with self._new_phonebook(tenant_uuid, 'name') as phonebook:
            ...
            result = self._crud.edit([tenant_uuid], database.PhonebookKey(uuid=phonebook['uuid']), new_body)

        assert_that(result, has_entries(id=phonebook['id'], uuid=phonebook['uuid'], **new_body))
    ...
    def test_that_editing_an_unknown_phonebook_raises(self):
        ...
        assert_that(
            calling(self._crud.edit).with_args([tenant_uuid], database.PhonebookKey(uuid=new_uuid()), {'name': 'test'}),
            raises(exception.NoSuchPhonebook),
        )
    ...
    def test_that_editing_a_phonebook_from_another_tenant_raises(self):
        tenant_uuid_1 = new_uuid()
        tenant_uuid_2 = new_uuid()
        with self._new_phonebook(tenant_uuid_1, 'a') as phonebook_a, self._new_phonebook(tenant_uuid_2, 'b'):
            assert_that(
                calling(self._crud.edit).with_args(
                    [tenant_uuid_2],
...
class TestPhonebookCRUDGet(_BasePhonebookCRUDTest):
    def test_that_get_returns_the_phonebook(self):
        ...
        with self._new_phonebook(tenant_uuid, 'a') as phonebook:
            result = self._crud.get([tenant_uuid], database.PhonebookKey(uuid=phonebook['uuid']))
    ...
    def test_that_get_with_an_unknown_id_raises(self):
        tenant_uuid = new_uuid()
        assert_that(
            calling(self._crud.get).with_args([tenant_uuid], database.PhonebookKey(id=42)),
            raises(exception.NoSuchPhonebook),
        )
    ...
    def test_that_get_with_an_unknown_uuid_raises(self):
        tenant_uuid = new_uuid()
        assert_that(
            calling(self._crud.get).with_args([tenant_uuid], database.PhonebookKey(uuid=new_uuid())),
            raises(exception.NoSuchPhonebook),
        )
    ...
    def test_that_get_from_another_tenant_raises(self):
        ...
        with self._new_phonebook(tenant_uuid_1, 'a') as phonebook:
            assert_that(
                calling(self._crud.get).with_args([tenant_uuid_2], database.PhonebookKey(uuid=phonebook['uuid'])),
                raises(exception.NoSuchPhonebook),
            )
...
class TestPhonebookCRUDList(_BasePhonebookCRUDTest):
    def test_that_all_phonebooks_are_listed(self):
        tenant_uuid = new_uuid()
        with (
            self._new_phonebook(tenant_uuid, 'a') as a,
            self._new_phonebook(tenant_uuid, 'b') as b,
    ...
    def test_that_phonebooks_can_be_ordered(self):
        tenant_uuid = new_uuid()
        with (
            self._new_phonebook(tenant_uuid, 'a', description='z') as a,
            self._new_phonebook(tenant_uuid, 'b', description='b') as b,
            self._new_phonebook(tenant_uuid, 'c') as c,
        ):
    ...
    def test_that_phonebooks_order_with_invalid_field_raises(self):
        tenant_uuid = new_uuid()
        with (
            self._new_phonebook(tenant_uuid, 'a', description='z'),
            self._new_phonebook(tenant_uuid, 'b', description='b'),
            self._new_phonebook(tenant_uuid, 'c'),
        ):
            assert_that(
    ...
    def test_that_phonebooks_can_be_ordered_in_any_order(self):
        ...
        with (
            ...
            result = self._crud.list([tenant_uuid], order='description', direction='desc')
        assert_that(result, contains_inanyorder(a, b, c))
    ...
    def test_that_list_only_returns_matching_phonebooks(self):
        tenant_uuid = new_uuid()
        with (
            self._new_phonebook(tenant_uuid, 'aa', description='foobar') as a,
            self._new_phonebook(tenant_uuid, 'bb') as b,
            self._new_phonebook(tenant_uuid, 'cc'),
        ):
...
class _BasePhonebookContactCRUDTest(_BaseTest):
    ...
    def tearDown(self):
        self._phonebook_crud.delete([self._tenant_uuid], database.PhonebookKey(uuid=self._phonebook_uuid))
        super().tearDown()
...
class TestPhonebookContactImport(_BasePhonebookContactCRUDTest):
    def test_that_I_can_create_many_contacts(self):
        ...
        _, errors = self._crud.create_many([self._tenant_uuid], database.PhonebookKey(id=self._phonebook_id), body)
...
class TestPhonebookContactCRUDEdit(_BasePhonebookContactCRUDTest):
    ...
    def test_that_the_phonebook_must_match_the_id(self):
        other_phonebook = self._phonebook_crud.create(self._tenant_uuid, {'name': 'the other'})
        contact = self._crud.create(
            [self._tenant_uuid],
            database.PhonebookKey(uuid=self._phonebook_uuid),
            self._body,
...
class TestPhonebookContactCRUDList(_BasePhonebookContactCRUDTest):
    ...
    def test_that_listing_contacts_works(self):
        result = self._crud.list([self._tenant_uuid], database.PhonebookKey(uuid=self._phonebook_uuid))
    ...
    def test_that_only_the_tenant_can_list(self):
        wrong_tenant_uuid = new_uuid()
        assert_that(
            calling(self._crud.list).with_args([wrong_tenant_uuid], database.PhonebookKey(uuid=self._phonebook_uuid)),
            raises(exception.NoSuchPhonebook),
        )
...
class TestPhonebookContactCRUDCount(_BasePhonebookContactCRUDTest):
    ...
    def test_that_counting_counts(self):
        result = self._crud.count([self._tenant_uuid], database.PhonebookKey(uuid=self._phonebook_uuid))

        assert_that(result, equal_to(3))

        result = self._crud.count([self._tenant_uuid], database.PhonebookKey(id=self._phonebook_id))
        assert_that(result, equal_to(3))
    ...
    def test_that_counting_from_another_tenant_return_0(self):
        assert_that(
            calling(self._crud.count).with_args([new_uuid()], database.PhonebookKey(uuid=self._phonebook_uuid)),
            raises(exception.NoSuchPhonebook),
        )
        assert_that(
            calling(self._crud.count).with_args([new_uuid()], database.PhonebookKey(id=self._phonebook_id)),
            raises(exception.NoSuchPhonebook),
        )
...
class TestContactCRUD(_BaseTest):
    ...
    def test_that_personal_contacts_are_unique(self, user_uuid):
        ...
        assert_that(
            calling(self._crud.create_personal_contact).with_args(user_uuid, self.contact_1),
            raises(exception.DuplicatedContactException),
        )

    @with_user_uuid
    def test_that_personal_contacts_remain_unique(self, user_uuid):
        contact_1_uuid = self._crud.create_personal_contact(user_uuid, self.contact_1)['id']
        self._crud.create_personal_contact(user_uuid, self.contact_2)['id']

        assert_that(
            calling(self._crud.edit_personal_contact).with_args(user_uuid, contact_1_uuid, self.contact_2),
            raises(exception.DuplicatedContactException),
        )
        contact_list = self._crud.list_personal_contacts(user_uuid)
        assert_that(
    ...
    def test_get_personal_contact(self, user_uuid):
        contact_uuid, _, __ = self._insert_personal_contacts(user_uuid, self.contact_1, self.contact_2, self.contact_3)
...
class TestFavoriteCrud(_BaseTest):
    ...
    def test_that_delete_from_an_unknown_source_raises(self, user_uuid, source):
        ...
        assert_that(
            calling(self._crud.delete).with_args(user_uuid, 'not-source', 'the-contact-id'),
            raises(exception.NoSuchFavorite),
        )

    def _user_exists(self, user_uuid):
        with closing(Session()) as session:
            user_uuid = session.query(database.User.user_uuid).filter(database.User.user_uuid == user_uuid).scalar()
...
class TestPhonebookContactSearchEngine(_BaseTest):
    ...
    def tearDown(self):
        self.phonebook_crud.delete([self.tenant_uuid], database.PhonebookKey(uuid=self.phonebook_uuid))
        super().tearDown()
...
class TestPersonalContactSearchEngine(_BaseTest):
    ...
    def test_that_find_first_returns_a_contact(self, user_uuid):
        ...
        self._insert_personal_contacts(user_uuid, self.contact_1, self.contact_2, self.contact_3)
    ...
    @with_user_uuid
    def test_that_listing_personal_contacts_returns_the_searched_contacts(self, user_uuid):
...
class TestProfileCRUD(_BaseTest):
    ...
    def test_create_unknown_source(self, display):
        body = {
            ...
            'name': 'profile',
            'display_uuid': None,
            'services': {'lookup': {'sources': [{'uuid': 'eb124746-09be-44db-b01d-5b7dc1ea59a3'}]}},
        }
        ...
        assert_that(
            ...
            has_entries(
                ...
                services=has_entries(
                    ...
                    reverse=has_entries(
                        sources=contains_inanyorder(has_entries(uuid=source_1['uuid'], backend=source_1['backend'])),
                        timeout=0.5,
                    ),
                ),
            ),
