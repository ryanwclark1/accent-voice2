...
class BaseCSVCRUDTestCase(BaseDirdIntegrationTest):
    asset = 'all_routes'
    valid_body = {
        'name': 'main',
        'file': '/tmp/foobar.csv',
    ...
    def source(self, client, *args, **kwargs):
        source = client.csv_source.create(*args, **kwargs)
        try:
            yield source
        finally:
...
class TestDelete(BaseCSVCRUDTestCase):
    ...
    def test_delete(self, foobar):
        assert_that(
            calling(self.client.csv_source.delete).with_args(foobar['uuid']),
        ...
        try:
            self.client.csv_source.delete(UNKNOWN_UUID)
    ...
    @fixtures.csv_source(name='foosub', token=VALID_TOKEN_SUB_TENANT)
    def test_delete_multi_tenant(self, sub, main):
        ...
        try:
            sub_tenant_client.csv_source.delete(main['uuid'])
        except Exception as e:
        ...
        assert_that(
            calling(main_tenant_client.csv_source.delete).with_args(sub['uuid']),
            not_(raises(Exception)),
        )
...
class TestList(BaseCSVCRUDTestCase):
    ...
    @fixtures.csv_source(name='bcd', token=VALID_TOKEN_MAIN_TENANT)
    @fixtures.csv_source(name='cde', token=VALID_TOKEN_SUB_TENANT)
...
class TestPost(BaseCSVCRUDTestCase):
    ...
    def test_multi_tenant(self):
        ...
        assert_that(
            calling(sub_tenant_client.csv_source.create).with_args(self.valid_body, tenant_uuid=MAIN_TENANT),
        ...
        with self.source(main_tenant_client, self.valid_body):
            assert_that(
                calling(sub_tenant_client.csv_source.create).with_args(self.valid_body),
                not_(raises(Exception)),
            )
...
class TestPut(BaseCSVCRUDTestCase):
    ...
    @fixtures.csv_source(name='foobar')
    @fixtures.csv_source(name='other')
    def test_put(self, foobar, other):
        ...
        try:
            self.client.csv_source.edit(foobar['uuid'], {})
        except Exception as e:
            assert_that(e.response.status_code, equal_to(400))
            assert_that(
                e.response.json(),
        ...
        result = self.client.csv_source.get(foobar['uuid'])
        assert_that(
            result,
            has_entries(
                uuid=foobar['uuid'],
    ...
    def test_put_multi_tenant(self, sub, main):
        ...
        try:
            sub_tenant_client.csv_source.edit(main['uuid'], self.new_body)
        except Exception as e:
            self.assert_unknown_source_exception(main['uuid'], e)
        else:
            self.fail('Should have raised')
...
class TestGet(BaseCSVCRUDTestCase):
    ...
    def test_get(self, accent):
        response = self.client.csv_source.get(accent['uuid'])
        assert_that(response, equal_to(accent))

        try:
            self.client.csv_source.get(UNKNOWN_UUID)
        except Exception as e:
            self.assert_unknown_source_exception(UNKNOWN_UUID, e)
        else:
            self.fail('Should have raised')
    ...
    def test_get_multi_tenant(self, sub, main):
        ...
        response = main_tenant_client.csv_source.get(sub['uuid'])
        assert_that(response, equal_to(sub))

        try:
            sub_tenant_client.csv_source.get(main['uuid'])
        except Exception as e:
            self.assert_unknown_source_exception(main['uuid'], e)
        else:
            self.fail('Should have raised')
