...
class TestList(BaseGoogleCRUDTestCase):
    ...
    def test_searches(self, c, b, a):
        ...
        assert_that(self.list_(name='abc'), has_entries(items=contains(a), total=3, filtered=1))
...
class TestPost(BaseGoogleCRUDTestCase):
    ...
    def test_invalid_body(self):
        try:
            ...
            assert_that(
                e.response.json(),
                has_entries(message=ANY, error_id='invalid-data', details=has_entries(name=ANY)),
            )
        else:
        ...
        with self.source(self.client, self.valid_body) as source:
            assert_that(
                ...
                has_entries(
                    uuid=uuid_(),
                    name='google',
                    auth=has_entries(host='localhost', port=443, verify_certificate=True),
                ),
            )
    ...
    def test_multi_tenant(self):
        ...
        with self.source(main_tenant_client, self.valid_body):
            assert_that(
                calling(sub_tenant_client.backends.create_source).with_args('google', self.valid_body),
                not_(raises(Exception)),
            )
...
class TestPut(BaseGoogleCRUDTestCase):
    ...
    def test_put(self, foobar, other):
        ...
        assert_that(
            ...
            has_entries(uuid=foobar['uuid'], tenant_uuid=foobar['tenant_uuid'], name='new'),
        )
    ...
    def test_put_multi_tenant(self, sub, main):
        ...
        assert_that(
            calling(self.edit).with_args(main_client, main['uuid'], self.new_body, tenant_uuid=SUB_TENANT),
            raises(Exception).matching(HTTP_404),
        )
