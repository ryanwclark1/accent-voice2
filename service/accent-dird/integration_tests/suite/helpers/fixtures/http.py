...
def google_result(contact_list, search_list=None):
    def decorator(decorated):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            main_expectation = mock_server.create_expectation('/v1/people/me/connections', contact_list, 200)
            main_expectation['times']['unlimited'] = True
            mock_server.mock_any_response(main_expectation)

            if search_list:
                search_expectation = mock_server.create_expectation('/v1/people:searchContacts', search_list, 200)
...
def office365_result(contact_list):
    def decorator(decorated):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            mock_server = UnVerifiedMockServerClient(f'http://127.0.0.1:{office365_port}')
            ...
            expectation = mock_server.create_expectation('/v1.0/me/contacts', contact_list, 200)
            expectation['times']['unlimited'] = True
            expectation['httpRequest']['queryStringParameters'] = {'$top': [str(expected_count)]}
    ...
    def decorator(decorated):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            for current_page in pages:
                ...
                expectation = mock_server.create_expectation(current_page_path, current_page, 200)
                expectation['times']['unlimited'] = True
...
def office365_error():
    def decorator(decorated):
        ...
        def wrapper(self, *args, **kwargs):
            office365_port = self.service_port(443, 'microsoft.com')
            mock_server = UnVerifiedMockServerClient(f'http://127.0.0.1:{office365_port}')
            expectation = mock_server.create_expectation('/v1.0/me/contacts/error', {}, 404)
            expectation['times']['unlimited'] = True
            mock_server.mock_any_response(expectation)
...
def google_source(**source_args):
    def decorator(decorated):
        ...
        def wrapper(self, *args, **kwargs):
            source_args.setdefault('name', random_string())
            source_args.setdefault('token', VALID_TOKEN_MAIN_TENANT)
            source_args.setdefault('auth', {'host': 'auth', 'port': 9497, 'prefix': None, 'https': False})
    ...
    def decorator(decorated):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if 'phonebook_uuid' not in source_args:
                phonebook_generator = source_args.pop('phonebook_generator', phonebook_autocreate_generator)
...
def accent_source(**source_args):
    ...
    def decorator(decorated):
        ...
        def wrapper(self, *args, **kwargs):
            client = self.get_client(source_args['token'])
            source = client.accent_source.create(source_args)
            try:
                result = decorated(self, source, *args, **kwargs)
            finally:
                try:
