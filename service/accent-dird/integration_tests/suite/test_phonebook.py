...
class TestList(BasePhonebookTestCase):
    ...
    def test_valid(self):
        ...
        valid_body = {'name': 'foobar'}
        phonebook_1 = raise_for_status(self.post_phonebook(valid_body, tenant=valid.uuid)).json()
    ...
    def test_pagination(self):
        ...
        phonebook_1 = raise_for_status(self.post_phonebook(valid_body_1, tenant=pagination.uuid)).json()
        phonebook_2 = raise_for_status(self.post_phonebook(valid_body_2, tenant=pagination.uuid)).json()
        phonebook_3 = raise_for_status(self.post_phonebook(valid_body_3, tenant=pagination.uuid)).json()

        def assert_matches(result, *phonebooks):
            assert_that(
                result.json(),
                has_entries(
                    items=contains(*[has_entries(**phonebook) for phonebook in phonebooks]),
                    total=3,
                ),
            )

        result = self.list_phonebooks(order='name', direction='asc', tenant=pagination.uuid)
        assert_matches(result, phonebook_1, phonebook_2, phonebook_3)

        result = self.list_phonebooks(order='name', direction='desc', tenant=pagination.uuid)
...
class TestGet(BasePhonebookTestCase):
    def test_unknown_tenant(self):
        ...
        valid_body = {'name': 'foobar'}
        phonebook = raise_for_status(self.post_phonebook(valid_body, tenant=valid.uuid)).json()
...
class TestDelete(BasePhonebookTestCase):
    def test_unknown_tenant(self):
        invalid, *_ = self.set_tenants('invalid')
        valid_body = {'name': 'foobar'}
        phonebook = raise_for_status(self.post_phonebook(valid_body, tenant=invalid.uuid)).json()
...
class TestPut(BasePhonebookTestCase):
    def test_unknown_tenant(self):
        ...
        self.set_tenants()
        result = self.put_phonebook(phonebook['uuid'], {'name': 'new'}, tenant=generate_tenant_uuid())
        assert_that(result.status_code, equal_to(401))
    ...
    def test_valid(self):
        ...
        result = self.put_phonebook(phonebook['uuid'], {'name': 'new'}, tenant=valid.uuid)
        assert_that(result.status_code, equal_to(200))
    ...
    def test_unknown_phonebook(self):
        ...
        other, *_ = self.set_tenants('other')
        result = self.put_phonebook(phonebook['uuid'], {'name': 'updated'}, tenant=other.uuid)
        assert_that(result.status_code, equal_to(404))

        self.delete_phonebook(phonebook['uuid'], tenant=valid.uuid)
        result = self.put_phonebook(phonebook['uuid'], {'name': 'updated'}, tenant=valid.uuid)
        assert_that(result.status_code, equal_to(401))
    ...
    def test_duplicated(self):
        ...
        result = self.put_phonebook(phonebook['uuid'], {'name': 'duplicate me'}, tenant=invalid.uuid)
        assert_that(result.status_code, equal_to(409))
...
class _BasePhonebookContactTestCase(BasePhonebookTestCase):
    def setUp(self):
        ...
        self.phonebook_1 = raise_for_status(self.post_phonebook({'name': 'one'}, tenant=self.tenant_1.uuid)).json()

        self.tenant_2, *_ = self.set_tenants('other')
        self.phonebook_2 = raise_for_status(self.post_phonebook({'name': 'two'}, tenant=self.tenant_2.uuid)).json()

    def tearDown(self):
        self.set_tenants(self.tenant_1.name)
        raise_for_status(self.delete_phonebook(self.phonebook_1['uuid'], tenant=self.tenant_1.uuid))

        self.set_tenants(self.tenant_2.name)
        raise_for_status(self.delete_phonebook(self.phonebook_2['uuid'], tenant=self.tenant_2.uuid))
        super().tearDown()
...
class TestContactDelete(_BasePhonebookContactTestCase):
    ...
    def _delete(self, tenant, phonebook_uuid, contact_uuid):
        return self.delete_phonebook_contact(phonebook_uuid, contact_uuid, tenant=tenant.uuid)
...
class TestContactGet(_BasePhonebookContactTestCase):
    ...
    def _get(self, tenant, phonebook_id, contact_uuid):
        return self.get_phonebook_contact(phonebook_id, contact_uuid, tenant=tenant.uuid)
    ...
    def test_unknown_tenant_phonebook_or_contact(self):
        ...
        raise_for_status(
            self.delete_phonebook_contact(self.phonebook_1['uuid'], self.contact_uuid, tenant=self.tenant_1.uuid)
        )
        res = self._get(self.tenant_1, self.phonebook_1['uuid'], self.contact_uuid)
        assert_that(res.status_code, equal_to(404), 'unknown contact')
...
class TestContactList(_BasePhonebookContactTestCase):
    def test_unknown_tenant_or_phonebook(self):
        self.set_tenants(self.tenant_2.name)
        result = self.list_phonebook_contacts(self.phonebook_1['uuid'], tenant=self.tenant_2.uuid)
        assert_that(result.status_code, equal_to(404))

        result = self.list_phonebook_contacts(self.phonebook_1['uuid'], tenant=self.tenant_1.uuid)
        assert_that(result.status_code, equal_to(401))
    ...
    def test_pagination(self):
        ...
        contact_1 = raise_for_status(
            self.post_phonebook_contact(phonebook_uuid, body_1, tenant=self.tenant_1.uuid)
        ).json()
        contact_2 = raise_for_status(
            self.post_phonebook_contact(phonebook_uuid, body_2, tenant=self.tenant_1.uuid)
        ).json()
        contact_3 = raise_for_status(
            self.post_phonebook_contact(phonebook_uuid, body_3, tenant=self.tenant_1.uuid)
        ).json()
        ...
        result = self.list_phonebook_contacts(phonebook_uuid, order='lastname', tenant=self.tenant_1.uuid)
        assert_matches(result, contact_3, contact_2, contact_1)

        result = self.list_phonebook_contacts(phonebook_uuid, order='firstname', limit=2, tenant=self.tenant_1.uuid)
        assert_matches(result, contact_1, contact_2)

        result = self.list_phonebook_contacts(phonebook_uuid, order='firstname', offset=1, tenant=self.tenant_1.uuid)
        ...
        invalid_limit_offset = [-1, True, False, 'foobar', 3.14]
        for value in invalid_limit_offset:
            result = self.list_phonebook_contacts(phonebook_uuid, limit=value, tenant=self.tenant_1.uuid)
            assert_that(result.status_code, equal_to(400), value)

            result = self.list_phonebook_contacts(phonebook_uuid, offset=value, tenant=self.tenant_1.uuid)
            assert_that(result.status_code, equal_to(400), value)

        invalid_directions = [0, 'foobar', True, False, 3.14]
        for value in invalid_directions:
            result = self.list_phonebook_contacts(phonebook_uuid, direction=value, tenant=self.tenant_1.uuid)
            assert_that(result.status_code, equal_to(400), value)
...
class TestContactPost(_BasePhonebookContactTestCase):
    def test_unknown_tenant_or_phonebook(self):
        ...
        self.set_tenants(self.tenant_2.name)
        result = self.post_phonebook_contact(self.phonebook_1['uuid'], body, tenant=self.tenant_2.uuid)
        assert_that(result.status_code, equal_to(404))

        result = self.post_phonebook_contact(self.phonebook_1['uuid'], body, tenant=self.tenant_1.uuid)
        assert_that(result.status_code, equal_to(401))
    ...
    def test_duplicates(self):
        ...
        self.set_tenants(self.tenant_1.name)
        raise_for_status(self.post_phonebook_contact(self.phonebook_1['uuid'], body, tenant=self.tenant_1.uuid))
        result = self.post_phonebook_contact(self.phonebook_1['uuid'], body, tenant=self.tenant_1.uuid)
        assert_that(result.status_code, equal_to(409))

        self.set_tenants(self.tenant_2.name)
        result = self.post_phonebook_contact(self.phonebook_2['uuid'], body, tenant=self.tenant_2.uuid)
        assert_that(result.status_code, equal_to(201))
...
class TestContactPut(_BasePhonebookContactTestCase):
    ...
    def _put(self, tenant, phonebook_id, contact_uuid, body):
        return self.put_phonebook_contact(phonebook_id, contact_uuid, body, tenant=tenant.uuid)

    def test_unknown_tenant_phonebook_or_contact(self):
        body = {'firstname': 'Bob'}

        self.set_tenants(self.tenant_2.name)
        res = self._put(self.tenant_2, self.phonebook_1['uuid'], self.contact_uuid, body)
        assert_that(res.status_code, equal_to(404), 'unknown tenant')

        res = self._put(self.tenant_1, self.phonebook_1['uuid'], self.contact_uuid, body)
        assert_that(res.status_code, equal_to(401), 'unknown tenant')

        self.set_tenants(self.tenant_1.name)
        res = self._put(self.tenant_1, self.phonebook_2['uuid'], self.contact_uuid, body)
        assert_that(res.status_code, equal_to(404), 'unknown phonebook')
...
'''
    ...
    def test_post(self):
        ...
        assert_that(result.status_code, equal_to(200))
        assert_that(
            self.list_phonebook_contacts(self.phonebook_1['uuid'], tenant=self.tenant_1.uuid).json(),
            has_entries(
