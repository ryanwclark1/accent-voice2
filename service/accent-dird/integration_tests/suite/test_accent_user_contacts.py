...
from .helpers.config import new_accent_users_multiple_accent_config
...
class TestAccentContactList(BaseDirdIntegrationTest):
    asset = 'accent_users_multiple_accent'
    config_factory = new_accent_users_multiple_accent_config

    def setUp(self):
        super().setUp()
        for source in self.client.sources.list(backend='accent', recurse=True)['items']:
            if source['name'] == 'accent_america':
                self.source_uuid = source['uuid']
            elif source['name'] == 'accent_america_sub':
    ...
    def test_multi_tenant(self):
        ...
        assert_that(
            calling(self.contacts).with_args(sub_tenant_client, self.source_uuid, tenant_uuid=MAIN_TENANT),
        ...
        assert_that(
            calling(self.contacts).with_args(main_tenant_client, self.source_uuid, tenant_uuid=SUB_TENANT),
            raises(Exception).matching(has_properties(response=has_properties(status_code=404))),
        )
    ...
    def contacts(self, client, uuid, *args, **kwargs):
        return client.backends.list_contacts_from_source(backend=BACKEND, source_uuid=uuid, *args, **kwargs)
