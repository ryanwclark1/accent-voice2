...
class Menu:
    ...
    def init_app(self, app):
        ...
        app.context_processor(lambda: {'current_menu': current_menu})
...
class MenuEntryMixin:
    ...
    def register(
        ...
        **kwargs,
        ...
        if active_when is not None:
            active_when_param = inspect.getfullargspec(active_when)[
                0
            ]  # if PY3 else inspect.getfullargspec(active_when)[0]
    ...
    def submenu(self, path, auto_create=True):
        ...
        if path_head == '':
            ...
            if auto_create:
                ...
                next_entry = self._child_entries[path_head] = MenuEntryMixin(path_head, self)
            else:
    ...
    def children(self):
        ...
        return sorted(self._child_entries.values(), key=lambda entry: getattr(entry, 'order', 0))
...
def register_menu(
    ...
    **kwargs,
    ...
    def menu_decorator(f):
        ...
        expected = inspect.getfullargspec(f).args if PY3 else inspect.getfullargspec(f).args
