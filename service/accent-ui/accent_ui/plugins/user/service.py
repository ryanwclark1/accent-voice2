...
class UserService(BaseConfdService):
    ...
    def get(self, resource_id):
        ...
        accent_user = self._auth.users.get(resource_id)
        resource['username'] = accent_user['username']
        resource['auth_enabled'] = accent_user['enabled']
    ...
    def update(self, user):
        ...
        password = user.pop('password')
        super().update(user)
        self._update_accent_user(user, username, password)
    ...
    def _update_accent_user(self, user, username, password):
    ...
    def _create_line_and_associations(self, line):
        ...
        if endpoint_sip is not None:
            # Note: This is done here until we find a better solution
            max_retries = 3
