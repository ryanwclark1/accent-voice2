...
class ConfigRegistrarForm(BaseForm):
    ...
    main_host = StringField(
        l_('Main registrar host'), validators=[InputRequired(), Length(max=128)]
    )
    ...
    proxy_main_host = StringField(
        l_('Main proxy host'), validators=[InputRequired(), Length(max=128)]
    )
    proxy_main_port = IntegerField(l_('Main proxy port'))
    proxy_backup_host = StringField(
        l_('Backup proxy host'), validators=[Length(max=128)]
    )
...
class RawConfigDeviceForm(BaseForm):
    X_key = StringField(default='accent')
    locale = SelectField(
        l_('Language'),
        choices=[
            ('', l_('None')),
    ...
    X_accent_phonebook_ip = StringField(l_('Phonebook server'), validators=[IPAddress()])
    user_username = StringField(
        l_('User username'), validators=[InputRequired(), Length(max=128)]
    )
    user_password = StringField(
        l_('User password'), validators=[InputRequired(), Length(max=128)]
    )
    admin_username = StringField(
        l_('Admin username'), validators=[InputRequired(), Length(max=128)]
    )
    admin_password = StringField(
        l_('Admin password'), validators=[InputRequired(), Length(max=128)]
    )
    sip_subscribe_mwi = BooleanField(
        l_('Explicit notification of messages'), default=False
    )
    ...
    vlan_pc_port_id = IntegerField(
        l_('VLAN PC Port ID'), validators=[Regexp('^[0-9]+$')]
    )
...
class GeneralConfigurationForm(BaseForm):
    plugin_server = StringField(
        l_('Plugin server URL'), validators=[InputRequired(), Length(max=2048), URL()]
    )
    ...
    locale = StringField(
        l_('Locale'), validators=[Length(max=5), Regexp(r'^[a-z]{2}_[A-Z]{2}$')]
    )
...
class NetworkConfigurationForm(BaseForm):
    ...
    provision_http_port = IntegerField(
        l_('Provisioning port'), validators=[Regexp('^[0-9]+$')]
    )
