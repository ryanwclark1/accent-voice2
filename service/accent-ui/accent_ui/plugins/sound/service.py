...
class SoundService(BaseConfdService):
    ...
    def download_sound_filename(self, tenant_uuid, category, file_name, **kwargs):
        return self._confd.sounds.download_file(
            category, file_name, tenant_uuid=tenant_uuid, **kwargs
        )

    def delete_sound_filename(self, tenant_uuid, category, sound_filename, **kwargs):
        self._confd.sounds.delete_file(
            category, sound_filename, tenant_uuid=tenant_uuid, **kwargs
        )

    def upload_sound_filename(
        self, tenant_uuid, category, sound_filename, binary_content, **kwargs
    ):
        self._confd.sounds.upload_file(
            category, sound_filename, binary_content, tenant_uuid=tenant_uuid, **kwargs
        )
