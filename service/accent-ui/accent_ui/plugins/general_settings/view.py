...
class ConfBridgeGeneralSettingsView(BaseGeneralSettingsView):
    ...
    def _map_resources_to_form(self, resource):
        resource['accent_default_user']['options'] = self._build_options(
            resource['accent_default_user']['options']
        )
        resource['accent_default_bridge']['options'] = self._build_options(
            resource['accent_default_bridge']['options']
        )
        form = self.form(data=resource)
        return form
    ...
    def _map_form_to_resources(self, form, form_id=None):
        data = form.to_dict()
        data['accent_default_user']['options'] = self._map_options_to_resource(
            data['accent_default_user']['options']
        )
        data['accent_default_bridge']['options'] = self._map_options_to_resource(
            data['accent_default_bridge']['options']
        )
        return data
