...
class AccessCommand(Command):
    ...
    def take_action(self, parsed_args):
        ...
        # random port
        for _attempt in range(ssh_max_retries):
...
Opening the access using this command:
    ...
    def open_access(self, parsed_args):
        ...
        full_command = ['timeout', '--foreground', str(parsed_args.timeout), *ssh_command]

        human_readable_timeout = time.strftime("%H:%M:%S", time.gmtime(parsed_args.timeout))
