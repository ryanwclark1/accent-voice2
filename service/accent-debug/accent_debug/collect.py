...
class CollectCommand(Command):
    """Collect information about a Accent server"""
    ...
    def take_action(self, parsed_args):
        with tempfile.TemporaryDirectory(prefix='accent-debug-') as temp_directory:
            logger.info('Created temporary directory: "%s"', temp_directory)

            gathering_directory = os.path.join(temp_directory, 'accent-debug')
...
def gather_engine_info(gathering_directory):
    ...
    command = ['rsync', '-a', '/usr/share/accent/ACCENT-VERSION', gathering_directory]
    call(command)
