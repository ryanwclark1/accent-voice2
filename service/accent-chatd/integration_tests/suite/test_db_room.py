...
from accent_chatd.database.models import Room, RoomMessage, RoomUser
...
class TestRoom(DBIntegrationTest):
    ...
    def test_create_with_users(self):
        room_user = RoomUser(uuid=USER_UUID_1, tenant_uuid=UUID, accent_uuid=UUID)
    ...
    def test_add_message(self, room):
        message = RoomMessage(user_uuid=UUID, tenant_uuid=UUID, accent_uuid=UUID)
...
class TestRoomRelationships(DBIntegrationTest):
    ...
    def test_users_get(self, room):
        room_user = RoomUser(
            room_uuid=room.uuid, uuid=USER_UUID_1, tenant_uuid=UUID, accent_uuid=UUID
        )
    ...
    def add_room_message(self, **kwargs):
        ...
        kwargs.setdefault('accent_uuid', uuid.uuid4())
