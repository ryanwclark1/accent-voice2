# accent_auth/policies/service.py
...
class PolicyService:  # No longer inherits from BaseService
    ...
    async def delete(self, policy_uuid: str, db: AsyncSession = None) -> None:
        await self._dao.policy.delete(policy_uuid, session=db)
    ...
    async def get_by_slug(self, slug, db: AsyncSession, tenant_uuids=None):
        return await self._dao.policy.find_by(
            slug=slug, tenant_uuids=tenant_uuids, session=db
        )
