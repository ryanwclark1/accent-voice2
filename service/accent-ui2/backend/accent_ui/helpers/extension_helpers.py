...
def split_extension(exten: str) -> tuple[str, ...]:
  """
    Splits a multi-part extension string into its individual components.

    This function takes a specially formatted extension string and splits it into
    its constituent parts. The format of the extension string is expected to have
    digits and asterisks in a specific pattern. The function will raise a ValueError
    if the format is incorrect.

    Args:
      exten (str): The extension string to be split.

    Returns:
      tuple[str, ...]: A tuple containing the individual parts of the extension.

    Raises:
      ValueError: If the input is not a string or if the format of the extension
            string is incorrect.
