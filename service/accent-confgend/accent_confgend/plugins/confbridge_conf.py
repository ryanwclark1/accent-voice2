...
class _ConfBridgeConf:
    ...
    def _format_bridge_profile(self, row):
        yield f'[accent-bridge-profile-{row.id}](accent_default_bridge)'
        yield 'type = bridge'
    ...
    def _format_user_profile(self, row):
        yield f'[accent-user-profile-{row.id}](accent_default_user)'
        yield 'type = user'
    ...
    def _format_admin_profile(self, row):
        yield f'[accent-admin-profile-{row.id}](accent-user-profile-{row.id})'
        yield 'admin = yes'
    ...
    def _gen_default_user_menu(self):
        yield '[accent-default-user-menu]'
        yield 'type = menu'
    ...
    def _gen_default_admin_menu(self):
        yield '[accent-default-admin-menu](accent-default-user-menu)'
        yield '* = playback_and_continue({})'.format(
            '&'.join(
                [
                    'dir-multi1',
    ...
    def _gen_default_meeting_config(self):
        yield '[accent-meeting-bridge-profile]'
        yield 'type = bridge'
        ...
        yield ''
        yield '[accent-meeting-user-profile]'
