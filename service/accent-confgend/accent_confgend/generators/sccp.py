...
class _SccpLineConf:
    ...
    def _generate_guest_line(self, ast_writer):
        ...
        ast_writer.write_option('context', 'accent-initconfig')
    ...
    def _generate_lines(self, sccplines, ast_writer):
        for item in sccplines:
            ...
            ast_writer.write_option(
                'setvar',
                'ACCENT_ORIGINAL_CALLER_ID="{cid_name}" <{cid_num}>'.format(**item),
            )
            ast_writer.write_option(
                'setvar', f"ACCENT_USERID={item['user_id']}"
            )  # Deprecated in 24.01
            ast_writer.write_option('setvar', f"ACCENT_USERID={item['user_id']}")
            ast_writer.write_option(
                'setvar', f"ACCENT_USERUUID={item['uuid']}"
            )  # Deprecated in 24.01
            ast_writer.write_option('setvar', f"ACCENT_USERUUID={item['uuid']}")
            ast_writer.write_option('setvar', f"ACCENT_TENANT_UUID={item['tenant_uuid']}")
            ast_writer.write_option(
                'setvar', 'PICKUPMARK={number}%{context}'.format(**item)
            )
            ast_writer.write_option('setvar', f"TRANSFER_CONTEXT={item['context']}")
            ast_writer.write_option('setvar', 'ACCENT_CHANNEL_DIRECTION=from-accent')
            ast_writer.write_option('setvar', f"ACCENT_LINE_ID={item['id']}")
