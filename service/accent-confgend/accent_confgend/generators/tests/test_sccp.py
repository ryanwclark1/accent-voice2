...
from accent_confgend.generators.sccp import (
...
from accent_confgend.generators.tests.util import (
    assert_config_equal,
...
class TestSccpConf(unittest.TestCase):
    @patch(
        'accent_dao.asterisk_conf_dao.find_sccp_general_settings', Mock(return_value=[])
    )
    @patch('accent_dao.asterisk_conf_dao.find_sccp_line_settings', Mock(return_value=[]))
    @patch(
        'accent_dao.asterisk_conf_dao.find_sccp_device_settings', Mock(return_value=[])
    )
    @patch(
        'accent_dao.asterisk_conf_dao.find_sccp_speeddial_settings', Mock(return_value=[])
    )
    def setUp(self):
    ...
    def test_generate(self):
        assert_generates_config(
            ...
            [guestline](accent_line_tpl)
            type = line
            context = accent-initconfig
            cid_name = Autoprov
            cid_num = autoprov
        ''',
        )
...
class TestSccpDeviceConf(unittest.TestCase):
    ...
    def test_one_device_no_line_no_voicemail(self):
        ...
        assert_config_equal(
            self._output.getvalue(),
            '''
            [SEPACA016FDF235](accent_device_tpl)
            type = device
        ''',
        )
...
class TestSccpLineConf(unittest.TestCase):
    ...
    def test_one_element_lines_section(self):
        ...
        assert_config_equal(
            ...
            cid_name = jimmy
            cid_num = 100
            setvar = ACCENT_ORIGINAL_CALLER_ID="jimmy" <100>
            setvar = ACCENT_USERID=1
            setvar = ACCENT_USERID=1
            setvar = ACCENT_USERUUID={uuid}
            setvar = ACCENT_USERUUID={uuid}
            setvar = ACCENT_TENANT_UUID=tenant-uuid
            setvar = PICKUPMARK=100%a_context
            setvar = TRANSFER_CONTEXT=a_context
            setvar = ACCENT_CHANNEL_DIRECTION=from-accent
            setvar = ACCENT_LINE_ID=13423
