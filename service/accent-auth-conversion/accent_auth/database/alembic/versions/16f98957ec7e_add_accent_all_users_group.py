...
def rename_groups_already_present():
    group_name_query = sa.sql.select(
        [sa.sql.functions.concat('accent-all-users-tenant-', tenant_table.c.uuid)]
...
def create_accent_all_users_group():
...
def delete_accent_all_users_group():
    query = group_table.delete().where(
        group_table.c.name.in_(
            sa.sql.select(
                [
                    sa.sql.functions.concat(
                        'accent-all-users-tenant-', tenant_table.c.uuid
                    ),
                ]
            ).select_from(tenant_table)
        )
...
def upgrade():
    ...
    group_uuids = create_accent_all_users_group()
    add_all_users_in_group(group_uuids)


def downgrade():
    delete_accent_all_users_group()
