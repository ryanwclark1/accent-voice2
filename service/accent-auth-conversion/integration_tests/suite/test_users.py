...
class TestUsers(base.APIIntegrationTest):
    ...
    def test_password_reset_does_not_disable_old_password(self, foobar, foobaz):
        ...
        assert_no_error(user_client.token.new, 'accent_user')
    ...
    def test_post_with_top_tenant_admin(self, isolated):
        ...
        with self.user(self.client, **args) as user:
            ...
            accent_all_users_group = self.client.groups.list(
                name=f'accent-all-users-tenant-{self.top_tenant_uuid}',
                tenant_uuid=self.top_tenant_uuid,
            )['items'][0]
            accent_all_users_group_members = self.client.groups.get_users(
                accent_all_users_group['uuid']
            )['items']
            assert_that(
                accent_all_users_group_members,
        ...
        with self.user(self.client, **user_args) as user:
            ...
            assert_http_error(401, user_client.token.new, 'accent_user')
    ...
    def test_put_password(self, user, policy):
        ...
        token_data = user_client.token.new('accent_user', expiration=5)
        ...
        assert_no_error(user_client.token.new, 'accent_user', expiration=5)
