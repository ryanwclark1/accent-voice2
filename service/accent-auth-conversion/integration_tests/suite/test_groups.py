...
class TestGroups(base.APIIntegrationTest):
    ...
    def test_list_tenant_filtering(self, _, one, two, three):
        ...
        with self.client_in_subtenant() as (client, _, sub_tenant):
            ...
            default_groups = [
                has_entries(name=starts_with('accent')),
            ] * NB_DEFAULT_GROUPS
    ...
    def test_list_searching(self, _, one, two, three):
        ...
        admin_group = has_entries(name='accent_default_admin_group')
    ...
    def test_list_sorting(self, _, one, two, three):
        ...
        all_users_group = self.client.groups.list(
            name=f'accent-all-users-tenant-{base.SUB_TENANT_UUID}',
            tenant_uuid=base.SUB_TENANT_UUID,
        )['items'][0]
        admins_group = self.client.groups.list(
            name='accent_default_admin_group',
            tenant_uuid=base.SUB_TENANT_UUID,
        )['items'][0]
    ...
    def test_default_groups(self, _):
        ...
        assert_that(
            ...
            has_entries(
                ...
                items=contains_inanyorder(
                    has_entries(slug=f'accent-all-users-tenant-{self.top_tenant_uuid}'),
                    has_entries(slug='accent_default_admin_group'),
                ),
            ),
        )
        ...
        admin_group_uuid = self.client.groups.list(
            tenant_uuid=self.top_tenant_uuid, slug='accent_default_admin_group'
        )['items'][0]['uuid']
        ...
        assert_that(
            response,
            has_entries(
                items=contains_inanyorder(has_entries(slug='accent_default_admin_policy'))
            ),
        )
        ...
        assert_that(
            ...
            has_entries(
                ...
                filtered=NB_DEFAULT_GROUPS,
                items=contains_inanyorder(
                    has_entries(slug=f'accent-all-users-tenant-{base.SUB_TENANT_UUID}'),
                    has_entries(slug='accent_default_admin_group'),
                ),
            ),
        )
        ...
        admin_group_uuid = self.client.groups.list(
            tenant_uuid=base.SUB_TENANT_UUID, slug='accent_default_admin_group'
        )['items'][0]['uuid']
        response = self.client.groups.get_policies(
