...
from accent_call_logd.email import TemplateFormatter
from accent_call_logd.plugins.export.notifier import ExportNotifier
...
class RecordingExportTask(Task):
    def _run(
        ...
        bus_publisher = BusPublisher(service_uuid=config['uuid'], **config['bus'])
        ...
        notifier.updated(export)
        if email:
            self._send_email(task_uuid, 'Accent user', email, config, connection_info)
