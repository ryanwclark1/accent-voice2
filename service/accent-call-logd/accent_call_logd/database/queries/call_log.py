...
from accent_call_logd.datatypes import CallDirection, OrderDirection
...
class CallLogDAO(BaseDAO):
    ...
    def _apply_filters(self, query: Query, params: dict[str, Any]) -> Query:
        ...
        if terminal_user_uuids := params.get('terminal_user_uuids'):
            ...
            # NOTE: if no participant has answered, destination participant is
            # an arbitrary 'destination' participant based on uuid ordering
            # NOTE: destination participant definition must be reimplemented here
    ...
    def create_from_list(self, call_logs):
        ...
        with self.new_session() as session:
            for call_log in call_logs:
                ...
                session.flush()
                # NOTE: fetch relationship before expunge_all
                call_log.recordings
