...
import dateutil
from accent.asterisk.line_identity import identity_from_channel
...
WAIT_FOR_MOBILE_REGEX = re.compile(r'^Local/(\S+)@accent_wait_for_registration-\S+;2$')
MATCHING_MOBILE_PEER_REGEX = re.compile(r'^PJSIP/(\S+)-\S+$')
MEETING_EXTENSION_REGEX = re.compile(r'^accent-meeting-.*$')
...
# in accent-calld and accent-agid
RECORDING_PATH_REGEX = re.compile(
    rf'/var/lib/accent/sounds/tenants/({UUID_REGEX.pattern})/monitor/({UUID_REGEX.pattern}).wav'
)
...
def _extract_user_missed_call_variables(extra):
    ...
    accent_tenant_uuid = extra_tokens[0].split(': ')[1]
...
def _parse_accent_originate_all_lines_extra(extra: str) -> OriginateAllLinesInfo:
    wrapper = extract_cel_extra(extra)
    if not wrapper:
        raise CELInterpretationError(
            event_name=CELEventType.accent_originate_all_lines, raw_data=extra
        )
...
class CallerCELInterpretor(AbstractCELInterpretor):
    def __init__(self):
        self.eventtype_map = {
            ...
            # CELGenUserEvent
            CELEventType.accent_from_s: self.interpret_accent_from_s,
            CELEventType.accent_incall: self.interpret_accent_incall,
            CELEventType.accent_outcall: self.interpret_accent_outcall,
            CELEventType.accent_user_fwd: self.interpret_accent_user_fwd,
            CELEventType.accent_meeting_name: self.interpret_accent_meeting_name,
            CELEventType.accent_conference: self.interpret_accent_conference,
            CELEventType.accent_user_missed_call: self.interpret_accent_user_missed_call,
            CELEventType.accent_call_log_destination: self.interpret_accent_call_log_destination,
        }
    ...
    def interpret_accent_from_s(self, cel, call):
        call.requested_exten = call.extension_filter.filter(cel.exten)
        call.requested_context = cel.context
        call.destination_exten = call.extension_filter.filter(cel.exten)
        return call

    def interpret_accent_incall(self, cel, call):
    ...
    def interpret_accent_outcall(self, cel, call: RawCallLog):
    ...
    def interpret_accent_user_fwd(self, cel, call: RawCallLog):
        call.was_forwarded = True
        extra = re.match(EXTRA_USER_FWD_REGEX, cel.extra)

        # Replace destination_exten here because ACCENT_USER_MISSED_CALL event
        ...
        if call.interpret_caller_accent_user_fwd:
            if extra:
            ...
            call.interpret_caller_accent_user_fwd = False
        return call

    def interpret_accent_conference(self, cel, call):
        extra = extract_cel_extra(cel.extra)
        if not extra:
            logger.error(
                'Cannot interpret ACCENT_CONFERENCE event(cel.id=%s), missing extra data',
                cel.id,
            )
            return call
        ...
        logger.debug(
            'Identified destination name from ACCENT_CONFERENCE(id=%s): %s', name, cel.id
        )
        return call

    def interpret_accent_meeting_name(self, cel, call):
        extra = extract_cel_extra(cel.extra)
        if not extra:
            logger.error(
                'Cannot interpret ACCENT_MEETING_NAME event(cel.id=%s), missing extra data',
                cel.id,
            )
            return call
        call.destination_name = extra['extra']
        logger.debug(
            'Identified destination name from ACCENT_MEETING_NAME(id=%s): %s',
    ...
    def interpret_accent_user_missed_call(self, cel, call: RawCallLog):
        extra = extract_cel_extra(cel.extra)
        if not extra:
            logger.error(
                'Cannot interpret ACCENT_USER_MISSED_CALL event(cel.id=%s), missing extra data',
                cel.id,
            )
            return call
        ...
        if source_user_uuid:
            ...
            logger.debug(
                ...
                " from ACCENT_USER_MISSED_CALL event",
                source_user_uuid,
                source_name,
            )
        ...
        call.set_tenant_uuid(accent_tenant_uuid)
        call.destination_name = destination_name
        logger.debug(
            'Identified destination ("%s" <%s>) from ACCENT_USER_MISSED_CALL(id=%s)',
            call.destination_name,
    ...
    def interpret_accent_call_log_destination(self, cel, call: RawCallLog):
        extra = extract_cel_extra(cel.extra)
        if not extra:
            return call

        extra_dict = _extract_call_log_destination_variables(extra)
        logger.debug('accent_call_log_destination payload: %s', extra_dict)
        ...
        if extra_dict['type'] == 'conference':
            ...
            logger.debug(
                ...
                " from ACCENT_CALL_LOG_DESTINATION",
            ...
            logger.debug(
                ...
                'from ACCENT_CALL_LOG_DESTINATION(type=%s)',
                call.destination_name,
                extra_dict['type'],
            )
        ...
        # we assume information from ACCENT_CALL_LOG_DESTINATION
        # is authoritative and should not be overwritten by other events
        logger.debug(
            'setting destination info from ACCENT_CALL_LOG_DESTINATION(id=%s) '
            'as authoritative',
            cel.id,
        )
...
class LocalOriginateCELInterpretor:
    def interpret_cels(self, cels, call: RawCallLog):
        ...
        is_incall = any([True for cel in cels if cel.eventtype == 'ACCENT_INCALL'])
        is_outcall = any([True for cel in cels if cel.eventtype == 'ACCENT_OUTCALL'])
        if is_incall:
            call.direction = 'inbound'
        if is_outcall:
            call.direction = 'outbound'

        # extract tenant and user info from ACCENT_ORIGINATE_ALL_LINES custom event
        try:
            accent_originate_all_lines = next(
                cel
                for cel in cels
                if cel.eventtype == CELEventType.accent_originate_all_lines
            )
        except StopIteration:
            logger.debug(f'No {CELEventType.accent_originate_all_lines} cel found')
        else:
            logger.info(f'processing {CELEventType.accent_originate_all_lines} cel entry')
            try:
                info = _parse_accent_originate_all_lines_extra(
                    accent_originate_all_lines.extra
                )
            except CELInterpretationError:
                logger.exception(
                    f'Failed to interpret info from {CELEventType.accent_originate_all_lines}'
                    ' payload for CEL id %d',
                    accent_originate_all_lines.id,
                )
            else:
                logger.debug(
                    f'tenant identified from {CELEventType.accent_originate_all_lines}: %s',
                    info['tenant_uuid'],
                )
                call.set_tenant_uuid(info['tenant_uuid'])
                call.raw_participants[accent_originate_all_lines.channame].update(
