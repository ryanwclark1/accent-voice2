...
from accent_call_logd.database.models import CallLog, Export, Recording
...
class _BasePurger(DBIntegrationTest):
    ...
    def setUpClass(cls):
        ...
        cls.config_file = '/etc/accent-purge-db/conf.d/10-enable-plugin.yml'
    ...
    def _purge(self, days_to_keep):
        command = [
            '/bin/bash',
            '-c',
            f'accent-purge-db -d {days_to_keep} &> /proc/1/fd/1',
        ]
        rc = self.docker_exec(
