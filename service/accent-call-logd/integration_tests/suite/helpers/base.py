...
import pytz
from accent_call_logd_client.client import Client as CallLogdClient
...
CEL_DB_URI = os.getenv('DB_URI', 'postgresql://asterisk:password123@127.0.0.1:{port}')
DB_URI = os.getenv('DB_URI', 'postgresql://accent-call-logd:secret@127.0.0.1:{port}')
...
class _BaseIntegrationTest(AssetLaunchingTestCase):
    ...
    def reset_clients(cls):
        ...
        if not isinstance(cls.auth, WrongClient):
            ...
            cls.configure_accent_auth_for_multitenants()
    ...
    def make_bus(cls):
        ...
        bus = CallLogBusClient.from_connection_fields(
            port=port, exchange_name='accent-headers', exchange_type='headers'
        )
        return bus
    ...
    def auth_stopped(self):
        ...
        self.reset_clients()
        until.true(self.auth.is_up, tries=5, message='accent-auth did not come back up')

    @classmethod
    def configure_accent_auth_for_multitenants(cls):
        # NOTE: This creates a tenant tree and associated users
        cls.auth.set_token(
            MockUserToken(
                MASTER_TOKEN,
                MASTER_USER_UUID,
