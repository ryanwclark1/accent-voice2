...
class _PackageBuilder:
    ...
    def validate(self, ctx):
        validator = Validator.new_from_config(ctx.config, ctx.accent_version, ctx.install_params)
    ...
    def install_dependencies(self, ctx):
        ...
        current_accent_version = ctx.accent_version
        for dependency in dependencies:
            ctx.log(logger.info, 'installing dependency %s', dependency)
            self.install_dependency(ctx, dependency, current_accent_version)
        return ctx

    def install_dependency(self, ctx, dep, current_accent_version):
        try:
        ...
        ctx = Context(
            ...
            accent_version=current_accent_version,
        )
    ...
    def package(self, ctx):
        ...
        installed_plugin_data_path = os.path.join(pkgdir, 'usr/lib/accent-plugind/plugins', ctx.namespace, ctx.name)
