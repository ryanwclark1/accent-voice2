...
import yaml
from accent_market_client import Client as MarketClient
from requests import HTTPError
from unidecode import unidecode

from accent_plugind.helpers import version
...
class MarketPluginUpdater:
    def __init__(self, plugin_db, current_accent_version):
    ...
    def _add_upgradable_field(self, plugin_info, plugin):
        for version_info in plugin_info.get('versions', []):
            ...
            min_accent_version = version_info.get('min_accent_version', self._current_accent_version)
            max_accent_version = version_info.get('max_accent_version', self._current_accent_version)
            proposed_version = version_info.get('version')

            if version.less_than(self._current_accent_version, min_accent_version):
                version_info['upgradable'] = False
            elif version.less_than(max_accent_version, self._current_accent_version):
                version_info['upgradable'] = False
...
class MarketDB:
    def __init__(self, market_proxy, current_accent_version, plugin_db=None):
        self._market_proxy = market_proxy
        self._updater = MarketPluginUpdater(plugin_db, current_accent_version)
    ...
    def _strict_filter(content, **kwargs):
        def match(metadata):
            return all(metadata.get(key) == value for key, value in kwargs.items())
...
class Plugin:
    def __init__(self, config, namespace, name):
        ...
        self.name = name
        self.debian_package_name = f'{config["default_debian_package_prefix"]}-{name}-{namespace}'
    ...
    def _extract_namespace_and_name(package_name_prefix, package_name):
        package_name_pattern = re.compile(fr'^{package_name_prefix}-([a-z0-9-]+)-([a-z0-9]+)$')
