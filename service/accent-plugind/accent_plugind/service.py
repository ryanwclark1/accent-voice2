...
from .helpers import AccentVersionFinder, exec_and_log
...
class PluginService:
    def __init__(
        ...
        self._accent_version_finder = accent_version_finder
    ...
    def create(self, method, params, options):
        ...
        accent_version = self._accent_version_finder.get_version()
        ctx = Context(
            self._config,
            method=method,
    ...
    def _new_market_db(self, market_proxy):
        current_accent_version = self._accent_version_finder.get_version()
        return db.MarketDB(market_proxy, current_accent_version, self._plugin_db)

    @classmethod
    def from_config(cls, config, *args, **kwargs):
        kwargs['plugin_db'] = db.PluginDB(config)
        kwargs['accent_version_finder'] = AccentVersionFinder(config)
