...
class BaseIntegrationTest(AssetLaunchingTestCase):
    ...
    def directory_is_empty_in_container(self, path):
        for _filename in self.list_file_in_container_dir(path):
            return False
        return True

    def exists_in_container(self, path):
        directory, filename = os.path.split(path)
        return any(current_filename == filename for current_filename in self.list_file_in_container_dir(directory))
