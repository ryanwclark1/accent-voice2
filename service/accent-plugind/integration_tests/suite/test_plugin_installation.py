...
class TestPluginDependencies(BaseIntegrationTest):
    ...
    def test_that_dependencies_are_installed(self):
        ...
        one_is_installed = self._is_installed('accent-plugind-one-dependency')
        two_is_installed = self._is_installed('accent-plugind-two-dependency')
        three_is_installed = self._is_installed('accent-plugind-three-dependency')
        four_is_installed = self._is_installed('accent-plugind-four-dependency')
    ...
    def test_that_a_satisfied_dependency_does_not_block_the_install(self):
        ...
        def assert_received(bus_accumulator):
            ...
            completed = [event for event in events if event['message']['data']['status'] == 'completed']
    ...
    def test_given_dependency_error_when_install_then_error(self):
        ...
        one_is_installed = self._is_installed('accent-plugind-one-dependencynotfound')
        three_is_installed = self._is_installed('accent-plugind-three-dependency')
...
class TestPluginInstallation(BaseIntegrationTest):
    ...
    def test_when_it_works(self):
        ...
        def assert_received(bus_accumulator):
            assert_that(
                ...
                has_items(
                    has_entries(
                        message=has_entries(data=has_entries(uuid=result['uuid'], status='starting')),
                    ),
                    has_entries(
                        message=has_entries(data=has_entries(uuid=result['uuid'], status='downloading')),
                    ),
                    has_entries(
                        message=has_entries(data=has_entries(uuid=result['uuid'], status='extracting')),
                    ),
                    has_entries(
                        message=has_entries(data=has_entries(uuid=result['uuid'], status='building')),
                    ),
                    has_entries(
                        message=has_entries(data=has_entries(uuid=result['uuid'], status='packaging')),
                    ),
                    has_entries(
                        message=has_entries(data=has_entries(uuid=result['uuid'], status='updating')),
                    ),
                    has_entries(
                        message=has_entries(data=has_entries(uuid=result['uuid'], status='installing')),
                    ),
                    has_entries(
                        message=has_entries(data=has_entries(uuid=result['uuid'], status='completed')),
                    ),
                ),
            )
    ...
    def test_get_plugin(self):
        ...
        assert_that(
            calling(plugind_unauthorized.plugins.get).with_args(namespace, 'not-foobar'),
            raises(HTTPError).matching(has_property('response', has_property('status_code', 401))),
        )
    ...
    def test_install_from_git_branch(self):
        ...
        result = self.install_plugin(url='file:///data/git/repo', method='git', options={'ref': 'v2'})
    ...
    def test_that_plugin_build_directory_is_removed_after_an_install(self):
        ...
        directory_is_empty = self.directory_is_empty_in_container('/var/lib/accent-plugind/tmp')
    ...
    def test_that_uninstalling_an_uninstalled_plugin_returns_404(self):
        assert_that(
            calling(self.uninstall_plugin).with_args(namespace='plugindtests', name='uninstalled'),
            raises(HTTPError).matching(has_property('response', has_property('status_code', 404))),
        )
    ...
    def test_that_an_unauthorized_token_return_401(self):
        assert_that(
            calling(self.install_plugin).with_args(url='/data/git/repo', method='git', token='expired'),
            raises(HTTPError).matching(has_property('response', has_property('status_code', 401))),
        )

    def test_that_an_unauthorized_token_return_401_when_uninstall(self):
        assert_that(
            calling(self.uninstall_plugin).with_args(namespace='plugindtests', name='foobar', token='expired'),
            raises(HTTPError).matching(has_property('response', has_property('status_code', 401))),
        )
    ...
    def test_that_an_out_of_date_debian_cache_does_not_break_package_install(self):
        self.install_plugin(url='file:///data/git/add_accent_source_list', method='git', _async=False)
        ...
        def assert_received(bus_accumulator):
            assert_that(
                ...
                contains_exactly(
                    ...
                    has_entries(
                        message=has_entries(
                            data=has_entries(uuid=result['uuid'], status='installing dependencies'),
                        )
                    ),
                    has_entries(
                        message=has_entries(
