...
from accent_calld.plugin_helpers.exceptions import InvalidExtension
...
class FaxesService:
    ...
    def send_fax(self, tenant_uuid, content, fax_infos, user_uuid=None):
        ...
        pdf_file_descriptor, pdf_path = mkstemp(prefix='accent-fax-', suffix='.pdf')
        with os.fdopen(pdf_file_descriptor, 'wb') as pdf_file:
            pdf_file.write(content)
            pdf_file.close()

        tif_path = f'{pdf_path}.tif'
        command = ['/usr/bin/accent-pdf2fax', '-o', tif_path, pdf_path]
        logger.debug('Running command: %s', command)
        try:
            subprocess.run(command, capture_output=True)
        except subprocess.CalledProcessError as e:
            logger.debug('accent-pdf2fax output: %s', e.stdout)
            logger.debug('accent-pdf2fax stderr: %s', e.stderr)
            logger.error(e)
            raise FaxFailure(message='Conversion from PDF to TIFF format failed')

        if not os.path.exists(tif_path):
            logger.error('accent-pdf2fax: no output file "%s"', tif_path)
            raise FaxFailure(
        ...
        originate_variables = {
            'ACCENT_FAX_DESTINATION_CONTEXT': fax_infos['context'],
            'ACCENT_FAX_DESTINATION_EXTENSION': fax_infos['extension'],
            'ACCENT_FAX_DESTINATION_IVR_EXTENSION': fax_infos['ivr_extension'] or '',
            'ACCENT_FAX_DESTINATION_WAIT_TIME': wait_time_str,
            'ACCENT_TENANT_UUID': tenant_uuid,
            'ACCENT_FAX_PATH': tif_path,
            'ACCENT_USERUUID': user_uuid or '',
        }
