...
from accent_calld.plugin_helpers.ari_ import Bridge, Channel
...
class AdhocConferencesService:
    ...
    def create_from_user(self, host_call_id, participant_call_ids, user_uuid):
        ...
        try:
            host_peer_accent_channel = self._find_peer_channel(host_call_id)
        ...
        for participant_call_id in participant_call_ids:
            ...
            try:
                peer_accent_channel = self._find_peer_channel(participant_call_id)
            except NotEnoughChannels:
                logger.error(
            ...
            if peer_accent_channel.user() != user_uuid:
                raise ParticipantCallNotFound(participant_call_id)
        ...
        self._redirect_host(
            host_call_id, host_peer_accent_channel.id, adhoc_conference_id
        )

        remaining_participant_call_ids = set(participant_call_ids) - {
            host_peer_accent_channel.id
        }
        logger.debug(
    ...
    def _redirect_host(self, host_call_id, host_peer_channel_id, adhoc_conference_id):
        ...
        try:
            host_channel.setChannelVar(
                variable='ACCENT_IS_ADHOC_CONFERENCE_HOST',
                value='true',
                bypassStasis=True,
            )
        ...
        if self._channels_are_in_stasis(*channel_ids):
            ...
            try:
                host_channel.setChannelVar(
                    variable='ACCENT_ADHOC_CONFERENCE_ID',
    ...
    def delete_from_user(self, adhoc_conference_id, user_uuid):
        ...
        if (
            bridge_helper.global_variables.get(variable='ACCENT_HOST_USER_UUID')
            != user_uuid
        ):
            raise AdhocConferenceNotFound(adhoc_conference_id)
    ...
    def add_participant_from_user(
        ...
        try:
            discarded_host_accent_channel = self._find_peer_channel(participant_call_id)
        except NotEnoughChannels:
            logger.error(
        ...
        if discarded_host_accent_channel.user() != user_uuid:
            raise ParticipantCallNotFound(participant_call_id)

        self._redirect_participant(
            participant_call_id, discarded_host_accent_channel.id, adhoc_conference_id
        )
