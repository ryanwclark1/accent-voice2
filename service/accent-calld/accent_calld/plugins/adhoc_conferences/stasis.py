...
from accent_calld.plugin_helpers.ari_ import Bridge, BridgeSnapshot, Channel
from accent_calld.plugins.calls.services import CallsService
...
class AdhocConferencesStasis:
    ...
    def on_channel_entered_bridge(self, channel, event):
        ...
        try:
            is_adhoc_conference_host = (
                channel.getChannelVar(variable='ACCENT_IS_ADHOC_CONFERENCE_HOST')['value']
                == 'true'
            )
        except ARINotFound:
            logger.error(
        ...
        if is_adhoc_conference_host:
            ...
            bridge_helper.global_variables.set('ACCENT_HOST_CHANNEL_ID', channel_id)
            bridge_helper.global_variables.set('ACCENT_HOST_USER_UUID', host_user_uuid)
            bridge_helper.global_variables.set(
                'ACCENT_HOST_TENANT_UUID', host_tenant_uuid
            )
    ...
    def on_channel_left_bridge(self, channel, event):
        ...
        try:
            host_user_uuid = bridge_helper.global_variables.get('ACCENT_HOST_USER_UUID')
        except KeyError:
            logger.error(
        ...
        if bridge_helper.has_lone_channel():
            ...
            try:
                adhoc_conference_host_channel_id = bridge_helper.global_variables.get(
                    variable='ACCENT_HOST_CHANNEL_ID'
                )
            except KeyError:
                logger.error(
    ...
    def on_bridge_destroyed(self, bridge, event):
        ...
        bridge_helper.global_variables.unset('ACCENT_HOST_CHANNEL_ID')
        bridge_helper.global_variables.unset('ACCENT_HOST_USER_UUID')
        bridge_helper.global_variables.unset('ACCENT_HOST_TENANT_UUID')
