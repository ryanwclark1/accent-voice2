...
from accent.caller_id import assemble_caller_id
from accent_amid_client import Client as AmidClient
from accent_confd_client import Client as ConfdClient
from ari.exceptions import ARINotFound

from accent_calld.ari_ import DEFAULT_APPLICATION_NAME, ARIClientProxy
...
class TransfersService:
    ...
    def originate_recipient(
        ...
        originate_variables['ACCENT_TRANSFER_ROLE'] = 'recipient'
        originate_variables['ACCENT_TRANSFER_ID'] = transfer_id
        originate_variables['CHANNEL(language)'] = initiator_channel.getChannelVar(
            variable='CHANNEL(language)'
        )['value']
        try:
            user_id = initiator_channel.getChannelVar(variable='ACCENT_USERID')['value']
            originate_variables['ACCENT_USERID'] = user_id  # Deprecated in 24.01
            originate_variables['ACCENT_USERID'] = user_id
        except ARINotFound:
            pass
        try:
            user_uuid = initiator_channel.getChannelVar(variable='ACCENT_USERUUID')[
                'value'
            ]
            originate_variables['ACCENT_USERUUID'] = user_uuid
            originate_variables['ACCENT_USERUUID'] = user_uuid  # Deprecated in 24.01
        except ARINotFound:
            pass
