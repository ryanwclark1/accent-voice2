...
from accent_calld.plugin_helpers.ari_ import Channel
from accent_calld.plugin_helpers.exceptions import BridgeNotFound
...
class TransferState:
    ...
    def _get_or_create_transfer_bridge(self):
        ...
        try:
            ...
            ari_helpers.set_bridge_variable(
                self._ari, bridge.id, 'ACCENT_TRANSFER_ID', self.transfer.id
            )
        return bridge
    ...
    def _set_initiator_variables(self):
        self._ari.channels.setChannelVar(
            ...
            variable='ACCENT_TRANSFER_ROLE',
            value='initiator',
        )
        self._ari.channels.setChannelVar(
            channelId=self.transfer.initiator_call,
            variable='ACCENT_TRANSFER_ID',
            value=self.transfer.id,
        )

    def _unset_initiator_variables(self):
        ari_helpers.unset_variable(
            self._ari, self._amid, self.transfer.initiator_call, 'ACCENT_TRANSFER_ID'
        )
        ari_helpers.unset_variable(
            self._ari, self._amid, self.transfer.initiator_call, 'ACCENT_TRANSFER_ROLE'
        )
    ...
    def _unset_transferred_variables(self):
        ari_helpers.unset_variable(
            self._ari, self._amid, self.transfer.transferred_call, 'ACCENT_TRANSFER_ID'
        )
        ari_helpers.unset_variable(
            self._ari, self._amid, self.transfer.transferred_call, 'ACCENT_TRANSFER_ROLE'
        )
    ...
    def _unset_recipient_variables(self):
        ari_helpers.unset_variable(
            self._ari, self._amid, self.transfer.recipient_call, 'ACCENT_TRANSFER_ID'
        )
        ari_helpers.unset_variable(
            self._ari, self._amid, self.transfer.recipient_call, 'ACCENT_TRANSFER_ROLE'
        )
...
class TransferStateAnswered(TransferState):
    ...
    @transition
    def complete(self):
        # NOTE: moving transferred channel into transfer bridge
