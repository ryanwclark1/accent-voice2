...
from accent_calld.plugin_helpers.ari_ import Channel as _ChannelHelper
...
class CallFormatter:
    ...
    def from_channel(self, channel, variables=None, node_uuid=None):
        ...
        if self._ari is not None:
            ...
            try:
                call.moh_uuid = (
                    channel.getChannelVar(variable='ACCENT_MOH_UUID').get('value') or None
                )
            except ARINotFound:
                call.moh_uuid = None

            try:
                call.user_uuid = channel.getChannelVar(variable='ACCENT_USERUUID').get(
                    'value'
                )
            except ARINotFound:
                call.user_uuid = None
            ...
            try:
                call.muted = (
                    channel.getChannelVar(variable='ACCENT_CALL_MUTED').get('value')
                    == '1'
                )
            except ARINotFound:
                call.muted = False
...
class _Snoop:
    bridge_name_tpl = 'accent-app-snoop-{}'
    _snooped_call_id_chan_var = 'ACCENT_SNOOPED_CALL_ID'
    _whisper_mode_chan_var = 'ACCENT_SNOOP_WHISPER_MODE'
