...
from .exceptions import AccentConfdUnreachable, NoSuchApplication, NoSuchMoh
...
# If this helper is used more than once, then it should go to the accent-confd-client
class ConfdIsReadyThread:
    ...
    def start(self):
        if self._started:
            raise Exception('Check when accent-confd is ready already started')
    ...
    def _run(self):
        while not self._should_stop.is_set():
            if self._is_ready():
                ...
                return
            logger.info(
                'accent-confd is not ready yet, retrying in %s seconds...',
                self._retry_time,
            )
