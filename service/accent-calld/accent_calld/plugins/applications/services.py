...
from accent_calld.plugin_helpers import ami, confd
from accent_calld.plugin_helpers.ari_ import Channel, set_channel_var_sync
...
class ApplicationService:
    ...
    def call_mute(self, application, call_id):
        try:
            ...
            set_channel_var_sync(channel, 'ACCENT_CALL_MUTED', '1')
    ...
    def call_unmute(self, application, call_id):
        try:
            channel = self._ari.channels.get(channelId=call_id)
            set_channel_var_sync(channel, 'ACCENT_CALL_MUTED', '')
    ...
    def start_user_outgoing_call(self, application, channel):
        set_channel_var_sync(channel, 'ACCENT_USER_OUTGOING_CALL', 'true')
    ...
    def call_progress_start(self, application, call_id):
        try:
            channel = self._ari.channels.get(channelId=call_id)
            channel.ring()
            set_channel_var_sync(channel, 'ACCENT_CALL_PROGRESS', '1')
        except ARINotFound:
            raise NoSuchCall(call_id)
    ...
    def call_progress_stop(self, application, call_id):
        try:
            channel = self._ari.channels.get(channelId=call_id)
            channel.ringStop()
            set_channel_var_sync(channel, 'ACCENT_CALL_PROGRESS', '')
        except ARINotFound:
            raise NoSuchCall(call_id)
    ...
    def originate(
        ...
        if autoanswer:
            variables['ACCENT_AUTO_ANSWER'] = '1'
    ...
    def start_call_hold(self, call_id):
        try:
            self._ari.channels.setChannelVar(
                channelId=call_id, variable='ACCENT_ON_HOLD', value='1'
            )
    ...
    def stop_call_hold(self, call_id):
        try:
            self._ari.channels.setChannelVar(
                channelId=call_id, variable='ACCENT_ON_HOLD', value=''
            )
