...
AUTO_ANSWER_VARIABLES = {
    ...
    # Polycom need Alert-Info: accent-autoanswer
...
class Channel:
    ...
    def user(self, default=None):
        if self.is_local():
            try:
                uuid = self._get_var('ACCENT_DEREFERENCED_USERUUID')
            except ARINotFound:
                return default
            return uuid

        try:
            uuid = self._get_var('ACCENT_USERUUID')
            return uuid
        except ARINotFound:
            return default
    ...
    def tenant_uuid(self, default=None):
        try:
            tenant_uuid = self._get_var('ACCENT_TENANT_UUID')
        except ARINotFound:
            return default
        return tenant_uuid
    ...
    def is_caller(self):
        try:
            user_outgoing_call = self._get_var('ACCENT_USER_OUTGOING_CALL')
            return user_outgoing_call == 'true'
        except ARINotFound:
            pass

        try:
            direction = self._get_var('ACCENT_CHANNEL_DIRECTION')
            return direction == 'to-accent'
        except ARINotFound:
            pass
    ...
    def is_in_stasis(self):
        try:
            self._ari.channels.setChannelVar(
                channelId=self.id, variable='ACCENT_TEST_STASIS'
            )
            return True
    ...
    def dialed_extension(self):
        try:
            return self._ari.channels.getChannelVar(
                channelId=self.id, variable='ACCENT_ENTRY_EXTEN'
            )['value']
        except ARINotFound:
            try:
                channel = self._ari.channels.get(channelId=self.id)
    ...
    def on_hold(self):
        try:
            on_hold = self._get_var('ACCENT_ON_HOLD')
            return on_hold == '1'
        except ARINotFound:
            return False
    ...
    def muted(self):
        try:
            muted = self._get_var('ACCENT_CALL_MUTED')
            return muted == '1'
        except ARINotFound:
            return False
    ...
    def parked(self):
        try:
            parked = self._get_var('ACCENT_CALL_PARKED')
            return parked == '1'
        except ARINotFound:
            return False
    ...
    def is_progress(self):
        try:
            progress = self._get_var('ACCENT_CALL_PROGRESS')
            return progress == '1'
        except ARINotFound:
            return False
    ...
    def line_id(self):
        try:
            return int(self._get_var('ACCENT_LINE_ID'))
        except ARINotFound:
            return
        except ValueError:
            logger.error(
                'Channel %s: Malformed ACCENT_LINE_ID=%s',
                self.id,
                self._get_var('ACCENT_LINE_ID'),
            )
            return
    ...
    def switchboard_timeout(self):
        try:
            timeout_str = self._get_var('ACCENT_SWITCHBOARD_TIMEOUT')
        except ARINotFound:
            return
        if not timeout_str:
            return
    ...
    def switchboard_noanswer_fallback_action(self):
        try:
            return self._get_var('ACCENT_SWITCHBOARD_FALLBACK_NOANSWER_ACTION')
        except ARINotFound:
            return
...
class Bridge:
    def __init__(self, bridge_id, ari):
        ...
        self.global_variables = GlobalVariableNameDecorator(
            ...
            f'ACCENT_BRIDGE_{self.id}_VARIABLE_{{}}',
        )
