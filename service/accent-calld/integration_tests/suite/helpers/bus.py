...
from .constants import BUS_EXCHANGE_ACCENT, VALID_TENANT
...
class BusClient(bus_helper.BusClient):
    def send_event(self, event, headers=None):
        with Connection(self._url) as connection:
            producer = Producer(
                connection, exchange=BUS_EXCHANGE_ACCENT, auto_declare=True
            )
            producer.publish(
        ...
        self.send_event(
            {
                'data': {
                    ...
                    'ChanVariable': {
                        'ACCENT_USERUUID': 'my-uuid',
                        'ACCENT_ENTRY_EXTEN': entry_exten if entry_exten else '*10',
