...
from accent_test_helpers.bus import BusMessageAccumulator
...
class TestTransfers(RealAsteriskIntegrationTest):
    ...
    def assert_transfer_is_answered(
        ...
        assert_that(
            transferred_channel_id,
            self.c.has_variable('ACCENT_TRANSFER_ID', transfer_id),
            'variable not set',
        )
        assert_that(
            transferred_channel_id,
            self.c.has_variable('ACCENT_TRANSFER_ROLE', 'transferred'),
            'variable not set',
        )
        ...
        assert_that(
            initiator_channel_id,
            self.c.has_variable('ACCENT_TRANSFER_ROLE', 'initiator'),
            'variable not set',
        )
        ...
        assert_that(
            recipient_channel_id,
            self.c.has_variable('ACCENT_TRANSFER_ROLE', 'recipient'),
            'variable not set',
        )
    ...
    def assert_transfer_is_cancelled(
        ...
        assert_that(
            transferred_channel_id,
            self.c.has_variable('ACCENT_TRANSFER_ROLE', ''),
            'variable not unset',
        )
        assert_that(
            transferred_channel_id,
            self.c.has_variable('ACCENT_TRANSFER_ID', ''),
            'variable not unset',
        )
    ...
    def set_initiator_channel(self, channel_id, initiator_uuid):
        self.ari.channels.setChannelVar(
            channelId=channel_id, variable='ACCENT_USERUUID', value=initiator_uuid
        )
...
class TestUserListTransfers(TestTransfers):
    ...
    def test_list_with_broken_index(self):
        ...
        transfers_index = json.loads(
            self.ari.asterisk.getGlobalVar(variable='ACCENT_TRANSFERS_INDEX')['value']
        )
        transfers_index.insert(0, 'invalid')
        transfers_index = self.ari.asterisk.setGlobalVar(
            variable='ACCENT_TRANSFERS_INDEX', value=json.dumps(transfers_index)
        )
...
class TestCreateTransfer(TestTransfers):
    ...
    def test_that_variables_are_applied_to_the_recipient_channel(self):
        ...
        self.ari.channels.setChannelVar(
            channelId=initiator_channel_id, variable='ACCENT_USERID', value='my-userid'
        )
        self.ari.channels.setChannelVar(
            channelId=initiator_channel_id,
            variable='ACCENT_USERUUID',
            value='my-useruuid',
        )
        ...
        expected = {
            ...
            'ACCENT_USERID': 'my-userid',
            'ACCENT_USERUUID': 'my-useruuid',
        }
    ...
    def test_that_unset_inherited_variables_do_not_block_transfer(self):
        ...
        assert_that(
            calling(self.ari.channels.getChannelVar).with_args(
                channelId=recipient_channel_id, variable='ACCENT_USERID'
            ),
            raises(ARINotFound),
        )
        # we can't check for missing ACCENT_USERUUID because initiator must have ACCENT_USERUUID for transfers to work
...
class TestNoAmid(TestTransfers):
    ...
    def test_given_no_amid_when_create_transfer_from_non_stasis_then_503(self):
        ...
        assert_that(response.status_code, equal_to(503))
        assert_that(response.json(), has_entry('message', contains_string('accent-amid')))
...
class TestInitialisation(TestTransfers):
    ...
    def test_given_started_transfer_when_accent_calld_restarts_then_transfer_may_continue(
        self,
    ):
        (
            transferred_channel_id,
            ...
            recipient_channel_id,
            transfer_id,
        ) = self.given_answered_transfer(variables={'ACCENT_TENANT_UUID': VALID_TENANT})
