...
from accent_provd.devices import ident
from accent_provd.devices.ident import (
    AddDeviceRetriever,
    DHCPRequest,
...
from accent_provd.servers.tftp import TFTPRequest
...
class TestAddDeviceRetriever(unittest.TestCase):
    ...
    @patch('accent_provd.devices.ident.log_security_msg')
...
class TestRequestHelper(unittest.TestCase):
    ...
    def test_update_device_on_device_change_and_remote_state_update(
        ...
        dev_updater = self._new_dev_updater_mock({'vendor': 'accent'})
        device: dict[str, Any] = {
            'id': 'a',
            'configured': True,
            'plugin': 'foo',
    ...
    def test_get_plugin_id_ok(self) -> None:
        device = {'id': 'a', 'plugin': 'accent-foo'}
        pg_id = self.helper.get_plugin_id(device)
        assert_that(pg_id, equal_to('accent-foo'))
