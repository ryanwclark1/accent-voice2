...
import accent_provd.config
import accent_provd.localization
import accent_provd.synchronize
from accent_provd import security, status
from accent_provd.app import ProvisioningApplication
from accent_provd.devices import ident, pgasso
from accent_provd.devices.config import ConfigCollection
from accent_provd.devices.device import DeviceCollection
from accent_provd.persist.json_backend import JsonDatabaseFactory
from accent_provd.rest.api.resource import ResponseFile
from accent_provd.rest.server import auth
from accent_provd.rest.server.server import new_authenticated_server_resource
from accent_provd.servers.http_site import AuthResource, Site
...
LOG_FILE_NAME = '/var/log/accent-provd.log'
API_VERSION = b'0.2'
...
class SynchronizeService(Service):
    ...
    def _new_sync_service_asterisk_ami(
        self,
    ) -> accent_provd.synchronize.AsteriskAMISynchronizeService:
        amid_client = accent_provd.synchronize.get_AMID_client(**self._config['amid'])
        return accent_provd.synchronize.AsteriskAMISynchronizeService(amid_client)
    ...
    ) -> accent_provd.synchronize.AsteriskAMISynchronizeService | None:
    ...
    def startService(self) -> None:
        ...
        if sync_service is not None:
            accent_provd.synchronize.register_sync_service(sync_service)
        Service.startService(self)

    def stopService(self) -> None:
        Service.stopService(self)
        accent_provd.synchronize.unregister_sync_service()


class LocalizationService(Service):
    def startService(self) -> None:
        l10n_service = accent_provd.localization.LocalizationService()
        accent_provd.localization.register_localization_service(l10n_service)
        Service.startService(self)

    def stopService(self) -> None:
        Service.stopService(self)
        accent_provd.localization.unregister_localization_service()
...
class ProvdBusConsumer(BusConsumer):
    ...
    def from_config(cls, bus_config: BusConfigDict) -> ProvdBusConsumer:
        return cls(name='accent-provd', **bus_config)
...
class ProvisioningServiceMaker:
    tapname = 'accent-provd'
    description = 'A provisioning server.'
    options = accent_provd.config.Options
    ...
    def _read_config(self, options: Options) -> ProvdConfigDict:
        ...
        return accent_provd.config.get_config(options)
