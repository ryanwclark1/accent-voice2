...
from accent_webhookd.auth import master_tenant_uuid
...
class SubscriptionBusEventHandler:
    ...
    def _build_headers(subscription: Subscription) -> SubscriptionHeaders:
        ...
        accent_uuid = subscription.events_accent_uuid
        ...
        if accent_uuid:
            headers.update(origin_uuid=str(accent_uuid))
        return headers
    ...
    def _callback(self, subscription: Subscription, payload: dict[str, Any]) -> None:
        ...
        try:
            ...
            # NOTE: That's not perfect in real life, because if celery
            ...
            # NOTE: If we have a programming error, don't retry forever
            raise
