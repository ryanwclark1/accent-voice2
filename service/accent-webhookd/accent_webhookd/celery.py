...
if TYPE_CHECKING:
    from accent_webhookd.types import WebhookdConfigDict
...
def spawn_workers(config: WebhookdConfigDict) -> multiprocessing.Process:
    ...
    argv = [
        'worker',
        # NOTE: setproctitle must be installed to have the celery
...
# NOTE: This defeats the point of using stevedore, but the celery worker process
...
import accent_webhookd.plugins.mobile.celery_tasks  # noqa
import accent_webhookd.plugins.subscription.celery_tasks  # noqa
