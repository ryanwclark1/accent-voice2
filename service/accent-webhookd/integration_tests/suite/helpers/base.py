...
ACCENT_UUID = '00000000-0000-4000-8000-00003eb8004d'
...
class BaseIntegrationTest(AssetLaunchingTestCase):
    ...
    def setUpClass(cls):
        ...
        if cls.asset in ("proxy", "base"):
            cls.configured_accent_auth()
            cls.docker_exec(['accent-webhookd-init-amqp', '--host', 'rabbitmq'])
    ...
    @classmethod
    def configured_accent_auth(cls):
        auth = cls.make_auth()
    ...
    def auth_stopped(self):
        ...
        auth = self.make_auth()
        until.true(
            auth.is_up, timeout=START_TIMEOUT, message='accent-auth did not come back up'
        )
    ...
    def webhookd_with_config(cls, config):
        ...
        config_file = f'/etc/accent-webhookd/conf.d/10-{name}.yml'
