...
ACCENT_UUID_EXTENDED = 'cd030e68-ace9-4ad4-bc4e-13c8dec67898-suffix'
OTHER_ACCENT_UUID = 'cae3a160-6b01-4746-acd2-8588a768e54c'
TEST_SUBSCRIPTION = {
    'name': 'test',
    'service': 'http',
...
TEST_SUBSCRIPTION_FILTER_ACCENT_UUID = {
    'name': 'test',
    'service': 'http',
    'config': {'url': 'http://third-party-http:1080/test', 'method': 'get'},
    'events': ['trigger'],
    'events_accent_uuid': ACCENT_UUID,
}
...
class TestHTTPCallback(BaseIntegrationTest):
    ...
    @subscription(TEST_SUBSCRIPTION_FILTER_ACCENT_UUID)
    def test_given_http_subscription_with_accent_uuid_when_bus_events_only_callback_when_accent_uuid_match(  # noqa: E501
        self, subscription
    ):
        # Non-matching events
        self.bus.publish(trigger_event(), routing_key=SOME_ROUTING_KEY)
        self.bus.publish(
            trigger_event(),
            routing_key=SOME_ROUTING_KEY,
            headers={'name': TRIGGER_EVENT_NAME, 'origin_uuid': OTHER_ACCENT_UUID},
        )
        self.bus.publish(
            trigger_event(),
            routing_key=SOME_ROUTING_KEY,
            headers={'name': TRIGGER_EVENT_NAME, 'origin_uuid': ACCENT_UUID_EXTENDED},
        )
        # Matching event
        self.bus.publish(
            trigger_event(),
            routing_key=SOME_ROUTING_KEY,
            headers={'name': TRIGGER_EVENT_NAME, 'origin_uuid': ACCENT_UUID},
        )
        until.assert_(
            self.make_third_party_verify_callback(
                request={'method': 'GET', 'path': '/test'}, count=1, exact=True
