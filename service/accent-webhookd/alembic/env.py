...
def get_url():
    ...
    # environment variable when the DB is not hosted on the same host as accent-webhookd.
    # When building the docker image for the database for example.
    from accent_webhookd.config import load_config

    accent_config = load_config('')
    return accent_config.get('db_uri')
