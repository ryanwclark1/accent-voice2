...
from accent_auth.database.queries.user import UserDAO
...
class TestSamlService(SAMLIntegrationTest):
    ...
    def setup(self, page: Page) -> None:
        self.page = page
        if 'ACCENT_SAML_CONFIG_FILE' in os.environ:
            conf_file = pathlib.Path(os.environ['ACCENT_SAML_CONFIG_FILE'])
        else:
            conf_file = (
        ...
        if 'ACCENT_SAML_LOGIN' in os.environ:
            self.login = RedactedValue(os.environ['ACCENT_SAML_LOGIN'])
        if 'ACCENT_SAML_PASSWORD' in os.environ:
            self.password = RedactedValue(os.environ['ACCENT_SAML_PASSWORD'])
    ...
    def _configure_saml(self, domain_name: str) -> None:
        ...
        acs_url: str = acs_url_template.replace('{{STACK_URL}}', 'stack.accentvoice.local')
        domain_uuid: str = self.client.tenants.get_domains(self.tenant['uuid'])[
            'items'
        ][0]['uuid']
        with open('./assets/var/lib/accent-auth/saml/saml.xml') as f:
            metadata: str = f.read()
    ...
    def _accept_self_signed_certificate_on_stack(self) -> None:
        self.page.goto("https://stack.accentvoice.local/api/auth/0.1/tokens")

    def _reload_saml_config(self) -> None:
        self.restart_auth()

    def _click_login(self, page) -> None:
        page.goto("https://app.accentvoice.local/")
    ...
    def test_login_logout(self) -> None:
        ...
        expect(self.page).to_have_url(
            'https://app.accentvoice.local/postacs.html?logged_out=true', timeout=20000
        )
