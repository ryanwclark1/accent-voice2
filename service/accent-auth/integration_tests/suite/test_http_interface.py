...
import requests
from accent_auth_client import exceptions
...
from accent_auth.database import helpers, models
...
class TestCore(base.APIIntegrationTest):
    ...
    def test_that_the_accent_uuid_is_included_in_POST_response(self):
        response = self._post_token('foo', 'bar')

        accent_uuid = response['accent_uuid']
        assert_that(accent_uuid, equal_to('the-predefined-accent-uuid'))
    ...
    def test_backends(self):
        ...
        backends = ['broken_init', 'broken_verify_password', 'accent_user', 'ldap_user']
    ...
    def test_that_get_returns_the_accent_uuid_in_the_response(self):
        token = self._post_token('foo', 'bar')['token']

        response = self.client.token.get(token)

        assert_that(response, has_entries(accent_uuid='the-predefined-accent-uuid'))
    ...
    def test_that_get_returns_the_pbx_user_uuid(self):
        ...
        assert_that(response, has_key('accent_user_uuid'))  # Compatibility
