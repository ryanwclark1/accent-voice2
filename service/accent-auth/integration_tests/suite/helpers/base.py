...
from accent_auth.database import helpers, queries
from accent_auth.database.queries import (
    group,
    ldap_config,
    policy,
    refresh_token,
...
logging.getLogger('accent_auth').setLevel(logging.WARNING)
...
class BaseAssetLaunchingTestCase(AssetLaunchingTestCase):
    ...
    def make_db_client(cls):
        ...
        return Database(db_uri, db='accent-auth')
    ...
    def make_bus_client(cls):
        ...
        upstream = Exchange('accent', 'topic')
        ...
        bus.downstream_exchange_declare('accent-headers', 'headers', upstream)
        return bus
    ...
    def restart_auth(cls):
        ...
        logging.getLogger('accent_test_helpers').setLevel(logging.INFO)
        until.return_(auth.status.check, timeout=30)
        logging.getLogger('accent_test_helpers').setLevel(logging.DEBUG)
...
class BaseIntegrationTest(unittest.TestCase):
    ...
    def profiling_enabled(self):
        ...
        container_directory = '/tmp/accent-profiling'
        random_strings = "".join(
            random.choice(string.ascii_lowercase) for _ in range(8)
        )
        local_directory = os.getenv(
            'ACCENT_TEST_PROFILING_DIR', f'/tmp/accent-profiling-{random_strings}'
        )
