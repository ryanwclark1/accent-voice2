        ...
        def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                INSERT INTO "auth_group_policy" VALUES (
                    ...
                    (SELECT uuid from auth_policy WHERE name = 'accent-all-users-policy')
                );
                INSERT INTO "auth_group_policy" VALUES (
                    (SELECT uuid FROM auth_group WHERE name = 'bulk-tenants-{index:0=5}-2'),
                    (SELECT uuid from auth_policy WHERE name = 'accent_default_admin_policy')
                );'''
                lines = (
...
def token(**token_args):
    def decorator(decorated):
        @wraps(decorated)
        def wrapper(self, *args, **kwargs):
            token_args.setdefault('backend', 'accent_user')
    ...
    def decorator(decorated):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            ldap_config_args.setdefault(
                'user_base_dn', 'ou=people,dc=accentvoice,dc=org'
            )
    ...
    def decorator(decorated):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            config_args.setdefault(
                'data',
                {
                    'acs_url': 'https://stack.accentvoice.local/api/0.1/saml/acs',
                    'entity_id': 'accentvoice.io',
