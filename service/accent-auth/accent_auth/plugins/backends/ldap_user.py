...
class LDAPUser(BaseAuthenticationBackend):
    ...
    def verify_password(self, username, password, args):
        ...
        accent_ldap = _AccentLDAP(config)
        user_email = None
        try:
            accent_ldap.connect()
            if bind_dn and bind_password:
                if accent_ldap.perform_bind(bind_dn, bind_password):
            ...
            if not user_dn or not accent_ldap.perform_bind(user_dn, password):
                logger.debug(
            ...
            if not user_email:
                user_email = self._get_user_ldap_email(
                    accent_ldap, user_dn, user_email_attribute
                )
            if not user_email:
                logger.debug(
            ...
            logger.warning('LDAP : SERVER not responding on "%s"', accent_ldap.uri)
            return False
    ...
    def _get_user_ldap_email(self, accent_ldap, user_dn, user_email_attribute):
        _, obj = accent_ldap.perform_search(
    ...
    def _perform_search_attributes(
        ...
        dn, obj = accent_ldap.perform_search(
            user_base_dn,
            ldap.SCOPE_SUBTREE,
            filterstr=filterstr,
