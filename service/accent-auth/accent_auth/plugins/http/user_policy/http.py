...
import marshmallow
from accent.auth_verifier import AccessCheck, Unauthorized
from flask import request

from accent_auth import exceptions, http, schemas
from accent_auth.flask_helpers import Token, get_tenant_uuids
from accent_auth.plugins.http.policies.schemas import policy_full_schema
...
class _UserPolicy(_BaseUserPolicyResource):
    def _delete(self, user_uuid, policy_uuid, tenant_uuids):
        # FIXME: Currently not multi-tenant
        # self.policy_service.assert_user_in_subtenant(tenant_uuids, policy_uuid)
        # FIXME: Dissociation should be done on the same tenant
    ...
    def _put(self, user_uuid, policy_uuid, tenant_uuids):
        ...
        # FIXME: Currently not multi-tenant
        # self.policy_service.assert_user_in_subtenant(tenant_uuids, policy_uuid)
        # FIXME: Association should be done on the same tenant
        # self.policy_service.assert_policy_in_subtenant(tenant_uuids, policy_uuid)

        access_check = AccessCheck(token.auth_id, token.session_uuid, token.acl)
        # FIXME: Policy should be accessible by the tenant
        # policy = self.policy_service.get(policy_uuid, tenant_uuids)
