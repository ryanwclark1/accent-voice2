...
class SetupService:
    ...
    def get_engine_token(self, engine_password):
        ...
        try:
            token_data = auth.token.new('accent_user', expiration=60)
    ...
    def get_nestbox_token(
        ...
        try:
            token_data = auth.token.new('accent_user', expiration=600)
        except HTTPError as e:
            raise SetupError(
                message='Failed to create authorization token',
                error_id='setup-token-failed',
    ...
    def post_confd_wizard(self, language, password, license_accepted):
        ...
        if len(discover['nameservers']) > 3:
            # NOTE: Should parse confd error when it will return json errors
            raise SetupError(
    ...
    def register_instance(
        ...
        # accent-deployd-client was just installed by accent-sysconfd, at the
        # request of accent-setupd, thus we need a lazy import
        from accent_deployd_client import Client as DeploydClient
    ...
    def inject_nestbox_config(
        ...
        generated_config_file = "/usr/share/accent-setupd/50-accent-plugin-nestbox.yml"
        config = {
            "nestbox": {
        ...
        data = {
            'accent-auth': 'restart',
            'accent-webhookd': 'restart',
        }
        try:
            response = session.post(url, json=data)
        except requests.RequestsException as e:
            raise SetupError(
                'accent-sysconfd connection error',
                error_id='accent-sysconfd-connection-error',
                details={'original_error': e},
            )
        if response.status_code != 200:
            raise SetupError(
                'accent-sysconfd failure',
                error_id='accent-sysconfd-failure',
