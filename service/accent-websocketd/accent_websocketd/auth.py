...
# Temporary fix for accent_auth_client import error
...
class AsyncAuthClient:
    ...
    async def get_token(self, token_id):
        logger.debug('getting token from accent-auth')
        ...
        try:
            ...
            # there's currently no clean way with accent_auth_client to know if the
            # error was caused because the token is unauthorized, or unknown
            # or something else
            raise AuthenticationError(e)

    async def is_valid_token(self, token_id, acl=_ACL):
        logger.debug('checking token validity from accent-auth')
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
