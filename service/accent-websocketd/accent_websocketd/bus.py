...
import aioamqp
from accent.auth_verifier import AccessCheck
...
class BusConsumer:
    ...
    def _generate_name(*parts: str) -> str:
        return '.'.join(['accent-websocketd', *parts])
...
class BusService:
    ...
    async def initialize_exchanges(self):
        ...
        # Migration <22.13
        # Upgrading from a previous version will keep `accent-websocketd` exchange
        ...
        async def remove_deprecated(config: dict, channel: Channel):
            if config['bus']['exchange_name'] != 'accent-websocketd':
                await channel.exchange_delete('accent-websocketd', if_unused=True)
                logger.info('migration: removed legacy `accent-websocketd` exchange...')

        logger.info('configuring RabbitMQ for accent-websocketd...')
        connection = self._connection_pool.get_connection()
        try:
