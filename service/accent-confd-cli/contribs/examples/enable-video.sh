...
if ! command -v accent-auth-cli &> /dev/null; then
    echo "accent-auth-cli is required to execute this script"
    exit 2
fi

if ! command -v accent-confd-cli &> /dev/null; then
    echo "accent-confd-cli is required to execute this script"
    exit 2
fi
...
if [[ -v AUTH_USERNAME ]] && [ -n "${AUTH_USERNAME}" ]; then
    ...
    TOKEN=$(accent-auth-cli token create --auth-username="${AUTH_USERNAME}" --auth-password="${AUTH_PASSWORD}")
else
    TOKEN=$(accent-auth-cli token create)
fi

# Delete token when done
delete_token() {
    accent-auth-cli token revoke "${TOKEN}"
}
...
done < <(accent-confd-cli --token "${TOKEN}" endpoint sip template list --tenant "${TENANT_UUID}" -fvalue)
...
endpoints_missing_template_webrtc_video() {
    ...
    comm -23 \
        <(accent-confd-cli --token "${TOKEN}" endpoint sip list --template "${WEBRTC_TEMPLATE_UUID}" --tenant "${TENANT_UUID}" -f value -c uuid --sort-column uuid) \
        <(accent-confd-cli --token "${TOKEN}" endpoint sip list --template "${WEBRTC_VIDEO_TEMPLATE_UUID}" --tenant "${TENANT_UUID}" -f value -c uuid --sort-column uuid)
}

echo -n 'Updating WebRTC endpoints'
for endpoint_uuid in $(endpoints_missing_template_webrtc_video); do
    echo -n '.'
    accent-confd-cli --token "${TOKEN}" endpoint sip add --template "${WEBRTC_VIDEO_TEMPLATE_UUID}" "${endpoint_uuid}"
done
echo
echo 'done'
