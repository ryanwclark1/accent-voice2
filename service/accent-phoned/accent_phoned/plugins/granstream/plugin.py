...
from .services import GrandstreamService


class Plugin:
    # NOTE: this vendor-ending URL will be deprecated
    ...
    def load(self, dependencies):
        ...
        service = GrandstreamService(amid_client, confd_client)
    ...
    def _add_resources(self, api, class_kwargs):
        api.add_resource(
            Lookup,
            self.lookup_url,
            endpoint='grandstream_lookup',
            resource_class_kwargs=class_kwargs,
        )
        api.add_resource(
            Lookup,
            self.directories_lookup_url,
            endpoint='grandstream_directories_lookup',
            resource_class_kwargs=class_kwargs,
        )
    ...
    def _add_user_service_resources(self, api, class_kwargs):
        api.add_resource(
            ...
            endpoint='grandstream_user_service_dnd_enable',
            resource_class_kwargs=class_kwargs,
        )
        api.add_resource(
            DNDUserServiceDisable,
            self.user_service_dnd_disable_url,
            endpoint='grandstream_user_service_dnd_disable',
            resource_class_kwargs=class_kwargs,
        )
