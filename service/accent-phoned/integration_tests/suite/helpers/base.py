...
from accent_test_helpers.auth import MockCredentials, MockUserToken
from accent_test_helpers.wait_strategy import NoWaitStrategy
...
ACCENT_UUID = 'de8f8614-9f42-4fb6-be5a-f9a8d4f8674b'
...
class BasePhonedIntegrationTest(AssetLaunchingTestCase):
    ...
    def configure_accent_auth(cls):
        key_file = parse_config_file(
            os.path.join(cls.assets_root, 'auth_keys', 'accent-phoned-key.yml')
        )
    ...
    @classmethod
    def get_menu_result(self, profile, vendor, accent_user_uuid=None):
        url = 'http://127.0.0.1:{port}/0.1/directories/menu/{profile}/{vendor}'
        params = {'accent_user_uuid': accent_user_uuid}
        port = self.service_port(9498, 'phoned')
        result = requests.get(
    ...
    @classmethod
    def get_ssl_menu_result(self, profile, vendor, accent_user_uuid=None):
        params = {'accent_user_uuid': accent_user_uuid}
        port = self.service_port(9499, 'phoned')
    ...
    @classmethod
    def get_input_result(self, profile, vendor, accent_user_uuid=None):
        params = {'accent_user_uuid': accent_user_uuid}
        port = self.service_port(9498, 'phoned')
    ...
    @classmethod
    def get_ssl_input_result(self, profile, vendor, accent_user_uuid=None):
        params = {'accent_user_uuid': accent_user_uuid}
        port = self.service_port(9499, 'phoned')
    ...
    def get_lookup_result(
        self, profile, vendor, accent_user_uuid=None, term=None, headers=None
    ):
        params = {'accent_user_uuid': accent_user_uuid, 'term': term}
        port = self.service_port(9498, 'phoned')
    ...
    def get_lookup_gigaset_result(
        self, profile, accent_user_uuid=None, term=None, headers=None, **kwargs
    ):
        params = {}
        if term:
        ...
        port = self.service_port(9498, 'phoned')
        url = 'http://127.0.0.1:{port}/0.1/directories/lookup/{profile}/gigaset/{accent_user_uuid}'
        result = requests.get(
            url.format(port=port, profile=profile, accent_user_uuid=accent_user_uuid),
            params=params,
            headers=headers,
        )
        return result
    ...
    def get_ssl_lookup_gigaset_result(
        self, profile, accent_user_uuid=None, term=None, **kwargs
    ):
        params = {}
        if term:
            params['set_first'] = term
        if kwargs:
            params.update(**kwargs)
        port = self.service_port(9499, 'phoned')
        url = 'https://127.0.0.1:{port}/0.1/directories/lookup/{profile}/gigaset/{accent_user_uuid}'
        result = requests.get(
            url.format(port=port, profile=profile, accent_user_uuid=accent_user_uuid),
            params=params,
            verify=False,
