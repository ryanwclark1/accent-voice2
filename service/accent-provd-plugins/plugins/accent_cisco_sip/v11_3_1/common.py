...
logger = logging.getLogger('plugins.accent-cisco-sip')
...
class BaseCiscoSipPlugin(StandardPlugin):
    ...
    _DIRECTORY_NAME = {
        'en_US': 'Accent Directory',
        'fr_FR': 'Répertoire Accent',
    }
    ...
    def _format_proxy(self, raw_config, line, line_no):
        ...
        if backup_proxy_ip:
            proxy_value = (
                f'accent_proxies{line_no}:SRV={proxy_ip}:{proxy_port}:p=0|'
    ...
    def _add_accent_phonebook_url(self, raw_config):
        plugins.add_accent_phonebook_url(raw_config, 'cisco')
