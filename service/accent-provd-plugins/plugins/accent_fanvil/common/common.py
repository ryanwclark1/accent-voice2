...
try:
    from accent_provd import plugins, synchronize, tzinform
    from accent_provd.devices.config import RawConfigError
    from accent_provd.devices.ident import RequestType
...
logger = logging.getLogger('plugin.accent-fanvil')
...
class BaseFanvilPlugin(StandardPlugin):
    ...
    def _add_accent_phoned_user_service_url(self, raw_config: dict[str, Any], service):
        if hasattr(plugins, 'add_accent_phoned_user_service_url'):
            plugins.add_accent_phoned_user_service_url(raw_config, 'fanvil', service)
    ...
    def configure(self, device, raw_config: dict[str, Any]) -> None:
        ...
        self._add_accent_phoned_user_service_url(raw_config, 'dnd')
    ...
    def _add_phonebook_url(self, raw_config: dict[str, Any]) -> None:
        if (
            hasattr(plugins, 'add_accent_phonebook_url')
            and raw_config.get('config_version', 0) >= 1
        ):
            plugins.add_accent_phonebook_url(
    ...
    def _add_phonebook_url_v2(self, raw_config: dict[str, Any]) -> None:
        if (
            ...
            original_phonebook_url = raw_config.get('XX_accent_phonebook_url')
            ...
            fixed_url = raw_config['XX_accent_phonebook_url'].replace('&', '&amp;')

            raw_config['XX_accent_phonebook_url_v2'] = fixed_url
            raw_config['XX_accent_phonebook_url'] = original_phonebook_url
