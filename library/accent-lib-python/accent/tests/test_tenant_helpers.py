...
from accent.auth_verifier import AuthServerUnreachable, InvalidTokenAPIException

from ..tenant_helpers import InvalidTenant, Tenant, Token, UnauthorizedTenant


class TestTenantAutodetect(TestCase):
    @patch('accent.tenant_helpers.Token')
    @patch('accent.flask.headers.request', spec={})
    ...
    def test_given_token_and_tenant_when_autodetect_then_return_given_tenant(
        ...
        Token.from_headers = Mock()
        Token.from_headers.return_value = token
        request.headers = {'X-Auth-Token': 'token', 'Accent-Tenant': tenant}
...
class TestTenantFromHeaders(TestCase):
    ...
    def test_given_tenant_when_from_headers_then_return_tenant(self, request):
        tenant = 'tenant'
        request.headers = {'Accent-Tenant': tenant}
...
class TestTenantFromQuery(TestCase):
    @patch('accent.tenant_helpers.request', spec={})
...
class TestTokenFromHeaders(TestCase):
    @patch('accent.tenant_helpers.extract_token_id_from_header')
