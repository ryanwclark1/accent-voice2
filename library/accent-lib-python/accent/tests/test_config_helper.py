...
ACCENT_UUID = '08c56466-8f29-45c7-9856-92bf1ba89b82'
...
class TestGetAccentUUID(unittest.TestCase):
    @patch('accent.config_helper.os.getenv', return_value=False)
    def test_given_no_uuid_then_raise_error(self, getenv):
        assert_that(calling(get_accent_uuid).with_args(Mock()), raises(UUIDNotFound))

    @patch('accent.config_helper.os.getenv', return_value=ACCENT_UUID)
    def test_given_uuid_then_return_uuid(self, getenv):
        result = get_accent_uuid(Mock())

        assert_that(result, equal_to(ACCENT_UUID))


class TestSetAccentUUID(unittest.TestCase):
    @patch('accent.config_helper.os.getenv', return_value=False)
    def test_given_no_uuid_then_raise_error(self, getenv):
        assert_that(calling(set_accent_uuid).with_args({}, Mock()), raises(UUIDNotFound))

    @patch('accent.config_helper.os.getenv', return_value=ACCENT_UUID)
    def test_given_uuid_then_set_uuid(self, getenv):
        config: dict[str, str] = {}
        set_accent_uuid(config, Mock())

        assert_that(config, has_entry('uuid', ACCENT_UUID))
