...
from accent.http_helpers import (
    BodyFormatter,
    LazyHeaderFormatter,
    log_request,
...
class TestLogRequest(unittest.TestCase):
    @patch("accent.http_helpers.g", spec={})
    def test_log_request(self, g):
        ...
        with (
            patch("accent.http_helpers.request", mock_request),
            patch("accent.http_helpers.current_app", Mock()) as mock_current_app,
        ):
