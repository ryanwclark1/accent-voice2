...
2. authenticate to it by providing a accent-auth token that has the `websocketd` ACL. If you don't
    know how to obtain a accent-auth token from your Accent, consult the
    [documentation on accent-auth](/uc-doc/system/accent-auth).
...
the WebSocket server is the same as the one used by the Accent web interface and the REST APIs. By
...
first visiting the `https://<accent-ip>/api/websocketd/` URL and adding a security exception. Other solutions to
...
The WebSocket service is provided by `accent-websocketd`, and its behaviour can be configured via its
[configuration files](/uc-doc/system/configuration_files) located under the
<span data-role="file">/etc/accent-websocketd</span> directory. After modifying the configuration
files, you need to restart `accent-websocketd` with `systemctl restart accent-websocketd`.
...
Authentication is done by passing a accent-auth token ID in the `token` query parameter.
...
Clients connected to `accent-websocketd` only receive events that they are authorized to receive. For
example, a client connected with a token obtained from the `accent_user` authentication backend will
_not_ receive call events of other users.

When a message is received from the bus by `accent-websocketd`, it extracts the ACL from the
...
information on the required ACL of events which are available by default on Accent.
...
with the same name is published on the "accent" exchange of the bus, the server forwards the message
...
See the [Events](/uc-doc/api_sdk/message_bus#bus-events) section for more
information on the events which are available by default on Accent.
