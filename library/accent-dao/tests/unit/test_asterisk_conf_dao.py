...
class TestSCCPLineSettingDAO(DAOTestCase, PickupHelperMixin):
    ...
    @patch('accent_dao.asterisk_conf_dao.find_pickup_members')
...
class TestAsteriskConfDAO(DAOTestCase, PickupHelperMixin):
    ...
    def test_find_exten_accentfeatures_setting(self):
        context = self.add_context()
        ...
        feature_extensions = asterisk_conf_dao.find_exten_accentfeatures_setting()
...
class TestFindSipMeetingGuestsSettings(BaseFindSIPSettings):
    ...
    def test_that_the_accent_caller_id_var_is_set(self):
        caller_id = '"Foo" <123>'
        endpoint = self.add_endpoint_sip(
            ...
            contains_exactly(
                has_entries(
                    endpoint_section_options=has_items(
                        contains_exactly(
                            'set_var',
                            f'ACCENT_ORIGINAL_CALLER_ID={caller_id}',
                        ),
                    ),
                )
            ),
    ...
    def test_that_the_tenant_uuid_var_is_set(self):
        ...
        assert_that(
            ...
            contains_exactly(
                has_entries(
                    endpoint_section_options=has_items(
                        ['set_var', f'__ACCENT_TENANT_UUID={endpoint.tenant_uuid}'],
                    ),
                )
            ),
        )
            ...
            contains_exactly(
                has_entries(
                    endpoint_section_options=contains_inanyorder(
                        ...
                        contains_exactly('set_var', 'ACCENT_CHANNEL_DIRECTION=from-accent'),
                        contains_exactly('set_var', f'ACCENT_MEETING_UUID={meeting.uuid}'),
                        contains_exactly('set_var', f'ACCENT_MEETING_NAME={meeting.name}'),
    ...
    def test_that_the_application_is_used(self):
        ...
        assert_that(
            ...
            contains_exactly(
                has_entries(
                    endpoint_section_options=has_items(
                        contains_exactly('context', f'stasis-accent-app-{application.uuid}'),
    ...
    def test_that_the_user_id_var_is_set(self):
        ...
        assert_that(
            ...
            contains_exactly(
                has_entries(
                    endpoint_section_options=has_items(
                        ['set_var', f'ACCENT_USERID={user.id}'],
                    ),
                )
            ),
        )
            ...
            contains_exactly(
                has_entries(
                    endpoint_section_options=has_items(
                        ...
                        # ['set_var', f'ACCENT_USER_UUID={user.uuid}'],
                    ),
                )
            ),
        )
    ...
    def test_that_the_line_id_var_is_set(self):
        ...
        assert_that(
            ...
            contains_exactly(
                has_entries(
                    endpoint_section_options=has_items(
                        ['set_var', f'ACCENT_LINE_ID={line.id}'],
                    ),
                )
            ),
        )
            ...
            contains_exactly(
                has_entries(
                    endpoint_section_options=contains_inanyorder(
                        ...
                        contains_exactly('set_var', 'ACCENT_CHANNEL_DIRECTION=from-accent'),
                        contains_exactly('set_var', f'ACCENT_LINE_ID={line.id}'),
                        contains_exactly('context', self.context.name),
                        contains_exactly('set_var', f'TRANSFER_CONTEXT={self.context.name}'),
                    )
                )
