...
import accent_fetchfw.storage as storage
...
class TestBasePkgStorage(unittest.TestCase):
    ...
    def test_dependencies_with_maxdepth_1_return_direct_deps(self):
        pkgs = {'a': self._new_pkg(['b']), 'b': self._new_pkg(['c']), 'c': self._new_pkg()}
        pkg_sto = storage.BasePkgStorage()
        pkg_sto._pkgs = pkgs
    ...
    def test_dependencies_multiple_depths_consider_shortest_depth(self):
        pkgs = {'a': self._new_pkg(['c']), 'b': self._new_pkg(['d']), 'c': self._new_pkg(['d']), 'd': self._new_pkg()}
        pkg_sto = storage.BasePkgStorage()
        pkg_sto._pkgs = pkgs
        self.assertEqual(['c', 'd'], sorted(pkg_sto.get_dependencies_many(['a', 'b'], maxdepth=1)))
    ...
    def test_dependencies_works_fine_with_indirect_cycle(self):
        pkgs = {'a': self._new_pkg(['b']), 'b': self._new_pkg(['c']), 'c': self._new_pkg(['a'])}
        pkg_sto = storage.BasePkgStorage()
        pkg_sto._pkgs = pkgs
