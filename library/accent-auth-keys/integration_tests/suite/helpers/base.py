...
class BaseIntegrationTest(AssetLaunchingTestCase):
    ...
    def setup_auth(cls):
        ...
        token = cls.auth.token.new('accent_user', expiration=3600)
    ...
    def _service_clean(self, users=False):
        ...
        cmd = ['accent-auth-keys', 'service', 'clean', *flags]
    ...
    def _service_update(self, recreate=False):
        ...
        cmd = ['accent-auth-keys', 'service', 'update', *flags]
        output = self.docker_exec(cmd)
        result = output.decode('utf-8')
    ...
    def _list_filenames(self):
        cmd = ['ls', '/var/lib/accent-auth-keys']
        output = self.docker_exec(cmd)
    ...
    def _get_owner(self, filename):
        cmd = ['stat', '-c', '%U', f'/var/lib/accent-auth-keys/{filename}']
        output = self.docker_exec(cmd)
    ...
    def _create_filename(self, filename):
        cmd = ['touch', f'/var/lib/accent-auth-keys/{filename}']
        self.docker_exec(cmd)

    def _delete_filename(self, filename):
        cmd = ['rm', f'/var/lib/accent-auth-keys/{filename}']
        self.docker_exec(cmd)

    def _copy_override_filename(self, filename):
        override_path = f'etc/accent-auth-keys/conf.d/{filename}'
    ...
    def _delete_override_filename(self, filename):
        self.docker_exec(['rm', f'/etc/accent-auth-keys/conf.d/{filename}'])

    def _get_last_modification_time(self, filename):
        cmd = ['stat', '-c', '%Y', f'/var/lib/accent-auth-keys/{filename}']
        output = self.docker_exec(cmd)
        result = output.decode('utf-8')
    ...
    def _get_service_config(self, service_name):
        cmd = ['cat', f'/var/lib/accent-auth-keys/{service_name}-key.yml']
        output = self.docker_exec(cmd)
