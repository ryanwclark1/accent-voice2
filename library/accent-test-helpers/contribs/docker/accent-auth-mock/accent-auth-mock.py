...
def external_auth_external_service_get(user_uuid: str, external_service: str) -> Response | tuple[str, int]:
    if external:
...
def add_invalid_credentials() -> tuple[str, int]:
    request_body = request.get_json()
    invalid_username_passwords.append((request_body['username'], request_body['password']))
...
def add_credentials_for_invalid_token() -> tuple[str, int]:
    request_body = request.get_json()
    token_that_will_be_invalid_when_used.append((request_body['username'], request_body['password']))
...
def token_head_ok(token: str) -> tuple[str, int]:
    ...
    if required_tenant_uuid:
        ...
        if not token_tenant:
            return '', 403
        visible_tenant_uuids = [tenant['uuid'] for tenant in _find_tenant_children(token_tenant)] + [
            token_tenant['uuid']
        ]
...
def users_list() -> Response:
    return jsonify({'items': list(users.values())})
...
def users_post() -> Response:
    ...
    user = {
        ...
        'tenant_uuid': request.headers.get('Accent-Tenant', None),
    }
...
def tenants_get() -> tuple[Response | str, int]:
    ...
    specified_tenant_uuid = request.headers.get('Accent-Tenant')
...
def _filter_tenants(tenants: list[TenantDict], name: str | None = None, **kwargs: Any) -> list[TenantDict]:
    if not name:
        return tenants
