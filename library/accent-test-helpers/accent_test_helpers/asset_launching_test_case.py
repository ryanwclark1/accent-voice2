...
class AbstractAssetLaunchingHelper:
    ...
    def launch_service_with_asset(cls) -> None:
        ...
        if os.getenv('ACCENT_TEST_NO_DOCKER_COMPOSE_PULL') == '1':
    ...
    def _docker_compose_options(cls) -> list[str]:
        ...
        extra = os.getenv("ACCENT_TEST_DOCKER_OVERRIDE_EXTRA")
        if extra:
    ...
    def _maybe_dump_docker_logs(cls) -> None:
        if os.getenv('ACCENT_TEST_DOCKER_LOGS_ENABLED', '0') == '1':
    ...
    def get_log_directory() -> str:
        if not AssetLaunchingTestCase.log_dir:
            ...
            AssetLaunchingTestCase.log_dir = os.getenv(
                'ACCENT_TEST_DOCKER_LOGS_DIR', f'/tmp/accent-integration-{random_strings}'
            )
    ...
    def _is_coverage_enabled() -> bool:
        return os.getenv('ACCENT_TEST_COVERAGE_ENABLED', '0') == '1'

    @staticmethod
    def get_coverage_directory() -> str:
        coverage_dir = os.environ['ACCENT_TEST_COVERAGE_DIR']
